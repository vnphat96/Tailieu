...
%=====================================
\usepackage{amsmath,amssymb,mathrsfs,maybemath,xlop,polynom,slashbox,tabularray}
...
\usepackage{pdfpages}
%==========
\usetikzlibrary{math,through,calc,intersections,angles,quotes,shapes,shapes.geometric,arrows,patterns,snakes,matrix,chains,arrows.meta,decorations.shapes,decorations.fractals,decorations.markings,shadows,3d}
...
\checkoddpage\ifoddpage %nếu trang lẻ
		...
		\node[text=\maufoot, anchor=south west,inner sep=0pt] at ([xshift=\trai cm,yshift=4pt]AA){
			\fontfamily{qag}\fontsize{10pt}{10pt}\selectfont\bfseries\faEdit\, \tenchude
		};
		%----- Kẻ đứng
...
\checkoddpage\ifoddpage %nếu trang lẻ
	...
	%-----bên trái
	\node[text=\maufoot, anchor=south west,inner sep=0pt] at ([xshift=\trai cm,yshift=4pt]AA){
		\fontfamily{qag}\fontsize{12pt}{12pt}\selectfont\bfseries\faEdit\, \tenchude
	};
\else %chẵn
	%-----đường kẻ
	\draw[\maufoot, line width=2pt] 
...
\newcommand{\myfancyfoot}{% dưới
	...
	\checkoddpage\ifoddpage %nếu trang lẻ
		...
		%-----tên tg
		\node[anchor=west,text=\maufoot,inner sep=0pt,fill=white] at ([xshift=2*\trai cm]AA){\fontfamily{qag}\fontsize{9pt}{1pt}\selectfont\bfseries \,\,\, \href{https://www.facebook.com/vu.ngocphat.7/}{\tentacgia} \,\,\, };
	\else %chẵn
		%---kẻ
		...
		%-----tên tg
		\node[anchor=east,text=\maufoot,inner sep=0pt,fill=white] at ([xshift=-2*\trai cm]BB){\fontfamily{qag}\fontsize{9pt}{1pt}\selectfont\bfseries \,\,\, \href{https://www.facebook.com/vu.ngocphat.7/}{\tentacgia} \,\,\,};
	\fi
	\end{tikzpicture}%
}
...
% \newtheorem{bt}{\color{\mauBT}\damEX Câu}[vd]
%===================================
\theoremstyle{plain} %
\theoremheaderfont{\scshape} %đầu
...
\newtcolorbox{dang}[1]{
	...
	underlay unbroken and first={
		...
		%--Bài và số bài
		\node[circle,draw=white,fill=\maudang, line width=1pt, inner sep=3pt,text=white] (SOBAI) at ([yshift=1pt]$(bai4)!0.5!(bai3)$) {\fontsize{12.5pt}{1pt}\fontfamily{put}\selectfont\bfseries \thedang};
		% \draw ([yshift=-1.5pt]SOBAI.west) node[left,myblue,scale=1.2] {\damEX{Dạng} \,};	
	},
	title={%\faFolderOpen\ Dạng~\stepcounter{dang}\thedang.\ 
		\hspace*{1.5cm}\parbox{\textwidth-2.75cm}{#1}\stepcounter{dang}}%
	\addcontentsline{toc}{subsection}{\sffamily \faFolderOpenO \,\, Dạng~\thedang. #1}
	\setcounter{subsubsection}{0}
		\setcounter{vd}{0}
		\setcounter{ex}{0}
		\setcounter{bt}{0}
...
\hoten \dotfill {\fontsize{10}{11}\selectfont \ngaylamde}
	...
	\begin{center}
		%%---
		{\damTT\color{\mauSO} \MakeUppercase{#1}}\\[1pt]
		{\dam\color{\mauSO} \MakeUppercase{#2} -- ĐỀ \stepcounter{deso}\thedeso}\\[1pt]
		% {\dam \MakeUppercase{#2}}\\[1pt]
		{\dam\color{\mauSO} \MakeUppercase{#3}}\\[1pt]
		{\fontsize{10}{10}\selectfont \textit{#4}}\\[1pt]
		% \includegraphics[width=2cm]{QRcode/\QRde.png}\\[-1mm]
	\end{center}
		}{\includegraphics[width=3.5cm]{logo/logo.jpg}}
	\end{tcolorbox}~\\
	%%--- Phần note đầu đề
	\notename
\vspace*{0.5cm}
	\addcontentsline{toc}{section}{\indamm{Đề \thedeso: #1 --- #3}} % đưa MT vào mục lục
}
\let\oldname\name
\newcommand\tatsode{
	\renewcommand{\name}[4]{
...
\hoten \dotfill {\fontsize{10}{11}\selectfont \ngaylamde}
	...
	\begin{center}
		%%---
		{\damTT\color{\mauSO} \MakeUppercase{##1}}\\[1pt]
		{\dam\color{\mauSO} \MakeUppercase{##2} \stepcounter{deso}}\\[1pt]
		% {\dam \MakeUppercase{##2}}\\[1pt]
		{\dam\color{\mauSO} \MakeUppercase{##3}}\\[1pt]
		{\fontsize{10}{10}\selectfont \textit{##4}}\\[1pt]
		% \includegraphics[width=2cm]{QRcode/\QRde.png}\\[-1mm]
	...
	\notename
\vspace*{0.5cm}
	\addcontentsline{toc}{section}{\hspace{-1cm}\indamm{Đề \thedeso: ##2 --- ##3}} % đưa MT vào mục lục
}
}
\newcommand\batsode{\let\name\oldname}
%--Sang trang 
...
%---Hiện bảng ĐA
\let\inputansboxold\inputansbox
\let\indapanold\indapan
\newcommand{\hienDA}{
...
%---Ẩn LG
\let\oldTrueEX\TrueEX
\let\oldloigiai\loigiai
\let\oldendloigiai\endloigiai
\newcommand{\anLG}{	
...
%---Hiện LG
\newcommand{\hienLG}{
	\renewcommand{\TrueEX}{\oldTrueEX}
	
	\let\loigiai\oldloigiai
	\let\endloigiai\oldendloigiai
	%---
	\def\loigiaiEXS{}
	%\choiceTF
...
%%%%%%%%%%%%%%%%%%%%%%%
\renewcommand{\shortansfix}[2][]{%
	\ifthenelse{\equal{#1}{0}}{}
		{\ifthenelse{\equal{#1}{}}
			{\par\noindent\hfill\oKQ{\widthSA}}
			{\ifthenelse{\equal{#1}{oly}}
				{\par \noindent\hfill\oKQ[oly]{4}}
				{\ifthenelse{\equal{#1}{...}}
					{\par\noindent\hfill\oKQ[...]{\widthSA}}
					{\par\noindent\hfill\oKQ{#1}}}}%
...
\newcommand{\boxmini}[1]{
	\begin{center}
		\tcbox[enhanced, breakable,colback=cyan!0,boxrule=.4mm,colframe=myblue]
		{\itshape\color{\mauEX} #1}
	\end{center}
}
...
\newcommand{\indamm}[1]{
	{\fontsize{12pt}{9pt}\sffamily \selectfont\bfseries\color{\maudl} #1}
}
\newcommand{\ind}[1]{
	{\fontsize{11pt}{9pt}\sffamily \selectfont\bfseries\color{\maucham} #1}
}
%%%===Các biểu tượng===
...
\newcommand{\dongcham}[1]{
	\ifthenelse{\equal{#1}{0}}
	{.\dotfill}
	{\ifthenelse{\equal{#1}{1}}{.\dotfill}
	{\def\sod{#1}
	\pgfmathsetmacro{\sodong}{\sod -1} 
...
\def\TL{
    {\bfseries Phần IV. Câu hỏi tự luận.}
}

\def\tickv{\faCheckCircleO~}
\def\caulc{\TN}
\def\cauds{\TNTF}
\def\caukq{\TNSA}
...
\newcommand{\baitaptn}{\subsubsection{Bài tập trắc nghiệm}}

\newcommand{\indamct}[1]{\boldmath #1 \unboldmath}