...
\hfill {\fontsize{10}{11}\selectfont \ngaylamde}
	...
	\begin{center}
		...
		{\fontsize{10}{10}\selectfont #4}\\[-1mm]
	\end{center}
	\end{tcolorbox}
	%%--- Phần note đầu đề
	\notename
...
\newcommand{\hienLG}{
	...
	\def\loigiaiEXS{}
	%\choiceTF
	\renewcommand{\writekeyTFone}{\gdef\TrueX{}\gdef\FalseX{\tick}}
	\renewcommand{\writekeyTF}{%
		&\centering\leavevmode\TrueX%
		&\parbox[t]{\linewidth}{\centering\leavevmode\FalseX}%
			\gdef\TrueX{}\gdef\FalseX{\tick}%
	}
...
\def\TNTF{
    {\bfseries Trong mỗi ý a), b), c) và d) ở mỗi câu, học sinh chọn đúng hoặc sai.}
}
\def\TN{
    {\bfseries Mỗi câu hỏi học sinh chọn một phương án.}
}
\def\BTTL{
    \begin{center}
