...
\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình bên. Tìm số nghiệm của phương trình $f(x)=-2$.
\choice
{$1$}
{$4$}
{\True $3$}
...
\end{tikzpicture}}
\loigiai
{\immini[thm]{Số nghiệm của phương trình $f(x)=-2$ bằng số giao điểm giữa đồ thị hàm số $y=f(x)$ và đường thẳng $y=-2$.\\
Dựa vào đồ thị, đường thẳng $y=-2$ cắt đồ thị hàm số $y=f(x)$ tại $3$ điểm phân biệt.\\
Vậy phương trình $f(x)=-2$ có $3$ nghiệm phân biệt.}{\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round,>=stealth,x=0.6cm,y=0.6cm]
\def \xmin{-1.5};
\def \xmax{4.9};
\def \ymin{-3.5};
\def \ymax{2.0};
...
{$4$}
\loigiai
{\immini[thm]{Số đỉnh của một khối lăng trụ tam giác là $6$.}{\begin{tikzpicture}[scale=0.8, font=\footnotesize, line join=round, line cap=round,>=stealth]
...
{$\{3;4\}$}
\loigiai
{\immini[thm]{Mỗi mặt của khối lập phương là tứ giác đều, mỗi đỉnh của khối lập phương là đỉnh chung của đúng $3$ mặt.\\
...
Tổng số đỉnh, số cạnh, số mặt của bát diện đều là	
	...
	{$30$}{$28$}{$14$}{\True $26$}
	\loigiai{
	\immini[thm]{	Bát diện đều có tổng số đỉnh, số cạnh, số mặt bằng $6+12+8=26$.}{
...
\begin{ex}%[Thi giữa kỳ 1, trường THPT Nguyễn Dục,Sở GD và ĐT - Quảng Nam, năm 2020 - 2021]%[Ninh Tiến Nam, 12EX32021]%[2H1Y3-2]
	...
	\loigiai{
\immini[thm]{		Thể tích khối chóp $S.ABCD$ bằng
...
{có giá trị lớn nhất bằng $-1$}
	\loigiai{
\immini[thm]{Ta có $y'=-3x^2+3$,\[y'=0\Leftrightarrow-3x^2+3=0 \Leftrightarrow \heva{&x=1\in (0;+\infty)\\&x=-1\not\in (0;+\infty).}\]	
Bảng biến thiên của hàm số như hình bên.\\Vậy hàm số có giá trị lớn nhất trên khoảng $(0;+\infty)$ bằng $3$.}{	\begin{tikzpicture}[line join=round, line cap=round,>=stealth,xscale=1,yscale=.8]
\begin{scope}[shift={(-.5,.5)}]
\def\a{6}
\def\b{4}
\draw (0,0) rectangle+(\a,-\b)
...
\immini[thm]{Trong các hàm số sau đây, hàm số nào có đồ thị như hình vẽ bên? 
\choice
{$y=x^3-3x+1$}
{\True $y=-x^3+3x+1$}
{$y=x^3+2$}
{$y=-x^4+3x$}}{\begin{tikzpicture}[scale=0.8, font=\footnotesize, line join=round, line cap=round,>=stealth,x=0.5cm, y=0.5cm]
\def \xmin{-2.5};
\def \xmax{2.7};
\def \ymin{-1.5};
\def \ymax{3.5};
...
\begin{ex}%[Thi giữa kỳ 1, trường THPT Nguyễn Dục,Sở GD và ĐT - Quảng Nam, năm 2020 - 2021]%[Ninh Tiến Nam, 12EX32021]%[2H1B3-2]
	...
	\loigiai{
		\immini[thm]{Xét khối lăng trụ tam giác đều $ABC.A'B'C$ có các cạnh bằng $2a$.\\Diện tích  đáy $ABC$ bằng 
...
\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình vẽ. Tìm số điểm cực trị của hàm số $y=|f(x)|$.
	\choice
	{$4$}{$6$}{\True $2$}{$3$}	}{\begin{tikzpicture}[>=stealth,line join=round,line cap=round,font=\footnotesize,scale=1]
...
\immini[thm]{		Ta có 
		...
		&-f(x),\ \text{nếu}\ f(x)< 0.}\]
	Đồ thị hàm số $y=|f(x)|$ có dạng như hình vẽ bên.}{\begin{tikzpicture}[>=stealth,line join=round,line cap=round,font=\footnotesize,scale=1]
		\draw[->](-2,0)--(2,0)node[scale=.9,below]{$x$};
...
\immini[thm]{Cho hàm số $y=f(x)=ax^3+bx^2+cx+d$, ($a, b, c, d\in\mathbb{R}$) có đồ thị như hình vẽ bên. Chọn khẳng định đúng trong các khẳng định sau.
\choice
{$a>0$, $b<0$, $d<0$}
{$a<0$, $b<0$, $d<0$}
{$a>0$, $b>0$, $d>0$}
{\True $a>0$, $b>0$, $d<0$}}{\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round,>=stealth,x=0.6cm, y=0.6cm]
\def \xmin{-4.9};
\def \xmax{3.0};
\def \ymin{-2.0};
\def \ymax{2.5};
...
{$(-1;2)$}{$(-\infty;-1)$}{\True $(-1;0)$}{$(-1;1)$}	
	...
	\loigiai{
\immini[thm]{	Ta có $g'(x)=2f'(x)+2x$, $g'(x)=0\Leftrightarrow f'(x)=-x$.\\
...
\begin{ex}%[Thi giữa kỳ 1, trường THPT Nguyễn Dục,Sở GD và ĐT - Quảng Nam, năm 2020 - 2021]%[Ninh Tiến Nam, 12EX32021]%[2H1K3-2]
	...
	\loigiai{
		\immini[thm]{Gọi $O$ là tâm của tam giác  $ABC$, $M$ là trung điểm của  $AB$.\\
...
tam giác $IAB$ đạt giá trị lớn nhất. Tổng $m_1+ m_2$ bằng	
	...
	\loigiai{
		Ta có $y'=3x^2-3m$ và $y=\dfrac{1}{3}x\cdot y'-2mx+2$.\\
		 Hàm số có hai cực trị khi và chỉ khi $m>0$, khi đó PTĐT đi qua hai điểm cực trị là $d\colon y=-2mx+2$, ($m>0$).
\immini[thm]{Gọi $H$ là hình chiếu vuông góc của $I$ lên $d$.\\
Ta có
...
{$4$}
\loigiai
{\immini{Số đỉnh của một khối lăng trụ tam giác là $6$.}{\begin{tikzpicture}[scale=0.8, font=\footnotesize, line join=round, line cap=round,>=stealth]
\path (0,0)coordinate (A) (3,0) coordinate (C) (1,-1) coordinate (B) (1,3) coordinate (h);
\foreach \p in {A,B,C} {\draw (\p)--(\p)--+(h) coordinate (\p');}
\draw (A')--(B')--(C')--cycle (A)--(B)--(C);
\draw[dashed] (A)--(C);
