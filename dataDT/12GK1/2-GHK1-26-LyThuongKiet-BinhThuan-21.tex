...
\immini[thm]{Đường cong trong hình vẽ bên là đồ thị của hàm số nào dưới đây?
\choice
{$y=x^4-\dfrac{3}{2}x^2+1$}
{\True $y=-x^4+\dfrac{3}{2}x^2+1$}
{$y=x^3-3x+1$}
{$y=-x^3-3x+1$}}{\begin{tikzpicture}
...
\end{ex}
\begin{ex}%[Đề thi giữa HKI trường THPT Lý Thường Kiệt, Bình Thuận 2020]%[Trần Hòa, dự án 12-EX-3-2021]%[2D1Y1-2] 
\immini[thm]{Cho hàm số $f(x)$ xác định trên $\mathbb{R}$ và có đồ thị như hình vẽ bên. Mệnh đề nào dưới đây đúng?
\choice
...
{Hàm số $f(x)$ đồng biến trên khoảng $(-2;0)$}}{\begin{tikzpicture}
[scale=1, font=\footnotesize, line join=round, line cap=round, >=stealth]
...
\immini[thm]{Cho hình tứ diện $ABCD$ có $AB$, $AC$, $AD$ đôi một vuông góc. $AB=4a$, $AC=6a$, $AD=2a$. Gọi $M$ là trung điểm cạnh $AC$. Tính thể tích khối chóp $B.CDM$.
\choice
{$24a^3$}
{$8a^3$}
{$12a^3$}
...
\end{ex}
\begin{ex}%[Đề thi giữa HKI trường THPT Lý Thường Kiệt, Bình Thuận 2020]%[Trần Hòa, dự án 12-EX-3-2021]%[2H1B3-2] 
\immini[thm]{Cho khối hộp chữ nhật $ABCD.A'B'C'D'$ có $AA'=a$, $AB=4a$, $BC=3a$. Gọi $O$ là trung điểm đường chéo  $BD'$. Tính thể tích khối chóp $O.BCC'B'$.
\choice
{\True $2a^3$}
{$a^3$}
{$3a^3$}
...
\end{tikzpicture}}
\loigiai{
\immini[thm]{Gọi $H$ là trung điểm $BC'$ ta có $V_{O.BCC'B'}=\dfrac{1}{3}OH\cdot S_{BCC'B'}=\dfrac{1}{6}AB\cdot BC\cdot BB'=2a^3$.}{\begin{tikzpicture}[scale=1,font=\footnotesize,line join = round, line cap = round, >= stealth]
\coordinate (A) at (0,0);
\def\x{4}
\def\y{2}
\def\z{3}
...
\end{ex}
\begin{ex}%[Đề thi giữa HKI trường THPT Lý Thường Kiệt, Bình Thuận 2020]%[Tran Tony, dự án 12-EX-3-2021]%[2D1Y4-1]
	\immini[thm]{Cho hàm số $y = f(x)$ có đồ thị như hình vẽ bên. Chọn khẳng định \textbf{sai}.
		\choice
...
\immini[thm]{Biết hàm số $y=f(x)$ có đạo hàm liên tục trên $\mathbb{R}$ và có đồ thị $f'(x)$ như hình vẽ bên. Tìm số điểm cực trị của hàm số $f(x)$.
\choice
{$4$}
{$1$}
{\True $3$}
...
\immini[thm]{Cho hàm số $y=ax^3+bx^2+cx+d$ có đồ thị như hình vẽ bên. Mệnh đề nào sau đây \textbf{sai}?
\choice
{$bd<0$}
{\True $bc<0$}
{$ac<0$}
...
\immini[thm]{Cho hàm số $y=f(x)$ liên tục trên đoạn $\left[\dfrac{1}{4};\dfrac{7}{3}\right]$ có đồ thị hàm số $y=f'(x)$ như hình vẽ bên. Hàm số đạt giá trị lớn nhất trên đoạn $\left[\dfrac{1}{2};3\right]$ tại điểm $x_0$ nào dưới đây?
\choice
{\True $x_0=\dfrac{1}{2}$}
{$x_0=1$}
{$x_0=3$}
