...
\TN
\Opensolutionfile{ans}[ans/ansc1l5-Phan-I]
...
\TNTF
\Opensolutionfile{ans}[ans/ansc1l5-Phan-II]
...
\TNSA
\Opensolutionfile{ans}[ans/ansc1l5-Phan-III]
...
\end{ex}
\begin{ex}%[TeX hóa đề Toán 12]%[Trần Văn Hùng]%[2D1C3-1]
	\immini{
	Cho hàm số $y=f(x)$ liên tục trên $\mathbb{R}$ và có đồ thị như hình vẽ bên. Xét hàm số $g(x)=f(x^3+x-1)+m^2+2m$ . Gọi $S$ là tập hợp chứa các giá trị thực của $m$ để $\max\limits_{[0;1]}g(x)=3$. Tính tổng các phần tử của tập $S$.	
	}
	{
	...
	\begin{scope}
		...
		\draw[samples=200,domain=-2.5:3,smooth,variable=\x] plot (\x,{1*((\x)^3)+0*((\x)^2)+-3*(\x)+1});
	\end{scope}
	\end{tikzpicture}
	}
	\shortans{$-2$}
	\loigiai{
	Dựa vào đồ thị hàm số $y=f(x)$ có $f'(x)=0\Leftrightarrow x=\pm 1$ và $f(1)=-1$; $f(-1)=3$.\\
	Ta có $g'(x)=(3x^2+1)f'(x^3+x-1)$; $f'(x^3+x-1)=0\Leftrightarrow \hoac{&x^3+x-1=-1\\&x^3+x-1=1}\Leftrightarrow \hoac{&x=0\\&x=1.}$\\
	Mặt khác $g(0)=f(-1)+m^2+2m=m^2+2m+3$ và $g(1)=f(1)+m^2+2m=m^2+2m-1$.\\
	Vì $g(0)>g(1)$ nên $\max\limits_{[0;1]}g(x)=g(0)$.\\
	Theo giả thiết suy ra $m^2+2m+3=3\Leftrightarrow \hoac{&m=0\\&m=-2.}$\\
	Suy ra $S=\{-2;2\}$.\\
	Vậy tổng các phần tử của tập $S$ bằng $-2$.
	}
\end{ex}

\begin{ex}%[Dự án 2025 - đề cấu trúc mới, Hung Doan]%[2D1C3-6]
	Ông A muốn xây dựng một bình chứa nước hình trụ có thể tích $150$ m$^3$. Đáy làm bằng bê tông giá $100$ nghìn VNĐ/m$^2$, thành làm bằng tôn giá $90$ nghìn VNĐ/m$^2$, nắp bằng nhôm không gỉ giá $120$ nghìn VNĐ/m$^2$. Tìm chiều cao của bình để chi phí xây dựng là thấp nhất?
	\shortans{$6{,}58$}
	\loigiai{
		Gọi $r$, $h$ lần lượt là bán kính đáy và chiều cao của bình chứa hình trụ $(r,h>0)$.\\
		Khi đó $V=pr^2h=150 \text{m}^3\Rightarrow h=\dfrac{150}{\pi r^2}$.\\
		Tổng chi phí xây dựng là $P(r)=100\cdot S_{\text{đáy bình}}+90S_{\text{xung quanh}}+120\cdot S_{\text{nắp bình}}$.\\
		$\Rightarrow P(r)=220S_{\text{đáy}}+90S_{\text{xung quanh}}=220\pi r^2+90(2\pi rh)=220\pi r^2+\dfrac{27000}{r}$.\\
		Bài toán trở thành tìm $\min P(r)=$ với $r>0$.\\
		Ta có $P'(r)=440\pi r-\dfrac{27000}{r^2}$, $P'(r)=0\Leftrightarrow r_o=\sqrt[3]{\dfrac{675}{11\pi }}$.\\
		Lập bảng biến thiên, ta được
		\begin{center}
			\begin{tikzpicture}
				\tkzTabInit[nocadre=false,lgt=2,espcl=3,deltacl=.55]
				{$r$/0.7, $P'(r)$/0.7, $P(r)$/2}
				{$0$,$r_0$,$+\infty$}
				\tkzTabLine{,-,$0$,+,}
				\tkzTabVar{+/$ $,-/$\min P(r)$,+/$ $}	
			\end{tikzpicture}
		\end{center}
		Dựa vào bảng biến thiên ta thấy yêu cầu bài toán \\$\Leftrightarrow r=\sqrt[3]{\dfrac{675}{11\pi }}$ và $h=\dfrac{150}{\pi \left(\sqrt[3]{\dfrac{675}{11\pi }}\right)^2}\approx 6{,}58$.}
\end{ex}
\Closesolutionfile{ans}
% \begin{indapan}
% 	{ans/ansc1l5}
