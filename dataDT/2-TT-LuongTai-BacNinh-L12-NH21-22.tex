...
\begin{ex}%[Dự án 16 - TeamTeXHoa - TT Sở Trường - Hồng Trường Sơn]%[2D1Y1-2]
	\immini[thm]{Cho hàm số $y=f\left( x \right)$ có đồ thị là đường cong trong hình vẽ. Hàm số đã cho đồng biến trên khoảng nào dưới đây?
	\choice
	{$\left( -\infty ;-1 \right)$}
	{$\left( 1;+\infty \right)$}
	{\True $\left( -1;1 \right)$}
...
%%==========Câu 27
	\begin{ex}%[2D3Y2-1]
	Nếu $\displaystyle\int\limits_{-1}^2f(x)\mathrm{\,d}x=8$ thì tích phân $\displaystyle\int\limits_{-1}^2{\left[ 3f\left( x \right)+2 \right]\mathrm{\,d}x}$ bằng
	\choice[0.4em]
	{ $10$}
	{ $22$}
	{ $26$}
	{\True $30$}
	\loigiai{
		Ta có $\displaystyle\int\limits_{-1}^2{\left[ 3f\left( x \right)+2 \right]\mathrm{\,d}x}=3\int\limits_{-1}^2{f\left( x \right)\mathrm{\,d}x}+\int\limits_{-1}^2{\text{2d}x}=3\cdot8+ 2x {\Big|}_{-1}^2=24+\left( 4+2 \right)=30$.}
\end{ex}
%%==========Câu 28
\begin{ex}%[2H1Y3-2]
...
%%==========Câu 31
\begin{ex}%[2D1Y5-1]
	\immini[thm]{Đường cong trong hình vẽ bên là đồ thị của hàm số nào?
		\choice[0.4em]
		{ $y=-x^3+3x+1$}
		{\True $y=2x^4-4x^2+1$}
		{ $y=-2x^4+4x^2+1$}
...
\begin{ex}%[2H3Y3-1]
	...
	\loigiai{Ta có $\overrightarrow{u_4}=\left( -2;1;0 \right)$ là một vectơ chỉ phương của $\Delta$.}
\end{ex}
%%==========Câu 36
\begin{ex}%[2H3B2-3]
...
\begin{ex}%[Dự án 16 - TT-LuongTai-BacNinh-L12-NH21-22]%[2D3K3-1]
	\immini[thm]{Cho hàm số $ y=f(x)$ liên tục trên $\mathbb{R}$ có đồ thị như hình vẽ. Giả sử diện tích phần kẻ dọc trên hình vẽ có diện tích bằng $a$. Tính theo $a$ giá trị của tích phân $I = \displaystyle\int\limits_{-3}^2 (2x + 1 )f'(x)\mathrm{\,d}x$?
		\choice
		{\True $I = 50 - 2a$}
		{$I = 50 - a$}
		{$I = -30 - 2a$}
...
\begin{ex}%[Dự án 16 - TT-LuongTai-BacNinh-L12-NH21-22]%[2D1K3-1]
	\immini[thm]{Cho hàm số $ y = f(x)$ liên tục trên $\mathbb{R}$ và có đồ thị của hàm số $f'(x)$ như hình vẽ bên. Tìm giá trị nhỏ nhất của hàm số $ y = f(x)$ trên đoạn $[-1; 2]$?
		\choice
		{$f(2)$}
		{$1$}
		{$f(-1)$}
...
\begin{ex}%[Dự án 16 - TT-LuongTai-BacNinh-L12-NH21-22]%[1H3B3-3]
	...
	{$90^\circ$}
	\loigiai{
		\immini[thm]{Gọi $H$ là trung điểm của $AB$.\\
...
\begin{ex}%[Dự án 16 - TT-LuongTai-BacNinh-L12-NH21-22]%[1H3K5-3]
	\immini[thm]{Cho lăng trụ tam giác đều $ABC.A'B'C'$ có cạnh đáy bằng $4a$. Góc giữa hai mặt phẳng $(A'BC)$ và $(ABC)$ bằng $30^\circ$. Gọi $M$ là trung điểm của cạnh $AB$. Tính khoảng cách từ điểm $M$ đến mặt phẳng $(A'BC)$?\\
		\choice
	...
	\end{tikzpicture}}
	\loigiai{
		\immini[thm]{Gọi $N$ là trung điểm của $BC$.\\
...
\begin{ex}%[2H1K3-2]
	...
	{ $18a^3$}
	\loigiai{
		\immini[thm]{Gọi $V$ là thể tích lăng trụ $ABC.A'B'C'$.\\
...
\end{center}
		...
		Do đó $S=\left|\displaystyle\int\limits_{60}^{12}\dfrac{\mathrm{\,d}t}{t}\right|=\left|\ln t\biggl|_{60}^{12}\right|=\left|\ln 12-\ln 60\right|=\left|\ln \dfrac{12}{60}\right|=|-\ln 5|=\ln 5$.}
\end{ex}
%%==========Câu 49
\begin{ex}%[2H3G3-7]
	Trong KG $Oxyz$, cho điểm $A(2;4;-2)$ và mặt phẳng $(P)\colon (m^2+1)x+(m^2-1)y+2mz+4=0$. Biết rằng, khi tham số thay đổi thì mặt phẳng $(P)$ luôn tiếp xúc với hai mặt cầu cố định cùng đi qua $A$ là $(S_1)$, $(S_2)$. Gọi $M$ và $N$ lần lượt là hai điểm nằm trên $(S_1)$ và $(S_2)$. Tìm giá trị lớn nhất của $MN$.
	\choice
	{ $16\sqrt{2}$}
	{\True $8+8\sqrt{2}$}
	{ $8\sqrt{2}$}
...
%%==========Câu 46
\begin{ex}%[2D1G5-3]
	\immini{Cho hàm số $ y=f(x)$ là hàm số bậc ba có đồ thị như hình vẽ. Tìm tất cả các giá trị của tham số $ m$ sao cho phương trình $ f(\sin x)=f(m+1)$ có nghiệm?
	\choice
	{ $-1\le m\le3$}
	{ $-2\le m\le0$}
	{\True $-3\le m\le1$}
