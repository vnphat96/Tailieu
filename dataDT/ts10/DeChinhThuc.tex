    ...
    \loigiai{
        \begin{center}
            \begin{tikzpicture}[line join = round, line cap = round, thick, font = \small, scale = .7,declare function ={R=4;}]
                ...
                ($(A)!.5!(B)$) coordinate (N);
                \path (intersection of A--{$($(A)!1mm!(B)$)!.5!($(A)!1mm!(M)$)$} and B--{$($(B)!1mm!(A)$)!.5!($(B)!1mm!(M)$)$}) coordinate (I);
                \foreach \x/\y/\z in {C/B/A,D/A/O,M/B/O,M/N/A}{
                        \draw pic[draw, angle radius = 6pt]{right angle = \x--\y--\z};
                    }
                \draw (O)circle(4);
                \draw (A)--(C)--(D)--cycle (M)--(O)--(H)--(B)--cycle (M)--(C) (O)--(B)--(A)--(I);
                \foreach \d/\g in {O/135,A/45,B/-45,C/-135,M/45,H/135,D/-45,K/90,I/135,N/135}
        ...
        \begin{enumerate}[1.]
            ...
            \item Gọi $J$ là giao điểm của tia $OM$ và đường tròn $(O;R)$.\\
                  ...
                  Gọi $N$ là giao điểm $AB$ và $OM$. Ta dễ dàng chứng minh được $IN$ vuông góc $AB$ hay $IN=r$ là bán kính đường tròn nội tiếp tam giác $MAB$.
        \end{enumerate}
    }
\end{bt}