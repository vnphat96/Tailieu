...
\begin{ex}%[2D1K2-3]%
	...
	{$m=1$}
	\loigiai{
		Ta có $y'=x^2-2mx+\left(m^2-4\right)$ ; $y^{\prime \prime}=2x-2m$ .\\
...
\begin{ex}%[2D1K2-3]%
	...
	{$m\in\left\{1;2\right\}$}
	\loigiai{
		Ta có $y' = 3x^2 -4mx + m; y^{\prime \prime} = 6x - 4m$.\\
...
\begin{ex}%[2D1K2-3]
	...
	{$0<m\le 4$}
	\loigiai{
		$y'=3x^2-6x+m$; $y^{\prime \prime}=6x-6$.\\
...
\begin{ex}%[2D1K2-3]%[THPT An Lão Hải Phòng 2019]
	...
	{$3$}
	\loigiai{
		$y'=x^2-2mx+m^2-m+1; y^{\prime \prime}=2x-2m$\\
...
\begin{ex}%[2D1K2-3]%[THPT Thăng Long-Hà Nội-Lần 2-2019]
	...
	{$\left\{1\right\}$}
	\loigiai{
		Ta có $y'=3x^2+2\left(3m-1\right)x+m^2\Rightarrow y^{\prime \prime}=6x+6m-2$.\\
...
\begin{ex}%[2D1K2-3]%[THPT Kinh Môn-2019]
	...
	\loigiai{
		...
		Ta có $y'=x^2-2mx+m+1; y^{\prime \prime}=2x-2m$.\\
		Hàm số đạt cực đại tại $x=-2 \Leftrightarrow \heva{& f'(-2)=0\\ & f^{\prime \prime}(-2)<0} \Leftrightarrow \heva{& 5m + 5 = 0 \\ & -4-2m<0} \Leftrightarrow \heva{& m =-1\\& m >-2} \Leftrightarrow m = -1$.
	}
\end{ex}
\begin{ex}%[2D1K2-3]%[Chuyên ĐHSPHN-Lần 3-2019]
	...
	{$\mathbb{R}$}
	\loigiai{
		$y'=3x^2-6mx+m+2; y^{\prime \prime}=6x-6m$.\\
		Hàm số đạt cực tiểu tại $x=1 \Leftrightarrow \left\{\begin{aligned}
			&y'(1)=0\\ 
			&y^{\prime \prime}(1)>0\\ 
		\end{aligned}\right.\Leftrightarrow\left\{\begin{aligned}
...
\begin{ex}%[2D1K2-3]%[Chuyên QH Huế-Lần 2-2019]
	...
	{$m=6$}
	\loigiai{
		$y'=f'(x)=1+\dfrac{m}{2\sqrt{x}},\left(x>0\right)$\\
		Để hàm số đạt cực trị tại $x=1$ thì $f'(1)=0\Leftrightarrow 1+\dfrac{m}{2}=0\Leftrightarrow m=-2$.\\
...
\begin{ex}%[2D1G2-3]%[Mã 101-2018]
	...
	\loigiai{
		...
		\begin{eqnarray*}
			...
			y'&=&0\Leftrightarrow{x^3}\left(8x^4+5\left(m-2\right)x-4\left(m^2-4\right)\right)=0\\
...
\begin{ex}%[2D1G2-3]%[Chuyên Quang Trung-Bình Phước 2019]
	...
	{\True $m>0$}
	\loigiai{
		Đặt $f(x)=\dfrac{x^5}{5}-\dfrac{m{x^4}}{4}+2$. Ta có $f'(x)=x^4-m{x^3}$.
		\begin{itemize}
			\item Khi $m=0$ thì $f'(x)=x^4\ge 0$, $\forall x\in\mathbb{R}$ nên hàm số không có cực trị.
			\item 	Khi $m\ne 0$, xét $f'(x)=0\Leftrightarrow{x^4}-m{x^3}=0\Leftrightarrow{x^3}\left(x-m\right)=0\Leftrightarrow\hoac{&x=0\\ 
				&x=m.}$		
			\begin{itemize}
...
\begin{ex}%[2D1G2-3]
	...
	\loigiai{
		...
		Ta xét $m\ne 1\Rightarrow{y}'=\left(m-1\right){x^4}+\left(m+2\right){x^3}\Rightarrow y'=0\Rightarrow\hoac{&{x_1}=0\\ 
			&{x_2}=-\dfrac{m+2}{m-1}.}$
		\begin{itemize}
			\item	\textbf{Trường hợp 1.} Xét $m>1$, suy ra $x_2<x_1$. Ta có bảng xét dấu $y'=\left(m-1\right){x^4}+\left(m+2\right){x^3}$.
			\begin{center}
				\begin{tikzpicture}
					\tkzTabInit[nocadre=false,lgt=1.2,espcl=2.5,deltacl=0.6]
					{$x$/1.2,$y'$/0.6}
			...
			\end{center}
			Dựa vào bảng xét dấu ta thấy $x=0$ là điểm cực tiểu. Suy ra $m>1$ (loại).
			\item \textbf{Trường hợp 2.} $-2<m<1$, suy ra $x_2>x_1$ . Ta có bảng xét dấu $y'=\left(m-1\right){x^4}+\left(m+2\right){x^3}$.
			\begin{center}
				\begin{tikzpicture}
					\tkzTabInit[nocadre=false,lgt=1.2,espcl=2.5,deltacl=0.6]
					{$x$/1.2,$y'$/0.6}
			...
			\item \textbf{Trường hợp 3.} $m<-2$, suy ra $x_2<x_1$ . Ta có bảng xét dấu $y'=\left(m-1\right){x^4}+\left(m+2\right){x^3}$.
			\begin{center}
				\begin{tikzpicture}
					\tkzTabInit[nocadre=false,lgt=1.2,espcl=2.5,deltacl=0.6]
					{$x$/1.2,$y'$/0.6}
...
\begin{ex}%[2D1G2-3]%[Mã 104-2018]
	...
	\loigiai{
		...
		\begin{eqnarray*}
			...
			y'&=&0\Leftrightarrow{x^3}\left(8x^4+5\left(m-3\right)x-4\left(m^2-9\right)\right)=0.\\
...
\begin{ex}%[2D1G2-3]%[Mã 103-2018] 
	...
	{$9$}
	\loigiai{
		Ta có $y'=8x^7+5\left(m-5\right){x^4}-4\left(m^2-16\right){x^3}x^3\left[8x^4+5\left(m-4\right)x-4\left(m^2-16\right)\right]=x^3\cdot g(x)$.\\
		Với $g(x)=8x^4+5\left(m-5\right)x-4\left(m^2-16\right)$.
		\begin{itemize}
			\item\textbf{Trường hợp 1.} $g(0)=0\Leftrightarrow m=\pm 4$.
			\begin{itemize}
				\item Với $m=4\Rightarrow y'=8x^7$. Suy ra $x=0$ là điểm cực tiểu của hàm số.
				\item Với $m=-4\Rightarrow y'=8x^4\left(x^3-5\right)$. Suy ra $x=0$ không là điểm cực trị của hàm số.
			\end{itemize}
			\item \textbf{Trường hợp 2.} $g(0)\ne 0\Leftrightarrow m\ne\pm 4$.\\
			Để hàm số đạt cực tiểu tại $x=0$ thì qua giá trị $x=0$ dấu của $y'$ phải chuyển từ âm sang dương do đó $g(0)>0\Leftrightarrow-4<m<4$.
		\end{itemize}
...
\begin{ex}%[2D1G2-3]
	...
	{$10$}
	\loigiai{
		Ta có $y'=12x^{11}+7(m-5){x^6}+6(m^2-25){x^5}$.
		\begin{itemize}
			\item \textbf{Trường hợp 1.} $m=5\Rightarrow y'=12x^{11}$. \\Khi đó $y'=0\Leftrightarrow x=0$ là nghiệm bội lẻ, đồng thời dấu của $y$ đổi từ âm sang dương, nên $x=0$ là điểm cực tiểu của hàm số,do đó không thỏa mãn, $m=5$ (loại).
			\item \textbf{Trường hợp 2.} $m=-5\Rightarrow y'=x^6(12x^5-70)=0\Rightarrow x=0$ là nghiệm bội chẵn, do đó $y$ không đổi dấu khi đi qua $x=0$, $m=-5$ (loại).
			\item \textbf{Trường hợp 3.} $m\ne\pm 5\Rightarrow y'=x^5\left[12x^6+7(m-5)x+6(m^2-25)\right]=x^5.g(x)$.\\
			Với $g(x)=12x^6+7(m-5)x+6(m^2-25)$, ta thấy $x=0$ không là nghiệm của $g(x)$.\\
			Để hàm số đạt cực đại tại $x=0$ thì $y’$ phải đổi dấu từ dương sang âm khi đi qua $x=0$, xảy ra khi và chỉ khi $\heva{&\mathop {\lim \limits_{n \to +\infty}}\limits_{x\to {0^-}}g(x)<0\\ 
				&\mathop {\lim \limits_{n \to +\infty}}\limits_{x\to {0^+}}g(x)<0}\Leftrightarrow 6(m^2-25)<0\Leftrightarrow-5<m<5.$
		\end{itemize}
...
\begin{ex}%[2D1G2-3]
	...
	\loigiai{
		...
		\allowdisplaybreaks
		\begin{eqnarray*}
			y'&=&6x^5+5\left(4+m\right){x^4}+4\left(16-m^2\right){x^3}=x^3\left(6x^2+5\left(4+m\right)x+16-m^2\right).\\
			y'&=&0\Leftrightarrow \heva{&{x^3}=0\\ 
		...
		\begin{itemize}
			\item \textbf{Trường hợp 1.} $16-m^2>0\Leftrightarrow 0<m<4$: $(*)$ có hai nghiệm âm phân biệt $x_1,$ $x_2,$ $\left(x_1<x_2\right)$, ta có bảng xét dấu $y'$ như sau:
			\begin{center}
				\begin{tikzpicture}
					\tkzTabInit[nocadre=false,lgt=1.2,espcl=2.5,deltacl=0.6]
					{$x$/0.6,$y'$/0.6}
			...
			\item \textbf{Trường hợp 2.} $16-m^2<0\Leftrightarrow m>4$: $(*)$ có hai nghiệm trái dấu $x_1,$ $x_2,$ $\left(x_1<0<x_2\right)$, ta có bảng xét dấu $y'$ như sau:\begin{center}
				\begin{tikzpicture}
					\tkzTabInit[nocadre=false,lgt=1.2,espcl=2.5,deltacl=0.6]
					{$x$/0.6,$y'$/0.6}
...
\begin{ex}%[2D1G2-3]%[Mã 102-2018]
	...
	\loigiai{
		...
		\allowdisplaybreaks
		\begin{eqnarray*} 
			y'&=&8x^7+5(m-1){x^4}-4(m^2-1){x^3}+1=x^3\left(8x^4+5\left(m-1\right)x-4\left(m^2-1\right)\right)\\
			y'&=&0\Leftrightarrow\hoac{&x=0\\&8x^4+5\left(m-1\right)x-4\left(m^2-1\right)=0\quad (1).}		\end{eqnarray*}
		\begin{itemize}
			\item Nếu $m=1$ thì $y'=8x^7$, suy ra hàm số đạt cực tiểu tại $x=0$.
			\item Nếu $m=-1$ thì $y'=0\Leftrightarrow\hoac{&x=0\\ 
				&8x^4-10x=0}\Leftrightarrow\hoac{&x=0\\ 
				&x=\sqrt[3]{\dfrac{5}{4}}}$, nhưng $x=0$ là nghiệm bội chẵn nên không phải cực trị.\\
			\item  Nếu $m\ne\pm 1$: khi đó $x=0$ là nghiệm bội lẻ. Xét  $g(x)=8x^4+5\left(m-1\right)x-4\left(m^2-1\right)$. \\Để $x=0$ là điểm cực tiểu thì $\mathop {\lim \limits_{n \to +\infty}}\limits_{x\to {0^-}}g(x)=-4(m^2-1)>0\Leftrightarrow{m^2}-1<0\Leftrightarrow-1<m<1$. \\Vì $m$ nguyên nên chỉ có giá trị $m=0$.
		\end{itemize}
...
\begin{ex}%[2D1B2-4]
	...
	\loigiai{
		...
		$y'=3x^2+6\left(a+b\right)x+3\left(a^2+b^2\right)$.\\
...
\begin{ex}%[2D1B2-4]%[THPT Hai Bà Trưng-Huế-2019]
	...
	\loigiai{
		...
		\begin{itemize}
			\item Nếu $m=0\Rightarrow y'=-2<0,\,(\forall x\in\mathbb{R})$. Nên hàm số không có cực trị.\\
			Do đó $m=0$ (chọn) \hfill$(1)$.
			\item Nếu $m\ne 0$, hàm số không có cực trị $\Leftrightarrow y'$ không đổi dấu $\Leftrightarrow\Delta '\le 0\Leftrightarrow 4m^2-3m(m-2)\le 0\Leftrightarrow{m^2}+6m\le 0\Rightarrow-6\le m<0$ (do $m\ne 0$) \hfill $(2)$.
		\end{itemize}
...
\begin{ex}%[2D1B2-5]%[Chuyên Sơn La-Lần 2-2019]
	...
	{$m\le 3$}
	\loigiai{
		$y'=-4x^3-2\left(m-3\right)x=-2x\left(2x^2+m-3\right)=0\Leftrightarrow\hoac{
			& x=0\\ 
			&{x^2}=\dfrac{3-m}{2}.}$\\
		Vì hàm số đã cho là hàm trùng phương với $a=-1<0$ nên hàm số có điểm cực đại mà không có điểm cực tiểu $\Leftrightarrow y'=0$ có đúng 1 nghiệm bằng $0\Leftrightarrow\dfrac{3-m}{2}\le 0\Leftrightarrow m\ge 3$.
	}
\end{ex}
\begin{ex}%[2D1B2-5]%[Quang Trung-Bình Phước-Lần 5-2019]
	...
	\loigiai{
		Tập xác định $\mathscr{D}=\mathbb{R}$.\\
		$y'=4x^3-4mx=4x\left(x^2-m\right)=0\Leftrightarrow 4x\left(x^2-m\right)=0\Leftrightarrow\hoac{&x=0\\ 
			&{x^2}=m&(*).}$\\
		Hàm số có $3$ cực trị $\Leftrightarrow y'=0$ có $3$ nghiệm phân biệt $\Leftrightarrow$ phương trình $(*)$ có $2$ nghiệm phân biệt $x\ne 0$ $\Leftrightarrow m>0$.}
\end{ex}
...
\begin{ex}%[2D1B2-4]%[THPT Yên Khánh A-Ninh Bình-2019]
	...
	{Vô số}
	\loigiai{
		Ta có $y'=3x^2-6\left(m+1\right)x+3\left(7m-3\right)=0\Leftrightarrow{x^2}-2\left(m+1\right)x+7m-3=0$.\\
...
\begin{ex}%[2D1K2-5]%[HSG-TP Đà Nẵng-2019]
	...
	\loigiai{
		Ta có $y'=4x^3+12m{x^2}+6\left(m+1\right)x$.
		\begin{itemize}
			\item \textbf{Trường hợp 1.} $m=-1$, ta có $y'=4x^3-12x^2=4x^2(x-3)$.\\
			Bảng xét dấu
			\begin{center}
				\begin{tikzpicture}
					\tkzTabInit[nocadre=false,lgt=2,espcl=2,deltacl=0.5]{$x$/1 ,$y'$/1} {$-\infty$ , $0$ , $3$ , $+\infty$}
					\tkzTabLine{, -,0 , - , 0 , +, }
				\end{tikzpicture}
			\end{center}
			Hàm số có $1$ cực tiểu duy nhất.\\
			Ta có $y'=0\Leftrightarrow\hoac{	&x=0\\ 
				&2x^2+6mx+3m+3=0&(*).}$
			\item \textbf{Trường hợp 2.} $m\ne-1$.\\
...
\begin{ex}%[2D1B2-4]%[THPT Ba Đình 2019]
	...
	\loigiai{
		Tập xác định: $\mathscr{D}=\mathbb{R}$.\\
		$y'=3 x^2-6 x+2 m$.
		\\Hàm số có cực đại và cực tiểu $\Leftrightarrow y'=0$ có $2$ nghiệm phân biệt.
...
\begin{ex}%[2D1B2-4]%[Chuyên Bắc Giang 2019]
	...
	{$\left[-1;2\right]$}
	\loigiai{
		Ta có $y'=x^2-2mx+m+2$. \\Để hàm số có hai cực trị thì $y'=0$ có hai nghiệm phân biệt nên $y'>0\Leftrightarrow{\Delta}'>0\Leftrightarrow{m^2}-m-2>0\Leftrightarrow\hoac{&m<-1\\ 
			&m>2.}$
	}
\end{ex}
\begin{ex}%[2D1K2-5]%[THPT Quỳnh Lưu 3 Nghệ An 2019]
	...
	\loigiai{
		...
		\begin{itemize}
			...
			\item \textbf{Trường hợp 2.} $m\ne 0$: Ta có $y'=4m{x^3}-2x$.\\
			$y'=0$ $\Leftrightarrow 4m{x^3}-2x=0$ $\Leftrightarrow 2x\left(2m{x^2}-1\right)=0$ $\Leftrightarrow\hoac{&x=0\\ 
				&2m{x^2}-1=0&(*).}$\\
...
\begin{ex}%[2D1K2-5]%[THPT Yên Định Thanh Hóa 2019]
	...
	{$-\dfrac{1}{2}\le m\le 0$}
	\loigiai{
		Với $m=0$, ta có $y=x^2+1$ $\Rightarrow y'=2x$. Khi đó hàm số có $1$ cực trị và cực trị đó là cực tiểu. Suy ra $m=0$ thỏa mãn yêu cầu bài toán.\hfill $(1)$\\
		Với $m\ne 0$, ta có $y'=4m{x^3}+2(2m+1)x=2x(2m{x^2}+2m+1)$\\
...
\begin{ex}%[2D1B2-5]%[Cụm Liên Trường Hải Phòng 2019]
	...
	{$3$}
	\loigiai{
		Ta có $y'=4x^3+4\left(m^2-m-6\right)x=4x\left[x^2+\left(m^2-m-6\right)\right]$.\\
