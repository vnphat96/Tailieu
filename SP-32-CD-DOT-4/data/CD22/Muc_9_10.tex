...
\begin{ex}%Câu 26 [2H2G1-5]
	...
	{	\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		Gọi $A'$ là hình chiếu của $A$ trên đường tròn tâm $O'$ khi đó ta có\\ $V_{OO'AB}=\dfrac{1}{2}{V_{B.OO'A'A}}=\dfrac{1}{6}\cdot S_{OO'A'A}\cdot \text{d}\left(B,\left(OO'A'A\right)\right)$ với $\text{d}\left(B,\left(OO'A'A\right)\right)=OB\cdot\sin\widehat{BO'A'}$.\\
		Do $S_{OO'A'A}$ là hằng số nên để thể tích khối tứ diện $O{O}'AB$ đạt giá trị lớn nhất thì $\text{d}\left(B,\left(OO'A'A\right)\right)$ là lớn nhất hay $\widehat{BO'A'}=90^0$.\\
...
\begin{ex}%Câu 29 [2H2G1-5]
	...
	{\begin{center}
		...
		Ta có $V_{ABCD}=\dfrac{1}{6}AB.CD.\text{d}\left(AB,CD\right).\sin\alpha=\dfrac{1}{6}.2.2.3.\sin\alpha=2\sin\alpha\le 2$.\\
