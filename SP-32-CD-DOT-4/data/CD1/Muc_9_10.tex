...
\begin{ex}[Đề Tham Khảo 2020 - Lần 1]%[2D1G1-2]
	...
	\loigiai{
		Ta có : $g(x)=f(1-2 x)+x^2-x \Rightarrow g^{\prime}(x)=-2 f^{\prime}(1-2 x)+2 x-1$.\\
		\immini{
			Đặt $t=1-2 x \Rightarrow g^{\prime}(x)=-2 f^{\prime}(t)-t$.\\
			$g'(x)=0 \Rightarrow f^{\prime}(t)=-\dfrac{t}{2}$.\\
			Vẽ đường thẳng $y=-\dfrac{x}{2}$ và đồ thị hàm số $f^{\prime}(x)$ trên cùng một hệ trục
		}	
		{
			\begin{tikzpicture}[scale=0.7,>=stealth, font=\footnotesize, line join=round, line cap=round]
				%\def\a{1} \def\b{-6} \def\c{9} \def\d{1} % Hệ số
...
\begin{ex}[Chuyên Lê Quý Đôn Điện Biên 2019]%[2D1G1-2]
	...
	{$(3 ; 4)$}
	\loigiai{
		$y'=f'(x-1)+3 x^2-12=f^{\prime}(t)+3 t^2+6 t-9=f^{\prime}(t)-\left(-3 t^2-6 t+9\right)$, với $t=x-1$.\\
		\immini{
			Nghiệm của phương trình $y^{\prime}=0$ là hoành độ giao điểm của các đồ thị hàm số $y=f^{\prime}(t)$ và $y=-3 t^2-6 t+9$.\\
			Vẽ đồ thị hàm số $y=f^{\prime}(t)$ và $y=-3 t^2-6 t+9$ trên cùng một hệ trục tọa độ như hình vẽ bên.
		}	
		...
		Dựa vào đồ thị trên, ta có bảng xét dấu của hàm số $y^{\prime}=f^{\prime}(t)-\left(-3 t^2-6 t+9\right)$ như sau $
		\left(t_0<-1\right)$
		\begin{center}
			\begin{tikzpicture}
...
\begin{ex}[Chuyên Phan Bội Châu Nghệ An 2019]%[2D1G1-2]
	...
	{$(-3 ;-2)$}
	\loigiai{
		$y^{\prime}=-2 f^{\prime}(1-x)+\dfrac{x}{\sqrt{x^2+1}}-1$. \\
		...
		\Rightarrow-2 f^{\prime}(1-x)+\dfrac{x}{\sqrt{x^2+1}}-1<0, ~\forall x \in(-2 ; 0)$.
	}
\end{ex}
\begin{ex}[Sở Vĩnh Phúc 2019]%[2D1G1-2]
	...
	\loigiai{
		...
		Có $f^{\prime}(x)=4 a x^3+3 b x^2+2 c x+d$.\\
		...
		Ta có $ y^{\prime}=3\left(f^{\prime}(x)+x^2-4 x+3\right)=3\left(\frac{5}{24}x^3+\frac{15}{8}x^2-\frac{55}{12}x\right)
		$\\
		Ta có $y^{\prime}=0 \Leftrightarrow\hoac{&x=-11 \\&x=0 \\&x=2.}$ \\
...
\begin{ex}[THPT Hoàng Hoa Thám Hưng Yên 2019]%[2D1G1-2]
	\immini{
		Cho hàm số $y=f(x)$ liên tục trên $\mathbb{R}$. Hàm số $y=f^{\prime}(x)$ có đồ thị như hình vẽ. 
	...
	}
	{
		...
		$
		g^{\prime}(x) \geq 0 \Leftrightarrow f^{\prime}(x-1)-1 \geq 0 \Leftrightarrow f^{\prime}(x-1) \geq 1 \Leftrightarrow \hoac{&x - 1 \leq - 1\\
...
\begin{ex}[(Sở Ninh Bình 2019]%[2D1K1-2]
	...
	\loigiai{
		...
		Ta có $g^{\prime}(x)=(-2 f(x)+2019)^{\prime}=-2 f^{\prime}(x), g^{\prime}(x)=0 \Leftrightarrow\hoac{&x=-2 \\&x=-1 \\&x=2 \\&x=4.}$.\\
		Ta có bảng xét dấu của $g^{\prime}(x)$
		\begin{center}
			\begin{tikzpicture}
				\tkzTabInit[nocadre,lgt=1,espcl=2,deltacl=0.6]
				{$x$ /0.6,$f'(x)$ /0.6}
...
\begin{ex}[THPT Lương Thế Vinh Hà Nội 2019]%[2D1G1-2]
	\immini{
		Cho hàm số $y=f(x)$. Biết đồ thị hàm số $y=f^{\prime}(x)$ có đồ thị như hình vẽ bên. 
	...
	\loigiai{
		...
		$
		-2 x \cdot f^{\prime}\left(3-x^2\right)=0 \Leftrightarrow\hoac{&
			x = 0\\
			&3 - x ^{2}= - 6\\
			&3 - x ^{2}= - 1\\
			&3 - x ^{2}= 2}
...
\begin{ex}[Chuyên Biên Hòa - Hà Nam - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số đa thức $f(x)$ có đạo hàm trên $\mathbb{R}$. Biết $f(0)=0$ và đồ thị hàm số $y=f^{\prime}(x)$ như hình sau.
	...
	\loigiai{
		\immini{
			...
			Ta có $h^{\prime}(x)=4 f^{\prime}(x)+2 x$. Do đó $h^{\prime}(x)=0 \Leftrightarrow f^{\prime}(x)=-\dfrac{1}{2}x$.\\
		}
		{
			\begin{tikzpicture}[scale=0.7,>=stealth, font=\footnotesize, line join=round, line cap=round]
				%\def\a{1} \def\b{-6} \def\c{9} \def\d{1} % Hệ số
		...
		Dựa vào sự tương giao của đồ thị hàm số $y=f^{\prime}(x)$ và đường thẳng $y=-\dfrac{1}{2}x$, ta có
		$
		h^{\prime}(x)=0 \Leftrightarrow x \in\{-2 ; 0 ; 4\}.\\
		$
...
\begin{ex}[Chuyên Thái Bình - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số $f(x)$ liên tục trên $\mathbb{R}$ có đồ thị hàm số $y=f^{\prime}(x)$ cho như hình vẽ bên.\\
	...
	\loigiai{
		Ta có đường thẳng $y=x$ cắt đồ thị hàm số $y=f^{\prime}(x)$ tại các điểm $x=-1 ; x=1 ; x=3$ như hình vẽ sau:
		\begin{center}
			\begin{tikzpicture}[scale=0.7,>=stealth, font=\footnotesize, line join=round, line cap=round]
				%\def\a{1} \def\b{-6} \def\c{9} \def\d{1} % Hệ số
				\def\xmin{-4} \def\xmax{5}
...
\begin{ex}[Chuyên Lào Cai - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số $f^{\prime}(x)$ có đồ thị như hình bên.\\
	...
	\loigiai
	{
		\immini{Xét hàm số $g(x)=f(3 x+1)+9 x^3+\dfrac{9}{2}x^2 \\
			\Rightarrow g^{\prime}(x)=3 f^{\prime}(3 x+1)+27 x^2+9 x$.\\
			Hàm số đồng biến  $\Leftrightarrow g^{\prime}(x)>0 \Leftrightarrow 3 f^{\prime}(3 x+1)+27 x^2+9 x>0$
			\\
			$
			\Leftrightarrow f^{\prime}(3 x+1)+3 x(3 x+1)>0 \qquad (*)
			$\\
			Đặt $t=3 x+1$, khi đó  $(*) \Leftrightarrow f^{\prime}(t)+(t-1) t>0$\\ $\Leftrightarrow f^{\prime}(t)>-t^2+t$.\\
			Vẽ parabol $y=-x^2+x$ và đồ thị hàm số $f^{\prime}(x)$ trên cùng một hệ trục
		}
		{
			\begin{tikzpicture}[line join=round, line cap=round,>=stealth,thick,scale=.8]
				\tikzset{label style/.style={font=\footnotesize}}
		...
		$
		f^{\prime}(t)>-t^2+t \Leftrightarrow\hoac{&- 1 < t < 1\\
			&t > 2}\Rightarrow \hoac{&
			- 1 < 3 x + 1 < 1\\
...
\begin{ex}[Sở Phú Thọ-2020]%[2D1G1-2]
	\immini{
		Cho hàm số $y=f(x)$ có đồ thị hàm số $y=f^{\prime}(x)$ như hình vẽ.\\
	...
	\loigiai{
		Dựa vào đồ thị hàm số $y=f^{\prime}(x)$ suy ra $f^{\prime}(x) \leq 0 ~ \forall x<3$ và $f^{\prime}(x)>0 ~ \forall x>3$.
		$
		g^{\prime}(x)=\mathrm{e}^x f^{\prime}\left(\mathrm{e}^x-2\right) .
		$
		Hàm số $g(x)=f\left(\mathrm{e}^x-2\right)-2020$ nghịch biến \\ $ \Leftrightarrow g^{\prime}(x)<0 \Leftrightarrow \mathrm{e}^x f^{\prime}\left(\mathrm{e}^x-2\right)<0$\\
		$
		\Leftrightarrow f^{\prime}\left(\mathrm{e}^x-2\right)<0 \Leftrightarrow \mathrm{e}^x-2<3 \Leftrightarrow \mathrm{e}^x<5 \Leftrightarrow x<\ln 5 .
		$\\
...
\begin{ex}[Lý Nhân Tông - Bắc Ninh - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số $f(x)$ có đồ thị hàm số $f^{\prime}(x)$ như hình vẽ.\\
	...
	\loigiai{
		...
		Ta có $g^{\prime}(x)=-\sin x \cdot f^{\prime}(\cos x)+2 x-1$\\
		Vì $\cos x \in[-1 ; 1]$ nên từ đồ thị $f^{\prime}(x)$ ta suy ra $f^{\prime}(\cos x) \in[-1 ; 1]$.\\
		Do đó $\left|-\sin x \cdot f^{\prime}(\cos x)\right| \leq 1, ~\forall x \in \mathbb{R}$.\\
		Ta suy ra $g^{\prime}(x)=\sin x \cdot f^{\prime}(\cos x)+2 x-1 \geq-1+2 x-1=2 x-2$
		$\Rightarrow g^{\prime}(x)>0, ~\forall x>1$.\\
		Vậy hàm số đồng biến trên $(1 ; 2)$.
	}
\end{ex}
\begin{ex}[THPT Nguyễn Viết Xuân - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số $f(x)$. Hàm số $y=f^{\prime}(x)$ có đồ thị như hình vẽ.\\
	...
	\loigiai
	{
		...
		Ta có $g^{\prime}(x)=6 x f^{\prime}\left(3 x^2-1\right)-18 x^3+6 x=6 x\left[f^{\prime}\left(3 x^2-1\right)-3 x^2+1\right]$.\\
		$
		g^{\prime}(x)=0 \Leftrightarrow\hoac{
			&x = 0\\
			&f ^{\prime}( 3 x ^{2}- 1 ) = 3 x ^{2}- 1}
		\Leftrightarrow \hoac{
			&x = 0\\
...
\begin{ex}[Trần Phú - Quảng Ninh - 2020]%[2D1G1-2]
	...
	\loigiai{
		Ta có $y^{\prime}=2 f^{\prime}(2 x+1)+2 x^2-8$.\\
		Xét $y^{\prime}\leq 0 \Leftrightarrow 2 f^{\prime}(2 x+1)+2 x^2-8 \leq 0 \Leftrightarrow f^{\prime}(2 x+1) \leq 4-x^2$.\\
		Đặt $t=2x+1$, ta có $f^{\prime}(t) \leq \dfrac{-t^2+2 t+15}{4}$.\\
		Vì $\dfrac{-t^2+2 t+15}{4}\geq 0, \forall t \in[-3 ; 5]$.\\
		Mà $f^{\prime}(t) \leq 0, \forall t \in[-3 ; 2]$.\\
		Nên $f^{\prime}(t) \leq \dfrac{-t^2+2 t+15}{4}\Rightarrow t \in[-3 ; 2]$.\\
		Suy ra $-3 \leq 2 x+1 \leq 2 \Leftrightarrow-2 \leq x \leq \dfrac{1}{2}$.}
\end{ex}

\begin{ex}[Chuyên Thái Bình - Lần 3 - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số $y=f(x)$ liên tục trên $\mathbb{R}$ có đồ thị hàm số $y=f^{\prime}(x)$ cho như hình vẽ.\\
		Hàm số $g(x)=2 f(|x-1|)-x^2+2 x+2020$ đồng biến trên khoảng nào?
		\choice
		{\True $(0 ; 1)$}
		{$(-3 ; 1)$}
	...
	\loigiai{
		Với $x>1$, ta có $g(x)=2 f(x-1)-(x-1)^2+2021 \Rightarrow g^{\prime}(x)=2 f^{\prime}(x-1)-2(x-1)$.\\
		Hàm số đồng biến $\Leftrightarrow 2 f^{\prime}(x-1)-2(x-1)>0 \Leftrightarrow f^{\prime}(x-1)>x-1 \quad(*)$.\\
		Đặt $t=x-1$, khi đó $(*) \Leftrightarrow f^{\prime}(t)>t \Leftrightarrow\hoac{&1<t<3 \\ &t<-1}\Rightarrow\hoac{&2<x<4 \\ &x<0 ~(\text{loại}).}$\\
		Với $x<1$, ta có $g(x)=2 f(1-x)-(1-x)^2+2021 \Rightarrow g^{\prime}(x)=-2 f^{\prime}(1-x)+2(1-x)$.\\
		Hàm số đồng biến $\Leftrightarrow-2 f^{\prime}(1-x)+2(1-x)>0 \Leftrightarrow f^{\prime}(1-x)<1-x \quad(* *)$.\\
		Đặt $t=1-x$, khi đó $(* *) \Leftrightarrow f^{\prime}(t)<t \Leftrightarrow\hoac{&-1<t<1 \\ &t>3}\Rightarrow\hoac{&0<x<2 \\ &x<-2}\Rightarrow\hoac{&0<x<1 \\ &x<-2.}$\\
		Vậy hàm số $g(x)$ đồng biến trên các khoảng $(-\infty ;-2),(0 ; 1),(2 ; 4)$.
	}
\end{ex}
\begin{ex}[Sở Phú Thọ - 2020]%[2D1G1-2]
	\immini{
		Cho hàm số $y=f(x)$ có đồ thị hàm số $f^{\prime}(x)$ như hình vẽ.\\
	...
	\loigiai{
		$g^{\prime}(x)=e^x f^{\prime}\left(1+e^x\right)$.\\
		Do $e^x>0, \forall x$ nên $g^{\prime}(x) \leq 0 \Leftrightarrow f^{\prime}\left(1+e^x\right) \leq 0 \Leftrightarrow 1+e^x \leq 3 \Leftrightarrow x \leq \ln 2$, dấu bằng xảy ra tại hữu hạn điểm.\\
...
\begin{ex}%[2D1K1-2]
	...
	{\True $(-1 ; 2)$}
	\loigiai{
		Ta có $y^{\prime}=-2 f^{\prime}(x)$.\\
		$
		y^{\prime}=0 \Leftrightarrow-2 f^{\prime}(x)=0 \Leftrightarrow\hoac{&
			x=-2 \\
			&x=-1 \\
			&x=2 \\
			&x=4.}$\\
		Từ bảng xét dấu của $f^{\prime}(x)$ ta có
		\begin{center}
			\begin{tikzpicture}
				\tkzTabInit[nocadre,lgt=1,espcl=2,deltacl=0.6]
				{$x$ /0.6,$y'$ /0.6}
...
\begin{ex}[THPT Anh Sơn - Nghệ An - 2020]%[2D1G1-2]
	Cho hàm số $f(x)$ xác định và liên tục trên $\mathbb{R}$ và có đạo hàm $f^{\prime}(x)$ thỏa mãn $f^{\prime}(x)=(1-x)(x+2) g(x)+2019$ với $g(x)<0, ~\forall x \in \mathbb{R}$ . Hàm số $y=f(1-x)+2019 x+2020$ nghịch biến trên khoảng nào?
	\choice
	{$(1 ;+\infty)$}
	{$(0 ; 3)$}
	{$(-\infty ; 3)$}
	...
	\loigiai{
		...
		Ta có $h^{\prime}(x)=-f^{\prime}(1-x)+2019$.\\
		Do $h^{\prime}(x)=0$ tại hữu hạn điểm nên để tìm khoảng nghịch biến của hàm số $h(x)$, ta tìm các giá trị của $x$ sao cho $h^{\prime}(x)<0 \Leftrightarrow-f^{\prime}(1-x)+2019<0$\\ 
		$\Leftrightarrow f^{\prime}(1-x)-2019>0 \\
...
\begin{ex}%[2D1G1-2]
	...
	\loigiai{
		Ta có $g^{\prime}(x)=-2 f^{\prime}(x) f^{\prime}(3-2 f(x))-3 x^2+6 x$.\\
		Vì $f(x)>2, ~\forall x \in \mathbb{R}$ nên $3-2 f(x)<-1 ~\forall x \in \mathbb{R}$.\\
		Từ bảng xét dấu $f^{\prime}(x)$ suy ra $f^{\prime}(3-2 f(x))<0, ~\forall x \in \mathbb{R}$.\\
...
\begin{ex}%[2D1G1-2]
	...
	{\True $(2 ; 3)$}
	\loigiai{
		Ta có $y^{\prime}=3 \cdot(f(x))^2 \cdot f^{\prime}(x)-6 \cdot f(x) \cdot f^{\prime}(x)=3 f(x) \cdot f^{\prime}(x) \cdot[f(x)-2]. \\
		y^{\prime}=0 \Leftrightarrow \hoac{&f(x)=0 \Leftrightarrow x \in\left\{x_1, 4 \mid x_1<1\right\}\\
			&f(x)=2 \Leftrightarrow x \in\left\{x_2, x_3, 3, x_4 \mid x_1<x_2<1<x_3<2 ; 4<x_4\right\}\\
			&f^{\prime}(x)=0 \Leftrightarrow x \in\{1,2,3,4\}.}$\\
...
\end{ex}
\begin{ex}%[2D1G1-2]
	Cho hàm số $y=f(x)$ có đồ thị nằm trên trục hoành và có đạo hàm trên $\mathbb{R}$, bảng xét dấu của biểu thức $f^{\prime}(x)$ như bảng dưới đây.
	\begin{center}
		\begin{tikzpicture}
			\tkzTabInit[nocadre,lgt=1.2,espcl=2,deltacl=0.6]
			{$x$ /0.6,$f'(x)$ /0.6}
	...
	{$(2 ;+\infty)$}
	\loigiai{
		$ g^{\prime}(x)=\dfrac{\left(x^2-2 x\right)^{\prime}\cdot f^{\prime}\left(x^2-2 x\right)}{\left(f\left(x^2-2 x\right)+1\right)^2}=\dfrac{(2 x-2) \cdot f^{\prime}\left(x^2-2 x\right)}{\left(f\left(x^2-2 x\right)+1\right)^2}. \\
		g^{\prime}(x)=0 \Leftrightarrow\hoac{
			&2 x - 2 = 0\\
			&f ^{\prime}( x ^{2}- 2 x ) = 0}
...
\begin{ex}[Liên trường huyện Quảng Xương - Thanh Hóa - 2021]%[2D1G1-2]
	\immini{
		Cho các hàm số $y=f(x)$; $y=g(x)$ liên tục trên $\mathbb{R}$ và có đồ thị các đạo hàm $f^{\prime}(x) ; g^{\prime}(x)$ (đồ thị hàm số $y=g^{\prime}(x)$ là đường đậm hơn) như hình vẽ.\\
	...
	\loigiai{
		Ta có: $h^{\prime}(x)=f^{\prime}(x-1)-g^{\prime}(x-1)$.\\
		Dựa vào hình vẽ ta có hàm số $h(x)$ nghịch biến\\
		$\Leftrightarrow h^{\prime}(x)<0 \Leftrightarrow f^{\prime}(x-1)<g^{\prime}(x-1)$\\
		$
...
\begin{ex}[THPT Quế Võ 1 - Bắc Ninh - 2021] %[2D1G1-2]
	\immini{
		Cho ba hàm số $y=f(x), y=g(x), y=h(x)$. Đồ thị của ba hàm số $y=f^{\prime}(x), y=g^{\prime}(x), y=h^{\prime}(x)$ được cho như hình vẽ.\\
	...
	\loigiai{
		Ta có $k^{\prime}(x)=f^{\prime}(x+7)+5 g^{\prime}(5 x+1)-4 h^{\prime}\left(4 x+\dfrac{3}{2}\right)$.\\
...
\begin{ex}[THPT Thanh Chương 1 - Nghệ An- 2021] %[2D1G1-2]
	...
	{$\left(2;\dfrac{5}{2}\right)$}
	\loigiai{
		Ta có $y'=6f^{\prime}(2x-1)-12x^2+30x-18=6\left[f'(2x-1)-2x^2+5x-3\right] $.\\
...
\begin{ex}%[2D1G1-3]%Câu 14
	...
	{$2018$}
	\loigiai{
		Ta có $g'(x)=\left(x^2-2x+m\right)^{\prime}{f}'\left(x^2-2x+m\right)=2\left(x-1\right){f}'\left(x^2-2x+m\right)$ .\\
...
\begin{ex}%[2D1G1-2]%Câu 18
	...
	\loigiai{		
		...
		Khi đó $\left(h(x)\right)^{\prime}=\left(2x-2\right){g}'\left(x^2-2x\right)=-2\left(2x-2\right)\left(x^2-2x\right){\left(x^2-2x-1\right)^2}{\left(x^2-2x+4\right)^3}$\\
