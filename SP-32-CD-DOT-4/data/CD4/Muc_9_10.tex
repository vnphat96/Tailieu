...
\begin{ex}%Câu 8% (Chuyên Thoại Ngọc Hầu 2018)%
	...
	\loigiai{
		...
		Mặt khác $\underset{x\to {1^{\pm }}}{\mathop{\lim \limits_{n \to +\infty}}}\dfrac{1}{2f\left(x \right)-5}=0$ nên $x=1$ không phải tiệm cận đứng.\\
...
\begin{ex}%Câu 10% (THPT Bạch Dằng Quảng Ninh 2019)%
	...
	\loigiai{
		...
		\end{aligned} \right.$
		\begin{itemize}
			\item Với $x=0$ ta có $\underset{x\to 0^+}{\mathop{\lim}}\,\dfrac{\left(x+1 \right)\left(x+3\right)\sqrt{x\left(x+1 \right)}}{x.f\left(x\right).\left[f\left(x \right)-2\right]}=\underset{x\to 0^+}{\mathop{\lim \limits_{n \to +\infty}}}\dfrac{\left(x+1\right)\left(x+3\right)\sqrt{x+1}}{\sqrt{x}.f\left(x \right).\left[ f\left(x\right)-2\right]}=+\infty$.\\
...
\begin{ex}%Câu 11% (Lý Nhân Tông - Bắc Ninh 2019)%
	...
	\loigiai{
		\begin{itemize}
			\item \textbf{Nhận xét 1}: Với $x_0^{}\ge 1$ và $\underset{x\to x_0^+}{\mathop{\lim \limits_{n \to +\infty}}}\,g\left(x\right)$ hoặc $\underset{x\to x_0^-}{\mathop{\lim}}\,g\left(x \right)$ có kết quả là $+\infty$ hoặc $-\infty$ thì $x=x_0^{}$ là tiệm cận đứng của của đồ thị hàm số $g\left(x\right)$
