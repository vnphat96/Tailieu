...
\begin{ex}%[2D1B5-3]%Câu 1
		...
		\immini[thm]{	Cho hàm số bậc ba $y=f(x)$ có đồ thị như hình vẽ bên. Số nghiệm của phương trình $3f(x)-4=0$ là
			\choice
			{\True $3$}
			{$1$}
			{$2$}
		...
		\end{tikzpicture}}
			\loigiai{
			\immini[thm]{Ta có $3f(x)-4=0 \Leftrightarrow f(x)=\dfrac{4}{3}$.\\
				...
				Vậy phương trình $3 f(x)-4=0$ có $3$ nghiệm.}{	\begin{tikzpicture}[scale=0.7,>=stealth, font=\footnotesize, line join=round, line cap=round]
					\def\a{1} \def\b{-3} \def\c{0} \def\d{4} % Hệ số
					\def\xmin{-2} \def\xmax{4}
					\def\ymin{-2} \def\ymax{5} 
					%	\draw[color=gray!50,dashed] (\xmin,\ymin) grid (\xmax,\ymax); 
		...
		\loigiai{
		\immini[thm]{Vẽ đường thẳng $y=2$ ta thấy đường thẳng $y=2$ cắt đồ thị hàm số $y=f(x)$ tại hai điểm.\\
		Do đó số nghiệm của phương trình $f(x)=2$ là $2$.}{	\begin{tikzpicture}[scale=0.7,>=stealth, font=\footnotesize, line join=round, line cap=round]
			\def\a{1} \def\b{-3} \def\c{0} \def\d{2} % Hệ số
			\def\xmin{-2} \def\xmax{4}
			\def\ymin{-3} \def\ymax{3} 
			%\draw[color=gray!50,dashed] (\xmin,\ymin) grid (\xmax,\ymax); 
	...
	\end{tikzpicture}}
		...
		\begin{ex}%[2-2021-DTK-05]%[2D1B5-3]
			\immini[thm]{Cho hàm số bậc bốn $y=f(x)$ có đồ thị như hình vẽ bên. Số nghiệm của phương trình $f(x)+1=0$ là
				\choice
				{$2$}
				{$3$}
				{\True $4$}
			...
			\end{tikzpicture}}
			\loigiai{
				\immini[thm]{Ta có $f(x)+1=0\Leftrightarrow f(x)=-1$. \\
