...
\begin{khung}
	...
	\immini[thm]{Muốn xác định góc của đường thẳng $a$ và $(P)$ ta tìm hình chiếu vuông góc $a'$ của $a$ trên $(P)$. Khi đó $\left(\widehat{a,(P)}\right)=\left(\widehat{a,a'}\right)$.}
...
\begin{khung}
	\begin{vd}[Đề minh họa BGD 2022-2023]%[Tô Ngọc Thy, ĐMH-LVĐ]%[2H1K3-2]
		...
		{\(\dfrac{\sqrt{2}}{4}a^3\)}
		\loigiai{
			\immini[thm]{Kẻ $AH\perp A'B$, $H\in A'B$.\\
				Vì $\heva{&BC\perp AB\\&BC\perp AA'}\Rightarrow BC\perp\left( ABB'A'\right)\Rightarrow BC\perp AH$.\\
				Ta có $BC\perp AH$, $AH\perp A'B\Rightarrow AH\perp \left(A'BC\right)$.\\ Do đó $\mathrm{\,d}\left(A,\left(A'BC\right)\right)=AH=\dfrac{a\sqrt{6}}{3}$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	\loigiai{
	\immini[thm]{Ta có $S_{ABC}=\dfrac{1}{2}AB\cdot AC=\dfrac{1}{2}\cdot a\sqrt 2\cdot a\sqrt{2}=a^2$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2] 
	...
	{$\dfrac{2a^3}{3}$}
\loigiai{
	\immini[thm]{Gọi $H$ là trung điểm $SB$, vì $\triangle SAB$ vuông cân tại $A\Rightarrow AH\perp SB$\quad $(1)$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2] 
	...
	{$24a^3\sqrt{3}$}
	\loigiai{
	\immini[thm]{Gọi $M$ là trung điểm của $BC$. Vì mặt bên là tam giác đều nên $BC\perp SM$. Mặt khác $BC\perp SO$ nên $BC\perp (SOM)\Rightarrow (SOM)\perp (SBC)$.\\
		Gọi $H$ là hình chiếu của $O$ lên $SM$ ta có\\ $OH\perp (SBC)$, do đó $\mathrm{\,d} (O,(SBC))=OH$.\\
	...
	Theo giả thiết $\mathrm{\,d}\left(O;\left(SBC\right)\right)=OH=2a$ nên $a=\dfrac{x\sqrt 6}{12}\Rightarrow x=2a\sqrt 6$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$2a^3$}
	\loigiai{
	\immini[thm]{Trong $(ABCD)$, kẻ $AE\perp BD, (E\in BD)$.\\
		Kẻ $AH\perp SE, (H\in SE)$.\\
		Ta có $\heva{& BD\perp SA\\&BD\perp AE}\Rightarrow BD\perp\left(SAE\right)\Rightarrow BD\perp AH$.\\
		$\Rightarrow AH\perp\left(SBD\right)\Rightarrow\mathrm{\,d}\left( A,\left(SBD\right)\right)=AH$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	\loigiai{
		\immini[thm]{Gọi $I$ là hình chiếu vuông góc của $A$  lên $BC$ suy ra $AI\perp BC$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$\dfrac{a^3\sqrt{3}}{3}$}
	\loigiai{
		\immini[thm]{Gọi $O$ là giao điểm của $AC$ và $BD$, gọi $M$ là trung điểm $BC$. Góc giữa mặt bên $(SBC)$ và mặt phẳng $(ABCD)$ là góc $\widehat{SMO}=60^\circ$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$\sqrt{6}a^3$}
	\loigiai{
		\immini[thm]{Gọi $M$ là trung điểm của $BC$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$9\sqrt{3}a^3$}
	\loigiai{
		\immini[thm]{Ta có $\heva{&\left(ABC'\right)\cap\left(ABCD\right)=AB\\&\left(ADD'A'\right)\perp AB\\&\left(BCC'B'\right)\cap\left(ABCD\right)=BC\\&\left(BCC'B'\right)\cap\left(ABC'\right)=BC'}$\\ $\Rightarrow\left(\widehat{\left(ABC'\right),\left(ABCD\right)}\right)=\widehat{C'BC}=60^\circ$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$15\sqrt{3}a^3$}
	\loigiai{
		\immini[thm]{Ta có $\widehat{A'BA}=\widehat{A'CA}=90^\circ$. Điểm $I$ trung điểm $AA'$ là tâm mặt cầu ngoại tiếp hình chóp $A'.ABC$. Gọi $H$ là hình chiếu của $I$ trên mặt phẳng $(ABC)$ suy ra $H$ là tâm đường tròn ngoại tiếp tam giác $ABC$ và $K$ là điểm đối xứng của $A$ qua $H$. Suy ra $AK$ là đường kính đường tròn ngoại tiếp tam giác $ABC$.	}
		{\begin{tikzpicture}[line join=round,line cap=round,line width=.6pt,font=\footnotesize,scale=1]
			\coordinate (A) at (0,0);
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$\dfrac{2a^3\sqrt{3}}{3}$}
	\loigiai{
		\immini[thm]{Kẻ $MN\parallel BC$, $N\in AB$. $HK\perp MN$, $HI\perp A'K$.\\
		$\mathrm{\,d}\left(A'M;BC\right)=\mathrm{\,d}\left(BC;\left(A'MN\right)\right)$\\
		$=\mathrm{\,d}\left(H;\left(A'MN\right)\right)=HI\Rightarrow HI=\dfrac{a}{2}$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[1H3K5-4]
	...
	{$3a$}
	\loigiai{
		\immini[thm]{Gọi $H$ là trọng tâm của $\triangle ABC$, $M$ là trung điểm $BC$.\\
			Kẻ $MI\perp AA'$ tại $I$.\\
			Kẻ $HK\perp AA'$ tại $K$.\\
		...
		$\mathrm{\,d}\left(AA',BC\right)=MI=\dfrac{3}{2}HK=\dfrac{3a}{2}$.
	}
\end{ex}
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[1H3K5-3]
	...
	{$\dfrac{a\sqrt{6}}{2}$}
	\loigiai{
		\immini[thm]{Ta có $S_{ABCD}=2S_{\triangle ABD}=AB\cdot AD\sin A=\dfrac{a^2\sqrt{3}}{2}$.\\
		...
		$\Rightarrow (SAB)\perp (SHK)$, $(SAB)\cap (SHK)=SK$.}
			{\begin{tikzpicture}[line join=round,line cap=round,line width=.6pt,font=\footnotesize,scale=1]
				...
				\foreach \d/\g in {A/0, B/-90, C/-80, S/90,D/180,H/-90,M/0,K/0,N/40}{
					...
					\end{tikzpicture}}
			\noindent
			Vẽ $HN\perp SK$ tại $N\Rightarrow HN\perp (SAB)\Rightarrow \mathrm{\,d}(H,(SAB))=HN$.\\
			$HN=\dfrac{HK\cdot HS}{\sqrt{HK^2}+HS^2}=\dfrac{a\sqrt{6}}{6}$; $\mathrm{\,d}\left(C,\left(SAB\right)\right)=2\mathrm{\,d}\left(H,\left(SAB\right)\right)=2HN=\dfrac{a\sqrt{6}}{3}$.	
	}
\end{ex}
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[1H3K4-3]
	...
	{$30^\circ$}
	\loigiai{
		\immini[thm]{Gọi $M$ là trung điểm của $CD$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[1H3K3-3]
	...
	{$\dfrac{1}{4}$}
	\loigiai{
		\immini[thm]{Gọi $O$ là tâm hình vuông $ABCD$, đặt $SO=x$ và $OC=y$. \\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	\loigiai{
		\immini[thm]{Gọi $M$ là trung điểm của $B'C'\Rightarrow A'M\perp B'C'$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$\dfrac{3a^3\sqrt{2}}{8}$}
	\loigiai{
		\immini[thm]{Diện tích đáy là $B=S_{\Delta ABC}=\dfrac{a^2\sqrt 3}{4}$.\\
			Chiều cao là $h=\mathrm{\,d}\left(\left(ABC\right);\left(A'B'C'\right)\right)=AA'$.\\
			Do tam giác $ABC$ là tam giác đều nên $O$ là trọng tâm của tam giác $ABC$. Gọi $I$ là trung điểm của $BC$, $H$ là hình chiếu vuông góc của $A$ lên $A'I$ ta có\\ $AH\perp\left(A'BC\right)\Rightarrow\mathrm{\,d}\left(A;\left(A'BC\right)\right)=AH$.\\
			$\dfrac{\mathrm{\,d}\left(O;\left(A'BC\right)\right)}{\mathrm{\,d}\left(A;\left(A'BC\right)\right)}=\dfrac{IO}{IA}=\dfrac{1}{3}$.\\
			$\Rightarrow\mathrm{\,d}\left(O;\left(A'BC\right)\right)=\dfrac{\mathrm{\,d}\left(A;\left(A'BC\right)\right)}{3}=\dfrac{AH}{3}=\dfrac{a}{6}\Rightarrow AH=\dfrac{a}{2}$.}
		{\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
		\path
		(0,0)coordinate(A)
		(4,-2)coordinate(B)
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	\loigiai{
		\immini[thm]{Do $(A'B'C')\parallel (ABC)$ nên góc giữa $(A'BC)$ và $(A'B'C')$ là góc giữa $(A'BC)$ và $(ABC)$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	{$a^3\sqrt{3}$}
	\loigiai{
		\immini[thm]{Ta có $\heva{&AB\perp AC\\&AB\perp AA'}\Rightarrow AB\perp\left(AA'C'C\right)$ tại $A$,\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	\loigiai{
		\immini[thm]{Đặt $AA'=x$, $AB=c$, $AC=b$, $BC=a$.\\
...
\begin{ex}%[Tô Ngọc Thy, PTDMH2023]%[2H1K3-2]
	...
	\loigiai{
		\immini[thm]{Từ giả thiết, ta có tứ giác $ABCE$ là hình vuông.\\
			...
			$E$ là trung điểm $AD$ nên\\ $\mathrm{\,d}\left(BE;A'D\right)=\mathrm{\,d}\left(E;(A'CD\right)=\dfrac{1}{2}\mathrm{\,d}\left(A;(A'CD)\right)=\dfrac{1}{2}AH$.\\
			Từ giả thiết ta có	}
		{\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
			\path
			(0,0)coordinate(A)
