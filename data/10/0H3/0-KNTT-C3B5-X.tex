...
\begin{dang}{Tính các giá trị lượng giác}
	\begin{itemize}
		...
		\item Sử dụng các công thức lượng giác (công thức lượng giác cơ bản, công thức cộng, công thức nhân đôi, $\ldots$).
	\end{itemize}
\end{dang}
\begin{vd}%[0H4N1-2]
	Tính các giá trị lượng giác của các góc $0^\circ $, $90^\circ $, $180^\circ $.
\loigiai{
	...
	\begin{tikzpicture}[scale=1.7,line cap=round,line join=round,>=stealth]
		...
		\node[right] at (0,0.7) {$ y_0 $};
		\draw (0,0)--(-0.7,0.7);
		\draw (0.15,0) arc (0:135:0.15);
	\end{tikzpicture}}
	\begin{itemize}
		\item Với $\alpha=180^\circ$. Khi đó, $M$ trùng với $C(-1;0)$. Do đó, $\sin180^\circ=0$, $\cos180^\circ=-1$, $\tan180^\circ=0$, $\cot180^\circ$ không xác định.
	\end{itemize}
	}
\end{vd}
\begin{vd}%[0H4N1-2]
	Tìm các giá trị lượng giác của góc $135^\circ$.
\loigiai{
	\immini{
...
\end{vd}
\begin{vd}%[0H4N1-2]
	\immini{	Trên nửa đường tròn đơn vị, cho góc $\alpha$ như hình vẽ. Hãy chỉ ra các giá trị lượng giác của góc $\alpha$.
	}{
	\begin{tikzpicture}[scale=0.8, font=\footnotesize,line join=round, line cap=round, >=stealth,x=2cm,y=2cm]
		\def\x{.7} %x0
		\def\a{45} % goc alpha
		\pgfmathsetmacro{\y}{\x*tan(\a)}
		\draw[->] (-1.5,0)--(1.6,0) node [below]{$\cos$};
		\draw[->] (0,-0.3)--(0,1.5) node [left]{$\sin$};
		\node at (0,0) [below left]{$O$};
		\draw (1,0) arc (0:180:2cm);
		\fill (1,0) node[below]{$1$} circle(1pt);
		\fill (-1,0) node[below]{$-1$} circle(1pt);
		\fill (0,1) node[above left]{$1$} circle(1pt);
		\fill (\x,0) node[below left]{\tiny$\frac{\sqrt{2}}{2}$} circle(1pt);
		\fill (\x,0) node[below]{$D$};
		\fill (\a:1) node[above right]{$M$} circle(1pt);
		\fill (0,\y) node[below left]{$E$} circle(1pt);
		\fill (0,\y) node[above left]{\tiny$\frac{\sqrt{2}}{2}$};
		\draw[dashed] (\x,0)|-(0,\y);
		\draw (0,0)--(\a:1); %OM
		\draw (0.2,0) arc (0:\a:0.4cm);
		\node at (.45,.1){\tiny $\alpha=45^\circ$};
	\end{tikzpicture}
	}
\loigiai{
	Ta có
	$\sin \alpha=\overline{OE}=\dfrac{\sqrt{2}}{2}$; $\cos \alpha=\overline{OD}=\dfrac{\sqrt{2}}{2}$; $\tan \alpha=\dfrac{\sin \alpha}{\cos \alpha}=\dfrac{\overline{OE}}{\overline{OD}}=1$; $\cot \alpha=\dfrac{\cos \alpha}{\sin \alpha}=\dfrac{\overline{OD}}{\overline{OE}}=1$.
	}
\end{vd}
\begin{vd}%[0H4N1-2]
	\immini{Trên nửa đường tròn đơn vị, cho góc $\alpha$ như hình vẽ. Hãy chỉ ra các giá trị lượng giác của góc $\alpha$.
	...
	\begin{tikzpicture}[scale=0.8, font=\footnotesize,line join=round, line cap=round, >=stealth,x=2cm,y=2cm]
		...
		\fill (-1,0) node[below]{$-1$} circle(1pt);
		\fill (0,1) node[above left]{$1$} circle(1pt);
		\fill (\x,0) node[below left]{\tiny$0,5$} circle(1pt);
		\fill (\x,0) node[below]{$D$};
		\fill (\a:1) node[above right]{$M$} circle(1pt);
		\fill (0,\y) node[below left]{$E$} circle(1pt);
		\fill (0,\y) node[below right]{\tiny$\frac{\sqrt{3}}{2}$};
		\draw[dashed] (\x,0)|-(0,\y);
		\draw (0,0)--(\a:1); %OM
		\draw (0.2,0) arc (0:\a:0.4cm);
		\node[rotate=60] at (.3,.3){\tiny $\alpha=60^\circ$};
	\end{tikzpicture}
	}
\loigiai{
	Ta có
	$$\sin \alpha=\overline{OE}=\dfrac{\sqrt{3}}{2}; \cos \alpha=\overline{OD}=0{,}5; \tan \alpha=\dfrac{\sin \alpha}{\cos \alpha}=\dfrac{\overline{OE}}{\overline{OD}}=\sqrt{3}; \cot \alpha=\dfrac{\cos \alpha}{\sin \alpha}=\dfrac{\overline{OD}}{\overline{OE}}=\dfrac{\sqrt{3}}{3}.$$
	}
\end{vd}
\begin{vd}%[0H4N1-2]
	\immini{Trên nửa đường tròn đơn vị, cho góc $\alpha$ như hình vẽ. Hãy chỉ ra các giá trị lượng giác của góc $\alpha$.
	...
	\begin{tikzpicture}[scale=1, font=\footnotesize,line join=round, line cap=round, >=stealth,x=2cm,y=2cm]
		...
		\fill (-1,0) node[below]{$-1$} circle(1pt);
		\fill (0,1) node[above left]{$1$} circle(1pt);
		\node at (.4,.1){\tiny $\alpha=90^\circ$};
		\draw (.1,0)--(.1,.1)--(0,.1);
		\path (0,1)node[above right]{$C$};
	\end{tikzpicture}
	}
	\choice
	{$\sin \alpha=1$; $\cos \alpha=0$; $\tan\alpha$ không xác định; $\cot \alpha=1$}
	{$\sin \alpha=0$; $\cos \alpha=1$; $\tan \alpha=1$; $\cot \alpha=0$}
	{\True $\sin \alpha=1$; $\cos \alpha=0$; $\tan \alpha$ không xác định; $\cot \alpha=0$}
	{$\sin \alpha=1$; $\cos \alpha=0$; $\tan \alpha=0$; $\cot \alpha$ không xác định}
\loigiai{
	Ta có
	$\sin \alpha=\overline{O C}=1$; $\cos \alpha=0$; $\tan\alpha$ không xác định; $\cot \alpha=\dfrac{\cos \alpha}{\sin \alpha}=\dfrac{0}{1}=0$.
	}
\end{vd}
\begin{vd}%[0H4N1-2]
	Viết giá trị lượng giác của góc $120^\circ$.
\loigiai{
	Ta có
	\begin{listEX}[2]
...
\end{vd}
\begin{vd}%[0H4N1-2]
	Cho $\alpha$, $90^\circ<\alpha<180^\circ$. Hãy tìm $\cos\alpha$, biết $\sin\alpha=-\dfrac{4}{5}$.
\loigiai{
	Do $90^\circ<\alpha<180^\circ$ nên $\cos\alpha<0$.\\
	Suy ra $\cos\alpha=-\sqrt{1-\sin^2\alpha}=-\dfrac{3}{5}$.
	}
\end{vd}
...
\end{vd}
\begin{vd}%[0H4H1-2]
	Cho $\cos x=\dfrac{1}{3}$. Tính giá trị biểu thức $P=4\sin^2 x+\cos^2 x$.
\loigiai{
