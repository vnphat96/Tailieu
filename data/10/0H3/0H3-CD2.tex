...
\begin{vd}%[0H2K3-4]%
	\immini[thm]{Muốn đo chiều cao của tháp chàm Por Klong Garai ở Ninh Thuận người ta lấy hai điểm $A$ và $B$ trên mặt đất có khoảng cách $AB=12$m cùng thẳng hàng với chân $C$ của tháp để đặt hai giác kế có chiều cao $h=1,2$m. Gọi $D$ là đỉnh tháp và hai điểm $A_1,B_1$ cùng thẳng hàng với $C_1$ thuộc chiều cao $CD$ của tháp. Người ta đo được góc $\widehat{DA_1C_1}=49^{\circ}$ và $\widehat{DB_1C_1}=35^{\circ}$. Chiều cao $CD$ của tháp gần với kết quả nào nhất.
