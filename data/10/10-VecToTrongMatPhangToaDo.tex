...
\end{boxdn}
\textbf{Nhận xét:} \fbox{$\vec{u}(x;y)=\vec{v}(x';y')\Leftrightarrow\heva{&x=x'\\&y=y'.}$}
\begin{boxdn}
	Trong mặt phẳng toạ độ $Oxy$ cho hai điểm $A(x_A;y_A)$ và $B(x_B;y_B)$. \begin{center}
		\fbox{$\overrightarrow{AB}=(x_B-x_A;y_B-y_A)$}
	\end{center}
\end{boxdn}
...
\begin{boxdn}
	...
	\begin{listEX}[2]
		\item $\vec{u}\pm \vec{v}=(x\pm x';y\pm y')$
...
\begin{itemize}
	\item \fbox{$\vec{a} \perp \vec{b} \Leftrightarrow a_1b_1 + a_2b_2 = 0$}
	\item $\overrightarrow{a}^2 = a_1^2 + a_2^2$ và $|\vec{a}|^2=\vec{a}^2 \Rightarrow$ \fbox{$|\vec{a}|=\sqrt{a_1^2+a_2^2}$}
	\item \fbox{$AB=\left|\overrightarrow{AB}\right|=\sqrt{(x_B-x_A)^2+(y_B-y_A)^2}$}
	\item Với $\vec{a}, \vec{b} \ne \vec{0}$, \fbox{$\cos \left(\overrightarrow{a},\overrightarrow{b}\right)= \dfrac{\overrightarrow{a}\cdot \overrightarrow{b} }{ |\overrightarrow{a}|\cdot |\overrightarrow{b}| } = \dfrac{a_1b_1 + a_2b_2}{\sqrt{a_1^2+a_2^2}\cdot \sqrt{b_1^2+b_2^2}}$}
\end{itemize}
	...
	\loigiai{
		\begin{enumerate}
			...
			\item Gọi $D(x;y)$, ta có $\overrightarrow{AB}=(1;-7)$, $\overrightarrow{DC}=(4-x;3-y)$.
			      \immini[thm]{$ABCD$ là hình bình hành khi $\overrightarrow{AB}=\overrightarrow{CD}\\\Leftrightarrow\heva{&1=4-x\\&-7=3-y}\Leftrightarrow\heva{&x=3\\&y=10} $. Vậy $D(3;10)$.}{\begin{tikzpicture}[scale=1,font=\footnotesize,line join = round, line cap = round, >= stealth]
					      \coordinate (A) at (0,0);
					      \def\a{3} \def\b{2}
...
\begin{bt}
Cho điểm $A(2;3)$ và $B(5;-3)$.
\begin{enumerate}
	\item Tìm toạ độ điểm $I$ sao cho $\vec{IA}+2\vec{IB}=\vec{0}$.
	\item Tìm toạ độ điểm $M$ sao cho $|2\vec{MA}+3\vec{MB}|$ đạt giá trị nhỏ nhất.
	\item Tìm toạ độ điểm $N$ sao cho $NA^2+2NB^2$ đạt giá trị nhỏ nhất.
\end{enumerate}
\loigiai{
		\begin{enumerate}
			\item Gọi $I(x;y)$.\\
			Ta có $\vec{IA}=(2-x;3-y)$ và $\vec{IB}=(5-x;-3-y)$.\\
			$\vec{IA}+2\vec{IB}=\vec{0}\Leftrightarrow\heva{&2-x+2(5-x)=0\\&3-y+2(-3-y)=0}\Leftrightarrow\heva{&x=4\\&y=-1.}$\\
			Vậy $I(4;-1)$.
			\item Gọi $M(a;b)$.\\
			Ta có $\vec{MA}=(2-a;3-b)$ và $\vec{MB}=(5-a;-3-b)$.\\
			$|2\vec{MA}+3\vec{MB}|=\sqrt{(4-5a)^2+(6-9b)^2}$.\\
			Để $|2\vec{MA}+3\vec{MB}|$ đạt giá trị nhỏ nhất thì $ (4-5a)^2+(6-9b)^2 $ đạt giá trị nhỏ nhất.\\
			Ta có $ (4-5a)^2+(6-9b)^2\ge 0 $.\\
			Dấu "$=$" xảy ra khi $4-5a=0$ và $6-9b=0\Leftrightarrow\heva{&a=\dfrac{4}{5}\\&b=\dfrac{2}{3}.}$\\
			Vậy $M\left(\dfrac{4}{5};\dfrac{2}{3}\right)$.
			\item Gọi $N(p;q)$.\\
			Ta có $NA^2=(2-p)^2+(3-q)^2$ và $NB^2=(5-p)^2+(-3-q)^2$.\\
			$NA^2+2NB^2=(2-p)^2+(3-q)^2+2[(5-p)^2+(-3-q)^2]=3p^2+3q^2-24p+6q+81=3\left(p-4\right)^2+3\left(q+1\right)^2+30$.\\
			Để $NA^2+2NB^2$ đạt giá trị nhỏ nhất thì $3\left(p-4\right)^2+3\left(q+1\right)^2$ đạt giá trị nhỏ nhất.\\
			Khi đó $3\left(p-4\right)^2=0$ và $3\left(q+1\right)^2=0\Leftrightarrow\heva{&p=4\\&q=-1.}$\\
			Vậy $N(4;-1)$.
		\end{enumerate}
	}
\end{bt}
