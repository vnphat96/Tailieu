...
\subsubsection{Tọa độ của véc-tơ}
	\begin{dn}{}
		\begin{itemize}
		\item  \emph{Trục} tọa độ (còn gọi là \emph{trục}, hay \emph{trục số}) là một đường thẳng mà trên đó đã xác định một điểm $O$ và một véc-tơ $\vec{i}$ có độ dài bằng $1$. Điểm $O$ gọi là \emph{gốc tọa độ}, véc-tơ $\vec{i}$ gọi là \emph{véc-tơ  đơn vị} của trục. Điểm $M$ trên trục biểu diễn số $x_0$ nếu $\overrightarrow{OM}=x_0\vec{i}$.
		\begin{center}
		\begin{tikzpicture}[scale=1,font=\footnotesize,line join = round, line cap = round, >= stealth]
		\def\a{-1} \def\b{5} \def\c{-1} \def\d{3}
		\coordinate (O) at (0,0);
		\coordinate (M) at (3.5,0);
		\draw[->] (\a,0)--(\b,0)node[below]{$x$};
		\draw[->,blue,thick](0,0)--(1,0); \draw (0,0)node[below]{$0$} (1,0) node[below]{$1$};
		\draw(0.5,0.3) node[]{$\vec{i}$}
		(M)node[below]{$x_0$}
		;
		\foreach \p/\g in {O/90,M/90} \draw[fill] (\p) circle(1pt)node [shift={(\g:.3)}] {$\p$};
		\end{tikzpicture}
		\end{center}
		\item  Trên mặt phẳng, xét hai trục $Ox$, $Oy$ có chung gốc $O$ và vuông góc với nhau. Véc-tơ đơn vị trên trục $Ox$ là $\vec{i}$, véc-tơ đơn vị của trục $Oy$ là $\vec{j}$. Hệ gồm hai trục $Ox$, $Oy$ như vậy được gọi là \emph{hệ trục tọa độ $Oxy$}. Điểm $O$ gọi là \emph{gốc tọa độ}, trục $Ox$ gọi là \emph{trục hoành}, trục $Oy$ gọi là \emph{trục tung}. Mặt phẳng chứa hệ trục tọa độ $Oxy$ gọi là {\color{red} mặt phẳng tọa độ} $Oxy$ hay mặt phẳng $Oxy$.
		\begin{center}
		\begin{tikzpicture}[scale=1,font=\footnotesize,line join = round, line cap = round, >= stealth]
		...
		\draw (2,0)node[below]{$x_0$} (0,3)node[left]{$y_0$}
		;
		...
		\end{tikzpicture}
		\end{center}
		\item Với mỗi véc-tơ $\vec{u}$ trên mặt phẳng $Oxy$, có duy nhất cặp số $(x_0;y_0)$ sao cho $\vec{u}=x_0\vec{i}+y_0\vec{j}$. Ta nói véc-tơ $\vec{u}$ có tọa độ $(x_0;y_0)$ và viết $\vec{u}=(x_0;y_0)$ hay $\vec{u}(x_0;y_0)$. Các số $x_0$, $y_0$ tương ứng được gọi là {\color{red}hoành độ, tung độ} của $\vec{u}$.
		\end{itemize}
	\end{dn}
	\textbf{Nhận xét}. Hai véc-tơ bằng nhau khi và chỉ khi chúng có cùng tọa độ.
...
\begin{ex}%[Bài giảng Toán 10 2022]%[Phạm Tiến Long]%[0H1Y4-3]
	...
	\loigiai{
		Ta có véc-tơ đơn vị $\overrightarrow{i}=(1;0)$.}
\end{ex}
\begin{ex}%[Bài giảng Toán 10 2022]%[Phạm Tiến Long]%[0H1Y4-3]
...
\begin{ex}%[Lê Quốc Hiệp, dự án BG10-2022]%[0H1Y4-3]
	...
	{$\left(4;-3\right)$}
	{\True $\left(2;5\right)$}
	\loigiai{Tọa độ của véc-tơ $\overrightarrow{AB} = \left(3-1;7-2\right) = \left(2;5\right)$.
	}
\end{ex}
...
\begin{ex}%[Lê Quốc Hiệp, dự án BG10-2022]%[0H1B4-2]
	Trong mặt phẳng tọa độ $Oxy$, cho các véc-tơ $\overrightarrow{a}=(-2;1)$, $\overrightarrow{b}=(1;-3)$ và $\overrightarrow{c}=(0;2).$ Tính tọa độ của véc-tơ $\overrightarrow{u}=\overrightarrow{a}+\overrightarrow{b}+\overrightarrow{c}.$
	\choice
...
\end{ex}
\begin{ex}%[Lê Quốc Hiệp, dự án BG10-2022]%[0H1B4-3]
	Trong mặt phẳng tọa độ $Oxy$, cho $\overrightarrow{a}=(m;3)$ và $\overrightarrow{b}=(2;-1)$. Tìm các giá trị của $m$ để hai véc-tơ $\overrightarrow{a}$ và $\overrightarrow{b}$ cùng phương.
	\choice
	{\True $m=-6$}
	{$m=12$}
	{$m=\dfrac{3}{4}$}
