...
\begin{tcolorbox}
	\immini[thm]{Tập hợp gồm các phần tử thuộc cả hai tập hợp $S$ và $T$ gọi là giao của hai tập hợp $S$ và $T$, kí hiệu là $S \cap T$.\\
...
\begin{vd}%[BG10-2022]%[Đỗ Văn Dự]%[0D1Y4-1]
	...
	\loigiai{
		\immini[thm]{$C=[2;7]$}{\begin{tikzpicture}[scale=1,>=stealth, font=\footnotesize, line join=round, line cap=round]
		...
		\end{tikzpicture}}
		\immini[thm]{$C=(-\infty;2)$}{\begin{tikzpicture}[scale=1,>=stealth, font=\footnotesize, line join=round, line cap=round]
				\draw [-stealth] (0,0)--(9,0);
				\path (2,0) node{$)$} (2,-0.1)node[below]{$2$};
