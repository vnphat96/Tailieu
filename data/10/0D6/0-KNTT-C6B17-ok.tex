...
\textbf{\iconMT~ Ta đã biết hàm số $y=ax^2+bx+c$ với $a \ne 0$ có đồ thị là một parabol.}
\begin{itemize}
	\item Nếu $a>0$, ta có các trường hợp sau:\\
	\begin{tcolorbox}[colframe=orange,colback=orange!4,boxrule=0.2mm]
		...
		\begin{minipage}{5cm}
			...
			\end{tikzpicture}\\
			Đồ thị nằm trên $Ox$ khi $x<x_1$ hoặc $x>x_2$; nằm dưới $Ox$ khi $x_1<x<x_2$.
		\end{minipage}
	\end{tcolorbox}
	\item Nếu $a<0$, ta có các trường hợp sau:\\
	\begin{tcolorbox}[colframe=orange,colback=orange!4,boxrule=0.2mm]
		...
		\begin{minipage}{5cm}
			...
			\end{tikzpicture}\\
			Đồ thị nằm dưới $Ox$ khi $x<x_1$ hoặc $x>x_2$; nằm trên $Ox$ khi $x_1<x<x_2$.
		\end{minipage}
	\end{tcolorbox}
\end{itemize}
...
\begin{ex}%[0D4Y5-1]
	...
	{\begin{tikzpicture}[font=\footnotesize]
			...
			\tkzTabLine{,+,$0$,-,}
	\end{tikzpicture}}
	{\True \begin{tikzpicture}[font=\footnotesize]
			\tkzTabInit[nocadre=false,lgt=1.3,espcl=1.8,deltacl=0.6]
			{$x$ /0.6,$f(x)$ /0.6}
			{$-\infty$,$7$,$+\infty$}
			\tkzTabLine{,-,$0$,-,}
