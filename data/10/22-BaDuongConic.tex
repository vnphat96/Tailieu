...
\subsection{Elip}
\begin{khung4}{Khái niệm đường elip}
Cho hai điểm cố định $ F_1 $, $ F_2 $ và một độ dài không đổi $ 2a $ lớn hơn $ F_1F_2 $. Elip $ (E) $ là tập hợp các điểm $ M $ trong mặt phẳng sao cho \boxmini{$F_1M+F_2M=2a.$}
...
\begin{center}
	\begin{tblr}{Q[c,m]|Q[c,m]}
		...
		\path[color=blue,-stealth](-3.5,0)--(4,0);
		\path[color=blue,-stealth](0,-2.5)--(0,2.5);
...
\end{center}
\begin{khung4}{Phương trình chính tắc của elip}
Chọn hệ trục tọa độ Oxy sao cho $ F_1(-c; 0) $ và $ F_2(c; 0) $.
	\boxmini{$M(x;y) \in (E) \Leftrightarrow \dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}=1\;(1)$} trong đó $b=\sqrt{a^2-c^2}$.\\
...
\begin{ex}%[Tex hoa TL Word 10 new_LVH]%[Quan Văn Ón,Tex hoa TL Word 10 new_LVH]%[0H3B3-3]
	...
	\loigiai
	{
		...
		Vậy $\overrightarrow{n} = (4;9)$ là một vectơ pháp tuyến của $M_1M_2$.\\
		Phương trình đường thẳng $M_1M_2\colon$ đi qua M(1;1) có vectơ pháp tuyến $\overrightarrow{n} = (4;9)$ là
...
\subsection{Hypebol}
\begin{khung4}{Định nghĩa đường hypebol}
	Cho hai điểm cố định $F_1$, $F_2$ và một độ dài không đổi $2a$ nhỏ hơn $F_1 F_2$. Hypebol $(H)$ là tập hợp các điểm $M$ trong mặt phẳng sao cho \boxmini{$\left|F_1 M - F_2 M\right|= 2a$.}
...
\begin{center}
	\begin{tblr}{cc}
	\begin{tikzpicture}[>=stealth,scale=.7, line join=round, line cap=round,font=\small,declare function={a=sqrt(3);b=sqrt(2);c=sqrt(5);} ]
		\path[->] (-4,0)--(0.25,0)--(4,0);
		\path[->] (0,-3)--(0,3);
		%Hypebol
		\clip (-4,-3) rectangle (4,3);
		\draw[orange,thick,smooth,samples=100,domain=a+0.00002:4] plot(\x,{sqrt(\x*\x/3-1)});
		\draw[orange,thick,smooth,samples=100,domain=-a-0.00002:-4] plot(\x,{sqrt(\x*\x/3-1)});
		\draw[orange,thick,smooth,samples=100,domain=a+0.00002:4] plot(\x,{-sqrt(\x*\x/3-1)});
		\draw[orange,thick,smooth,samples=100,domain=-a-0.00002:-4] plot(\x,{-sqrt(\x*\x/3-1)});
		%Điểm M
		\draw (-c,0)--(3,2)node[above left]{$ M(x; y) $}--(c,0);
		%Đỉnh
		\draw [fill=black]  (-c,0)node[below]{$ F_1 $}circle(1.5pt) (c,0)node[below]{$ F_2 $}circle(1.5pt) ;
	\end{tikzpicture}&
	\begin{tikzpicture}[>=stealth,scale=.7, line join=round, line cap=round,font=\small,declare function={a=sqrt(3);b=sqrt(2);c=sqrt(5);} ]
		...
		\draw[dashed,smooth,blue,samples=100,domain=-4:4] plot(\x,{-(b/a)*\x});
		\draw[dashed,smooth,blue,samples=100,domain=-4:4] plot(\x,{(b/a)*\x});
		%Đỉnh
		\draw [fill=black] (-a,0)node[above right]{$ A_1 $}circle(1.5pt) (a,0)node[above left]{$ A_2 $}circle(1.5pt) (-c,0)node[above]{$ F_1 $}circle(1.5pt) (c,0)node[above]{$ F_2 $}circle(1.5pt) (0,b)node[above left]{$ B_2 $}circle(1.5pt) (0,-b)node[below left]{$ B_1 $}circle(1.5pt) (-a,0)node[below right]{$ -a $} (a,0)node[below left]{$ a$} (-c,0)node[below]{$ -c $} (c,0)node[below]{$ c $} (0,0)circle(1.5pt);
	\end{tikzpicture}
\end{tblr}
\end{center}
\begin{khung4}{Phương trình chính tắc hypebol}
Chọn hệ trục tọa độ $Oxy$ sao cho $F_1(-c; 0)$ và $F_2(c; 0)$.\\
\boxmini{$M(x;y) \in (H) \Leftrightarrow \dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}=1, \quad (2)$} 
...
\begin{dang}{Hypebol}
	Dựa vào các yếu tố đề cho để tìm $a$, $b$, $c$. Từ đó suy ra các kết quả cần tìm.
\end{dang}
...
\begin{ex}%Câu 5
	...
	{\True $2$}
	\loigiai{
		\immini[thm]{Ta có $y^2=4x\Rightarrow p=2 \Rightarrow \Delta\colon x + 1 = 0$ là phương trình đường chuẩn của $(P)$.\\
...
\begin{ex}%Câu 6
	...
	{\True đáp số khác}
	\loigiai{
		\immini[thm]{Ta có  $M, N \in (P); MN \perp Ox$ và $Ox$ là trục đối xứng của $(P)$ nên $x_M=x_N$ và $y_M=-y_N$.\\
...
\begin{bt}%[BG2-2022-Huỳnh Xuân Tín]
	...
	\loigiai{
\immini[thm]{Gọi phương trình quỹ đạo elip có dạng $\dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}=1$.\\
...
\begin{bt}%[BG2-2022-Huỳnh Xuân Tín]
\immini[thm]{Một sao chổi đi qua hệ Mặt Trời theo quỹ đạo là một nhánh hypebol nhận tâm Mặt Trời là một tiêu điểm, khoảng cách gần nhất từ sao chổi này đến tâm Mặt Trời là $3\cdot 10^8$ km và tâm sai của quỹ đạo hypebol là $3{,}6$ (hình bên). Hãy lập phương trình chính tắc của hypebol chứa quỹ đạo, với một đơn vị đo trên mặt phẳng tọa độ tương ứng với $10^8$ km trên thực tế.}{\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round, >=stealth,
...
\end{tikzpicture}}		
	\loigiai{
\immini[thm]{Chọn hệ trục tọa độ như hình vẽ (với một đơn vị đo trên mặt phẳng tọa độ tương ứng với $10^8$ km trên thực tế). Khi đó quỹ đạo có phương trình dạng $\dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}=1$.\\
...
% \begin{bt}%[BG2-2022-Huỳnh Xuân Tín]
% \immini[thm]{Xét đèn có bát đáy parabol với kích thước được thể hiện như hình bên. Dây tóc bóng đèn được đặt ở vị trí tiêu điểm. Tính khoảng cách từ dây tóc tới đỉnh bát đáy.}{{\includegraphics[scale=0.8]{"Pic/22-01"}}}
% 	\loigiai{
% \immini[thm]{Chọn hệ trục như hình vẽ, parabol có dạng $y^2=2px$.\\
...
% \begin{bt}%[BG2-2022-Huỳnh Xuân Tín]
% \immini[thm]{Ăng-ten vệ tinh parabol ở hình bên có đầu thu đặt tại tiêu điểm, đường kính ăng-ten là $240$ cm, khoảng cách từ vị trí đặt đầu thu đến miệng ăng-ten là $130$ cm. Tính khoảng cách từ vị trí đặt đàu thu tới đỉnh ăng-ten.}{{\includegraphics[scale=0.8]{"Pic/22-02"}}}
