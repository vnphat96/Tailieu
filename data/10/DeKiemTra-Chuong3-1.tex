\section*{BT ÔN TẬP HỆ THỨC LƯỢNG SỐ 1}
% \subsection*{Đề số 1}
...
\begin{ex} 
	...
	{$\cot 160^\circ$}
	\loigiai{
		Ta có $90^\circ<120^\circ<180^\circ$ nên $\sin 190>0$.\\
...
\begin{ex}
	...
	\loigiai{
		\begin{center}
			\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round, >=stealth]
			\path (0,0)  coordinate(D) (2,0)  coordinate(A) (5,0)  coordinate(C) (30:4) coordinate(B) ($(D)!0.5!(B)$) coordinate (I);
			\draw (D)--(B)--(C)--cycle (B)--(A)--(I);
			\foreach \x/\g in {A/-90,B/90,C/-90,D/-90,I/90}\draw[fill=black] (\x) circle (.03) + (\g:.3) node{$\x$};
			\end{tikzpicture}
		\end{center}
		Trên tia đối của tia $AC$ lấy $D$ thỏa $AD=AB=c$ suy ra tam giác $BDA$ cân tại $A$ và $\widehat{BDA}=\dfrac{1}{2} \widehat{A}$.
			Áp dụng định lý hàm số cô-sin cho $\triangle ABD$ ta có
			\allowdisplaybreaks
			\begin{eqnarray*}
				BD^{2}&=&AB^{2}+AD^{2}-2AB \cdot AD\cdot\cos\widehat{BDA} \\
				&=&2c^{2}-2c^{2}\cdot\cos\left(180^{\circ}-A\right)\\
				&=&2c^{2}(1+\cos A)=2c^{2}\left(1+\dfrac{b^{2}+c^{2}-a^{2}}{2bc}\right) \\
				&=&\dfrac{c}{b}(a+b+c)(b+c-a)=\dfrac{4 c}{b}p(p-a) \\
				\text {suy ra}\,\,  BD&=&2\sqrt{\dfrac{cp(p-a)}{b}}.
			\end{eqnarray*}
...
\begin{ex}
	...
	\begin{enumerate}
		\item Tính diện tích và bán kính đường tròn nội tiếp tam giác $ABC$.
	...
	\loigiai
	{
		\begin{enumerate}
			...
			Ta có 
			$$S=pr\Rightarrow r=\dfrac{S}{p}=\dfrac{90\sqrt{2}}{30}=3\sqrt{2}.$$
			\item %Câu b
