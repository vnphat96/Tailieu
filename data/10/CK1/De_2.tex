...
\begin{ex}%[0D8H3-3]%[tex hóa đề ck2 - form 2025 - đợt 2 - Nguyễn Hữu Thiện]
	...
	\loigiai{Ta có
		\begin{eqnarray*}
			(2a-b)^5 \xrightarrow{C_5^4(2a)^{5+4}b^4}&& C_5^0 \cdot(2a)^5+C_5^1(2a)^4(-b)+C_5^2(2a)^3(-b)^2+\ldots\\
...
\begin{ex}%[0H9H1-3]%[KNTT - Lớp 10 - Ôn tập cuối học kì 1 - Đề 5]%[Phạm Hải Dương]
	...
	\loigiai{
		...
		Để $A$, $B$, $M$ thẳng hàng thì $\overrightarrow{AM}$ và $\overrightarrow{AB}$ cùng phương hay \[\overrightarrow{AM} = k\overrightarrow{AB} \Leftrightarrow \heva{
				&x-2 = k \\
				&3 = 7k} \Leftrightarrow x = \dfrac{17}{7}.\]
...
\begin{ex}
	...
	\loigiai{.\\
		...
		Khi đó $\vec{AE} \cdot \vec{AB}=\left(\vec{AD}+\vec{DE}\right) \cdot \vec{AB}=\underbrace{\vec{AD} \cdot \vec{AB}}_0+\vec{DE} \cdot \vec{AB}$\\
		$=DE \cdot AB \cdot \cos \left(\vec{DE},\vec{AB}\right)=DE \cdot AB \cdot \cos 0^\circ=2a^2$
	}
\end{ex}
\begin{ex}%[0H9V1-4]%[De-chuan-hoa-so-13]%[Huỳnh Quy]
	...
	{$(1;1)$}
	\loigiai{
		Gọi $ H(x;y) $. Khi đó $ \overrightarrow{AH}=(x-5;y-3) $, $ \overrightarrow{BH}=(x+1;y-5) $, $ \overrightarrow{BC}=(3;-6) $.\\
		Ta có $ \heva{& \overrightarrow{AH}\cdot \overrightarrow{BC}=0\\ & \overrightarrow{BH}\text{ cùng phương } \overrightarrow{BC}} \Rightarrow \heva{& 3(x-5)-6(y-3)=0\\& \dfrac{x+1}{3}=\dfrac{y-5}{-6}}\Rightarrow \heva{& x-2y=-1\\ & 2x+y=3}\Rightarrow \heva{& x=1\\ & y=1.}$\\
		Vậy $ H(1;1) $.
	}
\end{ex}
