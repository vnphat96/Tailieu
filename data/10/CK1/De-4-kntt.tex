...
\end{ex}
\begin{ex}%[0D3N1-2]
Tập xác định của hàm số $y=\sqrt{x-2024}$ là
	\choice
	{$(2024 ;+\infty)$}
	{$(-\infty ; 2024]$}
	{\True $[2024 ;+\infty)$}
	...
	\loigiai
	{
	Hàm số $y=\sqrt{x-2024}$ xác định $\Leftrightarrow x-2024 \geq 0 \Leftrightarrow x \geq 2024$.\\
	Vậy hàm số có tập xác định là $[2024 ;+\infty)$.
	}
\end{ex}
\begin{ex}%[0D3H2-1]
Biết rằng đồ thị hàm số $y=ax^2+bx+2$ là một parabol có đỉnh $I(2 ;-2)$. Tính giá trị của biểu thức $S=a+b$.
	\choice
	{$S=1$}
	{\True $S=-3$}
	{$S=-1$}
	...
	\loigiai
	{
	Vì đồ thị hàm số $y=a x^{2}+b x+2$ là một parabol có đỉnh $I(2 ;-2)$ nên ta có
	$$\heva{&a \neq 0\\ &-\dfrac{b}{2a}=2\\ &a \cdot 2^2 + b \cdot 2 + 2 =-2} \Leftrightarrow \heva{&a \neq 0\\ &b = -4a\\ &4a+2b=-4} \Leftrightarrow \heva{&a=1\\ &b=-4.}$$
	Vậy $S=a+b=1-4=-3$.
	}
\end{ex}
\begin{ex}%[0H4N1-1]
...
{$\dfrac{\sin B}{b}=2R$}
{$\dfrac{a}{\sin A}=R$}
{$\cos B=\dfrac{b^2+c^2}{2bc}$}
{\True $\cos C=\dfrac{b^2+a^2-c^2}{2ab}$}
	\loigiai
	{
	Theo công thức hệ quả của định lí Cosin trong tam giác thì $\cos C=\dfrac{b^2+a^2-c^2}{2ab}$.
	}
\end{ex}
\begin{ex}%[0H4N3-1]
...
\end{ex}
\begin{ex}%[0H5N2-1]
Cho $3$ điểm $A$, $B$, $C$ phân biệt. Khi đó $\overrightarrow{AC}+\overrightarrow{CB}$ bằng
	\choice
	...
	\loigiai
	{
	Theo quy tắc ba điểm ta có $\overrightarrow{AC}+\overrightarrow{CB}=\overrightarrow{AB}$.
	}
\end{ex}
\begin{ex}%[0H5H2-4]
...
\begin{ex} %[0D3V2-2]
	...
	\loigiai{
		\begin{itemchoice} 
			\itemch \textbf{Đúng}. \\
			Thay tọa độ điểm $A(-1;8)$ vào hàm số, ta được $a-b+c=8$.  
			\itemch \textbf{Sai}. \\
			Ta có $\dfrac{-b}{2a}=2 \Leftrightarrow b=-4a$,
			và điểm $I(2;-1)$ thuộc đồ thị hàm số nên \break $4a+2b+c=-1$.
			\itemch \textbf{Đúng}. \\
		...
		$	\heva{
			...
			\itemch  \textbf{Sai}.  \\
			Ta có BBT của hàm số $y=x^2-4x+3$ \\ 
			\begin{center}
				\begin{tikzpicture}[>=stealth]
					\tkzTabInit[nocadre=true,lgt=1,espcl=2,deltacl=0.5]{$x$/.7 ,$y$/2}
					{$-\infty$ ,$-3$,$0$,$2$ , $+\infty$}
					\path
					(N11)node[below](1){$+\infty$}
					($(N21)!0.3!(N22)$)node[below right](2){$24$}
					($(N31)!0.6!(N32)$)node[below right](3){$3$}
					(N42)node[below right](4){$-1$}
					(N51)node[below](5){$+\infty$}
					;
					\foreach \x/\y in {1/2,2/3,3/4,4/5}\draw[-stealth](\x)--(\y);
				\end{tikzpicture}
			\end{center}
			Từ đó suy ra giá trị nhỏ nhất của hàm số trên $[-3;0]$ bằng $3$.
		\end{itemchoice}  
	}
\end{ex}
...
\choiceTF
{Tọa độ vectơ $\overrightarrow{u}=2\overrightarrow{AB}+\overrightarrow{AC}$ là $(1;14)$}
{\True Ba điểm $A$, $B$, $C$ tạo thành một tam giác}
{\True Tích vô hướng của hai vectơ $\overrightarrow{AB}$ và $\overrightarrow{AC}$ bằng $-6$}
...
\immini[thm]{Phần nửa mặt phẳng không bị gạch (không kể đường thẳng $d$) ở hình vẽ sau là miền nghiệm của bất phương trình $x+m y>n$. Giá tri của biểu thức $S=5 m+n$ bằng bao nhiêu?}{
...
\begin{ex}%[0D3V2-6]
	...
	\loigiai{
		\begin{center}
			\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round, >=stealth,yscale=.7]
				\draw[->] (-1,0)--(3,0)
				node[shift={(-100:7pt)},font=\normalsize]{$t$};
				\draw[->] (0,-1)--(0,10)
				node[shift={(180:7pt)},font=\normalsize]{$h$};
				\draw (5pt,0) |- (0,5pt) (0,0) node[shift={(225:9pt)},font=\normalsize]{$O$};
				\tikzset{declare function={f(\x)=-4.9*(\x)^2+12.2*(\x)+1.2;}}
				\draw[dashed] (2,0)--(2,6)--(0,6) (1,0)--(1,8.5)--(0,8.5);
				\fill (1,0) node[below]{$1$} (1,8.5)circle(1pt) (0,8.5) node[left]{$8.5$}
				(2,0) node[below]{$2$} (2,6)circle(1pt) (0,6) node[left]{$6$}
				(0,1.2) node[left]{$1.2$}
				;
				\begin{scope}
					\clip (-5,-5) rectangle (5,10);
					\draw[samples=100] plot[domain=0:2.585] (\x, {f(\x)});
				\end{scope}
			\end{tikzpicture}
		\end{center}
		Do bóng được đá từ độ cao $1{,}2$ m nên trong hệ trục tọa độ $Oth$ ta có Parabol cắt trục $Oh$ tại điểm có tung độ $h_0=1{,}2$ m.\\ Khi đó phương trình Parabol có dạng $h(t)=at^2+bt+1{,}2$ $(t\geq 0)$.\\
		Theo giả thiết ta có hệ phương trình
		\[\heva{&h(1)=a+b+1{,}2=8{,}5 \\
			&h(2)=4a+2b+1{,}2=6}
		\Leftrightarrow \heva{&a+b=7{,}3 \\
			&2a+b=2{,}4}
		\Leftrightarrow \heva{&a=-4{,}9 \\
			&b=12{,}2.}\]
		Do đó khi quả bóng chạm đất thì độ cao của quả bóng so với mặt đất bằng $0$. Suy ra
		\[-4{,}9t^2+12{,}2t+1{,}2=0\Leftrightarrow t\approx 2{,}58.\]
	}
\end{ex}
\Closesolutionfile{ans}
=======
