...
\begin{ex}%[VN-MT-9]%[0-TK-HK1-KN-8-2425, Nguyễn Quang Hiệp]%[0H5H3-5]
	...
	\begin{ex}
		...
		{$\vec{AB} \cdot \vec{BD}=-64$}
		\loigiai{.\\
		Giả thiết không cho góc, ta phân tích các vectơ $\vec{AB},\text{ }\vec{BD}$ theo các vectơ có giá vuông góc với nhau.\\
...
\TL
			...
			\begin{ex}
			Khai triển nhị thức Newton $(3x-\dfrac12y)^5$.
			\end{ex}
			...
			\begin{ex}%[0D8V2-6]%[Dự án đề kiểm tra Toán 10 GHKI NH23-24- Nguyễn Hữu Duy]%[SGD Bắc Ninh]
				...
				\begin{ex}%[0H5C4-7]%[CTST - Lớp 10 - Ôn tập cuối học kì 1 - Đề 4]%[Phạm Văn Long]
					...
					\begin{ex}%[0H9V1-3]
						\immini{Một vật đồng thời bị ba lực tác động: lực tác động thức nhất $\overrightarrow{F_1}$ có độ lớn là $15$ N, lực tác động thức hai $\overrightarrow{F_2}$ có độ lớn là $12$ N, lực tác động thức ba $\overrightarrow{F_3}$ có độ lớn là $8$ N. Các lực này được biểu diễn bằng các véc-tơ như hình bên, với $\left(\overrightarrow{F_1},\overrightarrow{F_2}\right)=30^\circ$, $\left(\overrightarrow{F_1},\overrightarrow{F_3}\right)=45^\circ$  $\left(\overrightarrow{F_2},\overrightarrow{F_3}\right)=75^\circ$. Tính độ lớn lực tổng hợp tác động lên vật (làm tròn kết quả đến hàng đơn vị). }{\begin{tikzpicture}[scale=0.9,font=\footnotesize,line join=round, line cap=round,>=stealth]
						\def\a{0.8}
						\def\b{1}
						%Vẽ các vật
