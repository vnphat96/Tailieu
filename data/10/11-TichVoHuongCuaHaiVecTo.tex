...
\begin{ex}%[0H2K2-1]
	...
	\tkzDefPoint(0,0){O}
	\tkzDefPoint(2,0){B}
	\tkzDefPoint[label=above: {$M$}](1,1.74){M}
	\tkzDefPoint[label=above left: {$N$}](-1.74,1){N}
	\tkzDefPoint[label=below left: {$A$}](-2,0){A}
	\tkzDefCircle[radius](O,B) \tkzDrawCircle(O,B) \tkzDrawPoints(A,B,O,N,M)
	\tkzLabelPoints(B,O)
	\tkzDrawSegments(B,A A,B B,N N,A A,M M,B)
	\tkzInterLL(N,B)(A,M) \tkzGetPoint{I} \tkzDrawPoints(I) \tkzLabelPoints(I)
