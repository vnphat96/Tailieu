...
\end{multicols}
	...
	\loigiai{
\begin{enumerate}
	\item Thành phố Hồ Chí Minh là thủ đô của nước Việt Nam. Đây là mệnh đề.
	\item Dơi có phải là một loài chim không? Đây không là mệnh đề.
	\item Thời tiết hôm nay đẹp quá! Đây không là mệnh đề.
	\item $\sqrt{2}$ là số vô tỉ. Đây là mệnh đề.
	\item Vịnh Hạ Long là di sản thiên nhiên thế giới. Đây là mệnh đề.
	\item $3^2+4^2=7^2$. Đây là mệnh đề.
...
\begin{ex}%[0T2B2-3]
	\immini{
		...
		{\True $\heva{&x\ge 0\\&5x-4y\ge 10\\&4x+5y\le 10}$}
	}
	{
		\begin{tikzpicture}[
			join=round, cap=round, >=stealth, thick, font=\footnotesize,
			declare function={xmin=-0.5; xmax=3.5; ymin=-3; ymax=3;}
			]
			\clip (xmin,ymin) rectangle (xmax,ymax);
			%BPT ax+c>=0, a>0
			...
			\end{scope}
			%BPT ax+by+c<=0, a>0, b<0
			\begin{scope}[declare function={a=5; b=-4; c=-10; m=(-b/a)*ymin-(c/a); n=(-b/a)*ymax-(c/a);}]
				\draw[blue] (m,ymin)--(n,ymax) node[above, pos=.5, sloped]{$ (d_2) $};
				\fill[pattern=north east lines, opacity=.5] (m,ymin)--(n,ymax)--(xmax,ymax)--(xmax,ymin)--cycle;
			\end{scope}
			%BPT ax+by+c<=0, a>0, b>0
			\begin{scope}[declare function={a=4; b=5; c=-10; m=(-b/a)*ymin-(c/a); n=(-b/a)*ymax-(c/a);}]
	...
	\loigiai{
		...
		Cạnh $AB$ qua hai điểm $\left(\dfrac{5}{2};0\right)$ và $(0;2)$ nên có phương trình $\dfrac{x}{\tfrac{5}{2}}+\dfrac{y}{2}=1\Leftrightarrow 4x+5y=10$.\\
		Vậy hệ bất phương trình cần tìm là $\heva{&x\ge 0\\&5x-4y\ge 10\\&4x+5y\le 10.}$
	}
\end{ex}
%%==========Câu 21
\begin{ex}%[0T2K2-3]
...
%%==========Bài 1
\begin{bt}%[Pj17-0-CK2-NH22-23--TeamTeXHoa--VoVanTu]%[0T1B3-1]
Cho hai tập hợp $A=\left\{x\in\mathbb{Z}| -2<x\le 4\right\}$, $B=\left\{x\in\mathbb{Z}| |x|\le 2\right\}$. Tìm $A\cap B$.
	\loigiai{
	Ta có $A=\left\{x\in\mathbb{Z}| -2<x\le 4\right\}\Rightarrow A=\{-1;0;1;2;3;4\}$.\\
	$B=\left\{x\in\mathbb{Z}| |x|\le 2\right\}\Rightarrow B=\{-2;-1;0;1;2\}$.\\
	Vậy $A\cap B=\{-1;0;1;2\}$.
	}
\end{bt}
%%==========Bài 2
...
\begin{bt}%[Pj17-0-CK2-NH22-23--TeamTeXHoa--VoVanTu]%[0T2T2-3]
	...
	\begin{center}
		\begin{tikzpicture}[>=stealth,line join=round,line cap=round,scale=1]
			\draw (0.25,-.5)node{Nhóm} (4,-.25)node{Số máy} (10,0.1)node{Số máy trong từng nhóm để sản xuất} (4,-.75)node{trong mỗi nhóm} (10,-0.4)node{ra một đơn vị sản phẩm}
