...
\begin{ex}%[0D7V3-6]
\immini[thm]{Mặt cắt đứng của cột cây số trên quốc lộ có dạng nửa hình tròn ở phía trên và phía dưới có dạng hình chữ nhật (xem hình bên). Biết rằng đường kính của nửa hình tròn cũng là cạnh phía trên của hình chữ nhật và đường chéo của hình chữ nhật có độ dài $66$ cm. Tìm diện tích của hình chữ nhật, biết rằng diện tích của phần nửa hình tròn bằng $0{,}3$ lần diện tích của phần hình chữ nhật. Lấy $\pi=3{,}14$ và làm tròn kết quả đến hàng đơn vị.}{\begin{tikzpicture}[scale=.8, >=stealth]
\coordinate (D) at (2,0);
\coordinate (B) at (-2,-5);
\coordinate (A) at (-2,0);
\coordinate (C) at (2,-5);
