	...
	\loigiai{
		...
		*Vẽ đồ thị: Ta có đỉnh là $I\left(1;-1\right)$ và trục đối xứng là $x=1$.\\
		...
		\begin{center}
			...
			\end{tabular}
		\end{center}
		\immini[thm]{Ta có đồ thị của hàm số $y=x^2-2x$ là}
	...
	\loigiai{
		...
		*Vẽ đồ thị: Ta có đỉnh là $I\left(2;0\right)$ và trục đối xứng là $x=2$.\\
		...
		\begin{center}
			...
			\end{tabular}
		\end{center}
		\immini[thm]{Ta có đồ thị của hàm số $y =-\dfrac{1}{2}x^2+2x-2$ là}
		{\begin{tikzpicture}[line width=1.0pt,line join=round,>=stealth,scale=0.7,transform shape]
...
\end{bt}
% \begin{bt}%[0D2B3-1]
% 	\immini[thm]{Cho đồ thị của hàm số bậc hai như hình bên
% 	\begin{enumerate}
...
\end{bt}
\begin{bt}%[0D2B3-1]
	\immini[thm]{Xác định dấu của các hệ số $a$, $b$, $c$ và dấu của biệt thức $\Delta=b^2-4ac$ của hàm số bậc hai $y=ax^2+bx+c$, biết đồ thị của nó có dạng như hình bên.}{
	\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round, >=stealth]
...
\end{ex}
\begin{ex}%[0D2B3-3]
	\immini[thm]{Cho hàm số $y=ax^2+bx+c$ với $(a, b, c \in \mathbb{R}, a \neq 0)$ có đồ thị như hình bên. Đồ thị bên là của hàm số nào?
		\choice
		{\True $y=2x^2-4x-1$}
		{$y=x^2-4x-1$}
		{$y=2x^2-4x+1$}
...
\end{ex}
%\begin{ex}%[0D2K3-3]
%	\immini[thm]{	Cho hàm số $y=ax^2+bx+c$ có đồ thị như hình bên. Khẳng định nào sau đây đúng?	
%		\choice
%		{$a>0,b<0,c>0$}
%		{\True $a>0,b>0,c<0$}
%		{$a>0,b<0,c<0$}
...
%\begin{ex}%[0D2G3-3]
%	\immini[thm]{Cho hai hàm số $y=a_1 x^2+b_1 x +c_1$ và $y=a_2 x^2+b_2 x + c_2$ với $a_1$, $a_2$, $b_1$, $b_2$, $c_1$, $c_2$ là các số thực, lần lượt có đồ thị là $(C_1)$ và $(C_2)$ như hình bên. Mệnh đề nào dưới đây đúng?
%		\choice
...
\begin{ex}%[0D2T3]
	...
	{$4$ m/s}
	\loigiai{
		\immini[thm]{Đồ thị của hàm vận tốc $v$ có dạng Parabol  bề lõm hướng xuống dưới. Đỉnh của Parabol là $I(9;121)$. Do đó trong đoạn $[0;20]$, vận tốc lớn nhất của vật là $121$ m/s.
		}{
...
%	\immini[thm]{Hãy xác định hệ số $a$.
...
\begin{ex}%[0D2B3-5]
	\immini[thm]{Một chiếc cổng hình parabol có dạng của đồ thị hàm số $y=-\dfrac{1}{2} x^{2}$ và có chiều rộng $d=8$ m (hình minh họa). Hãy tính chiều cao $h$ của cổng.
		\choice
		{\True $h=8$ m}
		{$h=9$ m}
		{$h=7$ m}
...
\end{ex}
%\begin{ex}%[0D2T3-5]
%	\immini[thm]{Một chiếc cổng hình parabol có phương trình $y=-\dfrac{1}{2}x^2$. Biết cổng có chiều rộng $d=6$ mét (như hình bên). Hãy tính chiều cao $h$ của cổng.
%		\choicew{0.4\textwidth}
%		\choice
%		{$h=5$ mét}
%		{$h=3,5$ mét}
...
%	}
%	\loigiai{
%		\immini[thm]{Đặt hệ trục toạ độ với $Axy$ như hình vẽ.\\
...
%\begin{ex}%[0D2B3-1]
%	\immini[thm]{Hàm số nào trong các hàm số sau đây có bảng biến thiên như hình vẽ
%		\choice
%		{\True $y = -x^2 + 2x -3$}
%		{$y = x^2 +2x -1 $}
%		{$y = -x^2 -x -1$}
