...
\begin{dang}{Nghiệm và miền nghiệm bất phương trình bậc nhất hai ẩn}
\end{dang}
\viduminhhoa
...
\begin{vd}%[Dự án Chuyển tex 10-11, Cao Thành Thái]%[0D4B4-1]
	...
	\loigiai
	{
		\immini[thm]{
			...
			Lấy gốc tọa độ $O(0;0)$, ta thấy $O\notin \Delta $ và có $2 \cdot 0+0<3$ nên nửa mặt phẳng kể cả bờ $\Delta$ chứa gốc tọa độ $O$ là miền nghiệm của bất phương trình đã cho (miền không bị gạch).
		}
		{
	...
	\loigiai{
		\immini[thm]{
			\begin{enumerate}
				...
				Vậy miền nghiệm của bất phương trình là nửa mặt phẳng không kể đường thẳng $d$ không chứa điểm $M$. (phần không bị gạch).
				\item Thế tọa độ điểm $A$ vào vế trái của PTĐT $(d)$ ta được $-2 \cdot 2 + 3 \cdot 1 = -1 < 0$.\hfill $(1)$\\
				Thế tọa độ điểm $B$ vào vế trái của PTĐT $(d)$ ta được $-2 \cdot 3 + 3 \cdot 3 = 3 > 0$. \hfill $(2)$ \\
	...
	\loigiai{
		\immini[thm]{
			\begin{itemize}
				...
				\item[b)] Để mọi điểm thuộc đường thẳng  $(d')$ đều là nghiệm của bất phương trình thì điều kiện cần là $(d')$ phải song song với $(d)$. Ta có $d:  y = -x + 3$ và $d': y = (m^2-2)x +  m + n$.\\
				$(d)$ song song $(d')$ thì $\heva{& m^2 - 2 = -1 \\& m + n \ne 3} \Leftrightarrow \hoac{&\heva{&m=1\\&n\ne 2}\\&\heva{&m=-1\\&n\ne 4}}$\\
...
\begin{bt}%[Mai Hà Lan]%[0D4B4]
	...
	\loigiai{
		\immini[thm]{
			...
			Thay tọa độ điểm $M(0;2)$ vào vế trái PTĐT $(d)$, ta được: $-2 < 0$.\\
...
\begin{bt}%[Mai Hà Lan]%[0D4K4]
	Biểu diễn hình học tập nghiệm của bất phương trình bậc nhất hai ẩn $\dfrac{x}{3} + \dfrac{y}{6} < 1$. 
	\loigiai{
		\immini[thm]{
		$\dfrac{x}{3} + \dfrac{y}{6} < 1 \Leftrightarrow 2x + y  <6$\\
...
\begin{bt}%[Lê Xuân Dũng]%[0D4B4]%[0D4K4]
	...
	\begin{enumerate}
		\item Biểu diễn miền nghiệm của bất phương trình đã cho trong mặt phẳng tọa độ $Oxy$.
		\item Tìm tất cả giá trị tham số $m$ để điểm $M(m,1)$ nằm trong miền nghiệm của bất phương trình đã cho và biểu diễn tập hợp $M$ tìm được trong cùng hệ trục tọa độ $Oxy$ ở câu a).
	\end{enumerate}	
	\loigiai{
		\immini[thm]{\begin{enumerate}
		\item Đường thẳng $(d){:} \, 2x+y-1=0$ có đồ thị như hình vẽ bên.
			Ta có $2.0+0-1 <0.$ Do đó, miền nghiệm là đường thẳng $(d)$ và miền không gạch chéo như hình vẽ bên (Miền chứa gốc tọa độ).			
			\item Để $M$ là một nghiệm thì $2m+1-1 \le 0\Leftrightarrow m\le 0.$ 
...
% 	\loigiai{
% 		\immini[thm]{a) Xét đường thẳng  $(d_m){:} \, x-2y+4m=0$ có đồ thị như hình vẽ bên.
...
\begin{bt}%[Nguyện Ngô]%[0D4K4]
	...
	\loigiai{
		...
		$\Leftrightarrow \heva{& y=6-x\\&x\leq 4\\ &x\geq 3\\& T=-30x+360 \longrightarrow max}$
		$\Leftrightarrow \heva{&y=6-x\\&3\leq x\leq 4\\& T=-30x+360 \longrightarrow max.}$\\
		Vì $T=30x+360$ là hàm số bậc nhất và có hệ số $a=-30<0$ nên $T$ đạt GTLN tại $x=3$.\\
...
\end{ex}
\begin{ex}%[0D4K4-1]%
	\immini[thm]{ Bất phương trình nào sau đây có miền nghiệm (phần không gạch sọc) như hình vẽ bên?
		\choice
		{\True $2x-y+1<0$}
		{$x-y+1<0$ }
		{$2x-3y+1<0$ }
...
\begin{ex}%[0D4K4-1]%Câu 24%[Dự án Tex hóa đề thi lớp 10-11-Nhóm Word-T-Begin-Lần 4]%[Lê Quốc Dũng\& Phản biện: Thanh Phong]%
	...
	\loigiai{
		\immini[thm]{Vẽ đường thẳng $2x+3y-2=0$.\\
...
\begin{ex} %[Trần Ngọc Lam]%[1181-1200 Trần Chiến; PB: Nguyễn Tài Tuệ]%[0D4K4-1]%
	...
	\loigiai{
		\immini[thm]{
			Thay tọa độ điểm $ O(0;0) $ vào PTĐT ta thấy không thỏa mãn.\\
...
\begin{ex}%[BG Toán 10-2022]%[Trần Nhân Kiệt]%[0D4T4-3]
	...
	\loigiai{
		...
		$$(900\cdot 5+10x)+(1200\cdot 2+15y)\le 20000 \Leftrightarrow 10x+15y\le 13100\Leftrightarrow 2x+3y\le 2620.$$
	}
\end{ex}
...
\begin{ex}%[BG Toán 10-2022]%[Trần Nhân Kiệt]%[0D4T4-3]
	...
	{\centering \begin{tikzpicture}[scale=.7,font=\footnotesize, line join=round, line cap=round, >=stealth]
			...
			\node[left] at (0,4){$80$};
			\clip(0,0) rectangle (5.3,5.3);
			\fill[pattern=north east lines](0,0)-- (3,0) -- (0,4)-- cycle;
			\draw[line width=1.2pt,smooth,samples=100,domain=0:3] plot(\x,{-1.3333 *(\x) +4});
	\end{tikzpicture}}
	\loigiai{
		\immini[thm]{Ta có bất phương trình $4x+3y \le 240$ mô tả số giờ làm việc trong mỗi tháng của cửa hàng. Biểu diễn nghiệm của bất phương trình như sau
		}
		{
			\centering \begin{tikzpicture}[scale=.7,font=\footnotesize, line join=round, line cap=round, >=stealth]
				\draw[->] (0,0) -- (5.3,0)node[below]{$x$};
