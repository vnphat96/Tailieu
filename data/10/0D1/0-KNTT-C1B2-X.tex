...
\begin{center}
	\begin{tabular}{|c|c|c|}
		...
		\end{tikzpicture}
		\\\hline
		Đoạn $[a;b]$&$\left\lbrace x \in \mathbb{R} \ \middle|\ a\leq x \leq b\right\rbrace $&\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round, >=stealth,baseline]
			\draw[->](0,0)->(5,0);
			\IntervalLR{0}{1.5}
...
\loigiai{
	\begin{enumerate}
		\item $A=\{x\mid x~ \text{là số tự nhiên lẻ},\, x\le 15\} = \{2k+1 \mid k \in \mathbb{N}, k \le 7\}$.
		\item $B=\{n \in \mathbb{N} \mid n~ \text{là bội của } 5\} = \{5k \mid k \in \mathbb{N}\}$.
...
\loigiai{
	\begin{enumEX}{1}
		...
		\item Phương trình $6x^2-7x+1=0$ có hai nghiệm $x_1=1\in \mathbb{Z}$ và $x_2=\dfrac{1}{6} \notin \mathbb{Z}$. Do đó $C\ne \varnothing $.
...
\loigiai{
	\begin{enumerate}
		\item Vì phương trình $x^2-3=0$ có hai nghiệm $x=\pm \sqrt{3}$ nên $B=\left\{-\sqrt{3}; \sqrt{3}\right\}$. Do vậy $A=B$.
		\item Vì mọi tam giác đều đều là tam giác cân, nhưng không phải mọi tam giác cân là tam giác đều, nên $C\subset D$ và $C\ne D$.
		\item Vì $24\ \vdots \ 12$ nên $E\subset F$ và $E\ne F$.
	\end{enumerate}
	}
\end{vd}
\begin{vd}%[0D1H2-2]
...
\loigiai{
	\begin{enumerate}
		...
		\item Ta có $B\cup C=\left\{-1;0;\frac13;1;2;3;4;5;6\right\}$ suy ra $A\cap \left(B\cup C\right)=\left\{-1;0;1;2;3;4;5\right\}=A$.
	\end{enumerate}
	}
\end{bt}
\begin{bt}%[0D1H3-1]%[Dự án KNTT lần 2, Nguyễn Tuấn]
...
\loigiai{
	\begin{enumerate}
		...
		\item Suy ra 
		\begin{itemize}
			\item $A\cup B=\left\{-6;-2;-1;0;1;2;3;4\right\}$
			\item $A\cap B=\left\{2\right\}$
			\item $B\backslash C=\left\{0;2;4\right\}$
			\item ${C}_{A\cup B}\left(B\backslash C\right)=\left(A\cup B\right)\backslash \left(B\backslash C\right)=\left\{-6;-2;-1;1;3\right\}$
		\end{itemize}
...
\loigiai{
	...
	$\bullet$ $n(T \cap V)=3,\; n(T \cap A)=4,\; n(V \cap A)=5, n(A \cap T \cap V)=2$.\\
	Số học sinh của nhóm là
	\begin{eqnarray*}
		|V \cup A \cup T|&=&n(V)+n(A)+n(T)-n(V \cap A)-n(T \cap A)-n(T \cap V)+n(A \cap T \cap V)\\
		&=&18+10+12-(3+4+5)+2=30.
	\end{eqnarray*}
...
\loigiai{
	...
	Theo đề bài, ta có:
	\begin{itemize}
		\item Số học sinh giỏi Toán là $n(M)=10$
		\item Số học sinh giỏi Lý là $n(L)=10$.
		\item Số học sinh giỏi Hóa là $n(H)=11$.
		\item Số học sinh giỏi Toán và Lý là $n(M\cap L)=6$.
		\item Số học sinh giỏi Toán và Hóa là $n(M\cap H)=4$.
		\item Số học sinh giỏi Lý và Hóa là $n(L\cap H)=5$.
		\item Số học sinh giỏi cả $3$ môn Toán, Lý, Hóa là $n(M\cap L \cap H)=3$.
	\end{itemize}
	Số học sinh giỏi ít nhất một môn là\\
	$n(M \cup L \cup H)=n(M)+n(L)+n(H)-\left[n(M\cap L)+n(M\cap H)+n(L\cap H)\right] +n(M\cap L\cap H)$.\\
