...
\begin{dang}{Bài toán sử dụng quy tắc cộng}
	{\begin{dn}{}
			...
			\immini{	\begin{itemize}
					\item  Phương án một có $n_1$ cách thực hiện,
					\item Phương án hai có $n_2$ cách thực hiện.
			\end{itemize}}{	\begin{tikzpicture}[scale=1,line cap=round,line join=round,font=\footnotesize,>=stealth]
					\path
					(0,0) coordinate (A)
					(2,0.5) coordinate (B)
					(2,-0.5) coordinate (C)
					;
					\draw (A)--(B)node[right]{\text{Phương án 1} $\ldots n_1$ \text{cách}}
					(A)--(C) node[right]{\text{Phương án 2 } $\ldots n_2$ \text{cách}};
			\end{tikzpicture}}
