...
{Hàm số $y=f(x)$ nghịch biến trên khoảng $\left(0;2\right)$}}{\begin{tikzpicture}[line cap=round,line join=round,>=stealth,x=1.0cm,y=1.0cm, scale=0.5]
\draw[->] (-2.,0.) -- (4.,0.);
\foreach \x in {2}
...
\begin{ex}%[0D2Y1-3]
\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình bên. Kết luận nào sau đây là \textbf{sai}?
\choice
...
\begin{ex}%[0D2B1-3]
\immini[thm]{Hàm số $y=f(x)$ có đồ thị như hình vẽ bên. Hàm số $y=f(x)$ đồng biến trên khoảng nào sau đây?
\choice
{\True $(1;3)$}
{$(-1;1)$}
{$(3;5)$}
{$(1;5)$}}{\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round,>=stealth,x=0.6cm,y=0.6cm]
\def \xmin{-1.7};
\def \xmax{5.9};
\def \ymin{-2.5};
\def \ymax{3.9};
...
\begin{ex}%[0D2B1-3]
\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình vẽ. Hàm số đồng biến trên khoảng nào sau đây?
\choicew{.7\textwidth}
\choice
{$(-2;+\infty)$}
{\True $(0; 3)$}
...
\begin{ex}%[0D3B1-3]
    \immini[thm]{Đồ thị hình vẽ dưới đây là đồ thị của một hàm số trong bốn hàm số được liệt kê ở bốn phương án $A$, $B$, $C$, $D$ dưới đây. Hỏi hàm số đó là hàm số nào?
        \choice
        {$f(x) = \heva{& 2x - 3 &,x \geq 1\\&x - 2 &,x < 1}$}
        {\True $f(x) = \heva{&2x - 3 &, x < 1\\&x - 2 &,x \geq 1}$}
        {$f(x) = \heva{&3x - 4 &, x \geq 1\\&-x &,x < 1}$}
        {$f(x) = |x - 2|$}}{\begin{tikzpicture}[line join=round, line cap=round,>=stealth,thick,scale=.7]
...
\begin{ex}%[0D3B1-3]
    \immini[thm]{Đồ thị hình vẽ là đồ thị của một hàm số trong bốn hàm số được liệt kê ở bốn phương án A, B, C, D dưới đây.
        ...
        {$y=2|x|+1$}
        {$y=|2x+1|$}
        {$y=|x+1|$}}{\begin{tikzpicture}[>=stealth,scale=0.7]
