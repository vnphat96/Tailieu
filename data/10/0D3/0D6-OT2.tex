...
\begin{ex}%[0D2Y2]
	...
	{$M(-1;-5)$}
	\loigiai{
		Thay $x =  - 2;y =  - 4$ vào phương trình đường thẳng thì không thỏa mãn.
	}
\end{ex}
...
\begin{ex} 
	\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị trên miền $[-3;5]$ như sau. Hàm số nghịch biến trên khoảng nào sau đây?
	\choice
	{$(-3;0)$}
	{\True $(0;1)$}
	{$(0;3)$ }
...
\begin{ex}%[0D2K3]
	\immini[thm]{Hình vẽ bên là đồ thị của hàm số nào sau đây?
	\motcot
	{$y=x^2+4x+5$}
	{$y=x^2+2x-1$}
	{\True $y=2x^2+8x+5$}
...
\begin{ex}%[0D2B3-3]%
	\immini[thm]{Cho hàm số $y=ax^2+bx+c$ có đồ thị như hình vẽ bên. Mệnh nào sau đây đúng?
		\choice
		{\True $a>0$, $b>0$, $c>0$}
		{$a>0$, $b<0$, $c>0$}
		{$a<0$, $b>0$, $c>0$}
...
\begin{ex}%[0D2K3-3]%
	\immini[thm]{Cho hàm số bậc hai có đồ thị như hình bên. Tập xác định và tập giá trị của hàm số theo thứ tự là
		\choice
		{$[-1; 7]$ và $(-\infty; 8]$}
...
\begin{ex}%[0D2B3]
\immini[thm]{Hàm số nào sau đây có bảng biến thiên như hình vẽ dưới? 
	\choice
	{ $y = -x^2 -4x -9$}
	{ $y = x^2 + 4x - 5$}
	{\True $y = x^2 + 4x - 1$}
