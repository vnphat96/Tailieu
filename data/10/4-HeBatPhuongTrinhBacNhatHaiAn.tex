...
\begin{vd}%[0D4B4]
	...
	\loigiai{
		\immini{
			...
			Thay tọa độ điểm $O(0;0)$ vào vế trái PTĐT $(d)$, ta được: $0<8$.\\
			Vậy miền nghiệm của bất phương trình là nửa mặt phẳng không kể bờ $d$ chứa điểm $O$ (trên hình là nửa mặt phẳng không bị gạch bỏ).
		}{
			\begin{tikzpicture}[line cap=round, line join=round, font=\footnotesize, >=stealth, scale=0.6]
				...
				%------------- Vẽ hàm
				\draw [ domain=-2:6, samples=100] plot (\x, {(1/2)*\x - 2}) node[above]{$d$};
...
\begin{vd}%[0D4B4]
	...
	\loigiai{
		\immini{
			...
			Thay tọa độ điểm $O(0;0)$ vào vế trái PTĐT $(d)$, ta được: $0 < 3$.\\
			...
			\begin{tikzpicture}[line cap=round, line join=round, font=\footnotesize, >=stealth, scale=0.6]
				...
				%---------- vẽ hàm
				\draw [domain=-.5:1.5, samples=100] plot (\x, {-3*\x + 3});
				\node at (2,-1) {$d$};
...
\begin{vd}%[0D4B4]
	\begin{enumerate}
		\item Biểu diễn hình học tập nghiệm của bất phương trình bậc nhất hai ẩn  $-2x + 3y > 0.$
		\item Cho hai điểm $A(2;1)$ và $B(3; 3)$, hỏi hai điểm này cùng phía hay khác phía đối với bờ $(d)$.
	\end{enumerate}
	\loigiai{
		\begin{enumerate}
			\item 
			\immini{\vspace*{-2.7em}Vẽ đường thẳng $d: -2x + 3y = 0$.\\
				Thay tọa độ điểm $M(1;0)$ vào vế trái PTĐT $(d)$, ta được: $-2 < 0$.\\
				...
				\begin{tikzpicture}[line cap=round, line join=round, font=\footnotesize, >=stealth, scale=0.8]
					...
					%--------------------- Vẽ hàm
					\draw [ domain=-1:3, samples=100] plot (\x, {(2/3) * \x}) node [above] {$d$};
					
					%---------------------- Điểm M
					\fill (1,0) circle (1.5pt) node[below]{$M$};
			...
			\item Thế tọa độ điểm $A$ vào vế trái của PTĐT $d$ ta được
			\[-2 \cdot 2 + 3 \cdot 1 = -1 < 0\]
			Thế tọa độ điểm $B$ vào vế trái của PTĐT $d$ ta được
			\[-2 \cdot 3 + 3 \cdot 3 = 3 > 0.\]
...
\begin{bt}%[0D4B4]
	...
	\[x + \dfrac{3}{2}y \ge 1 - x + \dfrac{1}{2}.\]
	\loigiai{
		\immini{
			Bất phương trình tương đương đã cho tương đương $2x + y  \ge 1$.
			Vẽ đường thẳng $d: 2x + y = 1 $\\
			Thay tọa độ điểm $O(0;0)$ vào vế trái PTĐT $d$, ta được: $0 < 1$.\\
			Vậy miền nghiệm của bất phương trình là nửa mặt phẳng không chứa điểm $O$, kể cả bờ $d$ (trên hình là nửa mặt phẳng không bị gạch bỏ).
		}{
			\begin{tikzpicture}[line cap=round, line join=round, font=\footnotesize, >=stealth, scale=1]
				\tikzset{label style/.style={font=\footnotesize}}
				...
				%--------------------- Vẽ hàm
				\draw [ domain=-.5:1.5, samples=100] plot (\x, {-2*\x+1});
				\node at (1.75,-1.5) {$d$};
...
\begin{bt}%[0D4B4]
	Biểu diễn hình học tập nghiệm của bất phương trình bậc nhất hai ẩn \[-2022x -2023y \le  2021y.\]
	\loigiai{
		\immini{
			Bất phương trình đã cho tương đương $-x - 2y \le 0 $\\
			Vẽ đường thẳng $d: -x - 2y = 0 $.\\
			Thay tọa độ điểm $M(1;0)$ vào vế trái PTĐT $(d)$, ta được: $-1 < 0$.\\
			...
			\begin{tikzpicture}[line cap=round, line join=round, font=\footnotesize, >=stealth, scale=1]
				...
				%--------------------- Vẽ hàm
				\draw [ domain=-3:3, samples=100] plot (\x, {-(1/2)*\x});
				\node at (-2,1.5) {$d$};
				
				%---------------------- Điểm M
				\fill (1,0) circle (1pt) node[below right]{$M(1;0)$};
...
\begin{bt}%[0D4B4]
	\begin{enumerate}
		\item Biểu diễn hình học tập nghiệm của bất phương trình bậc nhất hai ẩn $\dfrac{x}{3} + \dfrac{y}{6} < 1$.
		\item Tìm điểm $A$ thuộc miền nghiệm của bất phương trình trên. Biết rằng điểm $A$ là giao điểm của parabol $(P):y = x^2 - 5x +4$ và trục hoành. 
	\end{enumerate}
	\loigiai{
		\immini{
			\begin{enumerate}
				\item Bất phương trình đương đương $2x + y  <6$.\\
				Vẽ đường thẳng $d: 2x + y = 6$.\\
				Thay tọa độ điểm $O(0;0)$ vào vế trái PTĐT $d$, ta được $0 < 6$.\\
				Vậy miền nghiệm của bất phương trình là nửa mặt phẳng chứa điểm $O$ (trên hình là nửa mặt phẳng không bị gạch bỏ).
				\item Điểm $A$ nằm trên parabol $(P)$ có dạng $y = x^2 - 5x +4$ và trục hoành nên hoành độ của $A$ là nghiệm của phương trình $x^2 - 5x + 4 = 0 \Leftrightarrow \hoac{& x = 1\\ & x = 4.}$
				Suy ra ta được hai điểm $(1;0)$ và $(4;0)$. Lần lượt thế tọa độ từng điểm vào vế trái của PTĐT $(d)$, do $A$ thuộc miền nghiệm của bất phương trình đã cho nên ta được $A$ có tọa độ là $(1;0)$.
			\end{enumerate}
		}{
			\begin{tikzpicture}[line cap=round, line join=round, font=\footnotesize, >=stealth, scale=0.6]
				\tikzset{label style/.style={font=\footnotesize}}	
				...
				%--------------------- Vẽ hàm
				\draw [ domain=-1:4.5, samples=100] plot (\x, {-2*\x + 6});
				\node at (3.5,-2.75) {$(d)$};
				
				%---------------------- Vẽ (P)
				\draw [ domain=-.7:5.7, samples=100] plot (\x, {(\x)^2 - 5*\x + 4});
				\node at (1,-2) {$(P)$};
...
\begin{ex}%[0D4K4-4]
	...
	{\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round, >=stealth,xscale=1.5]
...
\begin{vd}%[BG10-2022]%[Toanvo]%[0D4G4-4]
	Cho cặp $\left(x;y\right)$  là nghiệm của hệ  $\heva{&3x-y \geq -1\\&2x+y \leq 6\\&x+3y>3}$. Tìm giá trị lớn nhất và nhỏ nhất của biểu thức $f\left(x;y\right)=2x-3y+1$.
	\loigiai{
		\begin{itemize}
...
\end{bt}
\subsubsection{Bài tập trắc nghiệm}
\Opensolutionfile{ans}[ans/ans-0D2-min-max]
\begin{ex}%[0D4K4-2]
...
\begin{ex}%[0D4K4-2]
	Tìm các cặp số $(x;y)$ thỏa mãn hệ bất phương trình dưới đây sao cho nó làm cho biểu thức $S=2x+y$ đạt giá trị lớn nhất.
	$$\heva{&0\le y\leq 2\\&y\leq x\\&x+y\leq 5\\&x\leq 4}$$	
	\choice
	{$(x;y)=(4;0)$}
	{\True $(x;y)=(4;1)$}
	{$(x;y)=(3;2)$}
