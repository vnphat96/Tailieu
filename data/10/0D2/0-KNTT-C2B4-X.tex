...
\begin{vd}%[0D2N2-1]%[Biên soạn tài liệu dạy học bộ KNTT-CS]%[Trịnh Ngọc Minh]
	Xét các hệ sau. Hãy cho biết hệ nào là hệ bất phương trình bậc nhất hai ẩn $x$, $y$ và giải thích.
		\begin{itemize}
			\item $\heva{&x-2y=1\\&x+y>0.}$ \dotfill
			\item $\heva{&\dfrac{x}{y}+2y>1\\&x-y\ge0.}$\dotfill
			\item $\heva{&3x-2y\ge1\\&x+3y<5.}$\dotfill
			\item $\heva{&(x-1)^2-(x-1)(x+1)+y>0\\&2x-y\le3.}$\dotfill
			\item $\heva{&x-2y>1\\&2x+y\ge0\\&x+y\le3.}$\dotfill
		\end{itemize}	
\loigiai{
	\begin{itemize}
...
\baitaptl
\setcounter{bt}{0}
\begin{bt}%[0D2H2-2]%[Biên soạn tài liệu dạy học bộ KNTT-CS]%[Trịnh Ngọc Minh]
...
\end{bt}
\begin{bt}%[0D2H2-3]%[Biên soạn tài liệu dạy học bộ KNTT-CS]%[Trịnh Ngọc Minh]
...
\end{bt}
\begin{bt}%[0D2V2-3]%[Biên soạn tài liệu dạy học bộ KNTT-CS]%[Trịnh Ngọc Minh]
...
\loigiai{
	...
	\begin{itemchoice}
		...
		Suy ra $ f(4;5) $ là giá trị lớn nhất của hàm số $ f(x;y) $ trên miền nghiệm của hệ $ (*). $}{\begin{tikzpicture}[line width=1pt,>=stealth,x=1cm,y=1cm,scale=0.6,line width=0.7pt]
