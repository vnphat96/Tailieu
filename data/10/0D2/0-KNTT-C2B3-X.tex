...
$$
Trong đó $a$, $b$, $c$ là những số thực và $a^2+b^2\neq 0$.\\
...
\end{vd}
\begin{vd}%[0D2N1-1]
	Cặp số nào sau đây là nghiệm của bất phương trình $10x+20y \leq 420$?
	\begin{enumEX}{2}
		\item $(9;5)$;
		\item $(2;400)$.
	\end{enumEX}
\loigiai{
	\begin{listEX}[1]
		...
		Vậy $(9;5)$ là nghiệm của bất phương trình $10x+20y \leq 420$.
		\item  Ta có $10 \cdot 2+20 \cdot 400=8020>420$.\\
		Vậy $(2;400)$ \textbf{không} phải là nghiệm của bất phương trình $10x+20y \leq 420$.
	\end{listEX}}
\end{vd}
\begin{vd}%[0D2N1-1]
...
\begin{dang}{Biểu diễn miền nghiệm của một bất phương trình bậc nhất hai ẩn}
	...
	\begin{enumerate}[\bfseries Bước 1.]
		\item Vẽ đường thẳng $\Delta\colon ax+by+c=0$.
		\item Lấy một điểm $M_0\left(x_0;y_0\right)$ không thuộc $\Delta$ (thường lấy gốc tọa độ $O$).
		\item Tính $ax_0+by_0+c$ và so sánh với $0$.
		\item Kết luận
		\begin{itemize}
...
\loigiai{
	...
	\begin{scope}
		...
		\fill[pattern=horizontal lines] (-3,5.33)--(-3,-1.33)--(7,-1.33)--cycle;
...
\end{vd}
\begin{vd}%[0D2H1-2]
	\immini{Nửa mặt phẳng \textbf{không} bị gạch (kể cả $d$) là miền nghiệm của bất phương trình nào?}
...
\begin{vd}%[0D2H1-2]
	...
	\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round, >=stealth,yscale=0.8]
		\draw[step=1,help lines,color=black!10!white] (-3,-1) grid (1.3,4);
		\draw[->] (-3,0)--(0,0) node[below left]{O}--(1.5,0) node[below]{$x$};
		\draw[->] (0,-1.1)--(0,4.2) node[left]{$y$};
		\draw[color=black]  plot[domain=-0.33:1.33](\x,{-3*(\x)+3});
		...
		(-3,-1) coordinate (K)
		;
		\fill[pattern=north east lines,opacity=.3] (M)--(N)--(P)--(K)--cycle;
	\end{tikzpicture}
	}
\loigiai{
	Đường thẳng $ 3x+y=3 $ đi qua hai điểm $ A(1;0) $ và $ B(0;3) $.\\
	Điểm $ O(0;0) $ không thuộc miền nghiệm của bất phương trình $ 3x+y\ge 3 $.
	}
\end{vd}
