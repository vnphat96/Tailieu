...
\begin{ex}%[Phan Anh]%[0D2K1-3]
    \immini[thm]{Cho đồ thị hàm số $y=x^3$ như hình bên. Khẳng định nào sau đây \textbf{sai}?
    \choice
...
\begin{ex}%[Phan Anh]%[0D2K1-3]
    \immini[thm]{Cho hàm số $y=f(x)$ có tập xác định là $\left[-3;3\right]$ và đồ thị của nó được biểu diễn bởi hình bên. Khẳng định nào sau đây là đúng?
    \choice
...
\begin{ex}%[Phan Anh]%[0D2B2-3]
    \immini[thm]{Đồ thị hình vẽ là đồ thị của một hàm số trong bốn hàm số được liệt kê ở bốn phương án A, B, C, D dưới đây.
    Hỏi hàm số đó là hàm số nào?
    \choice
    {$y=|x|+1$}
    {\True $y=2|x|+1$}
    {$y=|2x+1|$}
    {$y=|x+1|$}}{\begin{tikzpicture}[>=stealth,yscale=0.5]
    \draw[->](-2,0)--(2,0)node[above]{$x$};
...
\begin{ex}%[Phan Anh]%[0D2B2-3]
    \immini[thm]{Đồ thị hình bên là đồ thị của một hàm số trong bốn hàm số được liệt kê ở bốn phương án A, B, C, D dưới đây. Hỏi hàm số đó là hàm số nào?
    \choice
    {$y=\left| x\right|$}
    {$y=\left| x\right|+1$}
    {\True $y=1-\left| x\right|$}
    {$y=\left| x\right|-1$}}{\begin{tikzpicture}[>=stealth,scale=0.6]
    \draw[->](-2,0)--(2,0)node[above]{$x$};
    \draw[->](0,-1)--(0,2)node[right]{$y$};
...
\begin{ex}%[Phan Anh]%[0D2B2-3]
    \immini[thm]{Cho hàm số $y=ax+b$ có đồ thị là hình bên. Tìm $a$ và $b$.
    \choice
    {$a=-2$ và $b=3$}
    {$a=-\dfrac{3}{2}$ và $b=2$}
    {$a=-3$ và $b=3$}
    {\True $a=\dfrac{3}{2}$ và $b=3$}}{\begin{tikzpicture}[>=stealth,yscale=0.7,scale=0.5]
...
\begin{ex}%[Phan Anh]%[0D2B2-3]
    \immini[thm]{Đồ thị hình vẽ là đồ thị của một hàm số trong bốn hàm số được liệt kê ở bốn phương án A, B, C, D dưới đây. Hỏi hàm số đó là hàm số nào?
    \choice
    {$y=\left| x\right|$}
    {$y=-x$}
    {$y=\left| x\right|$ với $x>0$}
    {\True $y=-x$ với $x<0$}}{\begin{tikzpicture}[>=stealth,scale=0.5]
    \draw[->](-3,0)--(1.5,0)node[above]{$x$};
...
\begin{ex}%[Phan Anh]%[0D2K2-4]
Tìm PTĐT $d\colon y=ax+b$. Biết đường thẳng $d$ đi qua điểm $I(1;2)$ và tạo với hai tia $Ox$, $Oy$ một tam giác có diện tích bằng $4$.
\choice
{$y=-2x-4$}
{\True $y=-2x+4$}
{$y=2x-4$}
