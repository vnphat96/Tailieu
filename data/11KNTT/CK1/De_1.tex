...
\begin{ex} %[1D3N1-1]
	Cho hai dãy $\left(u_n \right)$ và $\left(v_n \right)$ thỏa mãn $\lim \limits{n \to +\infty}u_n=2$ và $\lim \limits{n \to +\infty}v_n=3$. Giá trị của $\lim \limits{n \to +\infty}\left(u_n\cdot v_n \right)$ bằng
	\choice{$5$}
	{\True $6$}
	{$-1$}
	{$1$}
	\loigiai{
		Ta có $\lim \limits{n \to +\infty}\left(u_n\cdot v_n \right)=\lim \limits{n \to +\infty}u_n \cdot \lim \limits{n \to +\infty}v_n=2\cdot 3=6$.
	}
\end{ex}
...
\begin{ex}%[1D3N1-2]%[Dự án đề ôn tập Toán Khối 11 HK1 NH23-24-Dot 1-Vương Quốc Phong]%[CTST - Đề số 3]
	...
	{$\dfrac{1}{5}$}
	\loigiai{
		Ta có $\lim \limits{n \to +\infty}\dfrac{1}{2n+5} = 0$.
	}
\end{ex}
