...
\begin{ex}%[1H4H4-2]
	\immini[thm]{Cho hình chóp $S. ABCD$. Gọi $M$, $N$, $P$ lần lượt là trung điểm các cạnh $SA$, $AB$ và $AD$ (tham khảo hình bên).
...
\begin{ex}%[1H4N1-3]%[Lê Xuân Hòa ]
	...
	{\True $(ACD)$}
	\loigiai{
		\immini[thm]{Vì $I=MN\cap BD$ nên $I\in\left(ABD\right)$, $I\in\left( BCD\right)$, $I\in\left(CMN\right)$.
		}
		{
...
\begin{ex}%[BG 11 - Nguyen Huynh]%[1D1H4-3]
	\immini[thm]{Cho hàm số $y=2\sin x$ trên đoạn $[-\pi;\pi]$ có đồ thị như hình bên. Xét tính đúng sai của các khẳng định sau:
	\choice
...
\begin{ex}%[1D3N1-1]%[Dự án đề ôn tập Toán khối 11 NH23-24-Đợt 1- Bùi Thanh Cương]%[CTST-Đề số 08]
	...
	{$ 1$}
	\loigiai{
		Ta có $\lim\left(u_n+v_n\right)= \lim \limits_{n \to +\infty}u_n+ \lim \limits_{n \to +\infty}v_n=2+3=5$.
	}
...
\begin{ex}%[Pj10-Đề 07 HK1 NH2023-2024 CTST]%[Nguyễn Văn Nay]%[1D3N1-2]
	Tìm giới hạn $\lim \limits_{n \to +\infty}\dfrac{3n-1}{2n+1}$.
	\choice
	{$\dfrac{2}{3}$}
	{$3$}
	{$0$}
	{\True $\dfrac{3}{2}$}
	\loigiai{
		Ta có $\lim \limits_{n \to +\infty}\dfrac{3n-1}{2n+1}
			=\lim \limits_{n \to +\infty}\dfrac{3-\dfrac{1}{n}}{2+\dfrac{1}{n}}
			=\dfrac{\lim \limits_{n \to +\infty}\left(3-\dfrac{1}{n} \right)}{\lim \limits_{n \to +\infty}\left(2+\dfrac{1}{n} \right)}
			=\dfrac{\lim \limits_{n \to +\infty}3-\lim \limits_{n \to +\infty}\dfrac{1}{n}}{\lim \limits_{n \to +\infty}2+\lim \limits_{n \to +\infty}\dfrac{1}{n}}
...
\begin{ex}%[1D1H4-7]%[BG K11, Nhật Thiện]
	\immini[thm]{Đồ thị trong hình vẽ bên là đồ thị của hàm số nào dưới đây?
		\choice
		{$y=\sin 2x$}
		{$y=2\cos x$}
		{$y=\cos 2x$}
...
\begin{ex} %[1H4C4-6]
	...
	\immini[thm]{Trong hình thang cân $MNPQ$, kẻ $QH\perp MN,PF\perp MN$.\\
