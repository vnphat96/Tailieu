...
\caulc
\Opensolutionfile{ans}[ans/11-CK1-1]
...
\begin{ex}%[1D3V3-3]%[KNTT - Lớp 11 - Ôn tập cuối học kì 1 - Đề 3]%[Võ Thị Thùy Trang]
	Cho hàm số $f(x)=\heva{&\dfrac{\sqrt{x+3}-2}{x-1}&\text{khi}~ x > 1\\ &-x^2+m &\text{khi}~ x\leq 1}$.
	\choiceTF
	...
	\loigiai{
		\begin{itemchoice}
			\itemch Hàm số xác định trên $\mathbb{R}$.
			\itemch Ta có $f(1)=-1+m$.
			\itemch Ta có $\lim\limits_{x \to 1^{-}} f(x)=\lim\limits_{x \to 1^{-}}\left(-x^2+m\right)=-1+m$.
			\itemch Ta có $f(1)=-1+m$.
		\end{itemchoice}
		$\lim\limits_{x \to 1^{-}} f(x)=\lim\limits_{x \to 1^{-}}\left(-x^2+m\right)=-1+m$.
		\allowdisplaybreaks
		\begin{eqnarray*}
