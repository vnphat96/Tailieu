...
\begin{ex}%[1H4N1-3]%[Dự án đề ôn tập khối 11 HKI NH2023-2024-Đơt 1-TheHung Nguyen]%[CD-Đề số 1]
	\immini[thm]{Cho hình chóp $S.ABCD$, đáy $A B C D$ là hình bình hành tâm $O$. Giao tuyến của hai mặt phẳng $(S A C)$ và $(S A D)$ là
		\choice
		{$S O$}
		{$S D$}
		{\True $SA$}
...
\begin{ex}%[1D3N1-1]%[Dự án đề kiểm tra Toán 11 HHKI NH23-24- Phạm Văn Long]%[Thi thử - KNTT]
	Cho hai dãy $\left(u_n \right)$ và $\left(v_n \right)$ thỏa mãn $\lim \limits_{n \to +\infty}u_n=2$ và $\lim \limits_{n \to +\infty}v_n=3$. Giá trị của $\lim \limits_{n \to +\infty}\left(u_n+v_n \right)$ bằng
	\choice{\True $5$}
	{$6$}
	{$-1$}
	{$1$}
	\loigiai{
		Ta có $\lim \limits_{n \to +\infty}\left(u_n+v_n \right)=\lim \limits_{n \to +\infty}u_n+\lim \limits_{n \to +\infty}v_n=2+3=5$.
	}
\end{ex}
...
\begin{ex}%[1D3N1-2]%[Dự án đề kiểm tra Toán 11 GHKI NH23-24- Nguyễn Văn Sang]%[ĐỀ KNTT SỐ 5]
	...
	{$+\infty$}
	\loigiai{
		Ta có $\lim \limits_{n \to +\infty}\dfrac{2}{n^2+1}=\lim \limits_{n \to +\infty}\dfrac{1}{n^2}\cdot \dfrac{2}{1+\tfrac{1}{n^2}}=0\cdot 2=0$.
	}
\end{ex}
...
\begin{ex} %[1D3H2-2]
	$A=\lim\limits_{x \to 2}( x^3-18x^2+2 )$ có giới hạn hữu hạn là
	\choice
	{\True $-62$}
	{ $-15$}
	{ $62$}
	{ $15$}
	\loigiai{
	$A=\lim\limits_{x \to 2}( x^3-18x^2+2 )=\lim\limits_{x \to 2}x^3-\lim\limits_{x \to 2}18x^2+\lim\limits_{x \to 2}2=2^3-{18\cdot 2^2}+2=-62$.
	} \end{ex}
...
\begin{ex}%[1H4H1-4]
	...
	\loigiai{
		\immini[thm]{		Trong mặt phẳng $(SAC)$, gọi $E=KH \cap SO$.\\
...
\begin{ex}%[1H4H3-2]
	...
	\loigiai{\immini[thm]{$\heva{&P Q \parallel A B \\ &A B \subset(A B C D)  \\& P Q \not \subset(A B C D)}\Rightarrow P Q \parallel(A B C D)$.}{
...
\begin{ex}%[1H4H1-2]%[Dự án đề ôn tập Toán khối 11 HKI NH23-24-Dot 1-Xuan Vy Pham]%[CTST-Đề số 4]
	...
	{\True 6}
	{1}
	\loigiai{ \immini[thm]{Xét hình chóp ngũ giác $S.ABCDE$ có đáy là ngũ giác $ABCDE$. Dựa vào hình vẽ ta có hình chóp ngũ giác này có 6 mặt là $(SAB)$, $(SBC)$, $(SCD)$, $(SDE)$, $(SAE)$, $(ABCDE)$.}{	\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round]
...
\begin{ex}%[Dự án BGK10-11,2024]%[Nguyễn Văn Nay]%[1D1H4-6]
	...
	{$y=\sqrt{2}\sin \left(x+\dfrac{\pi}{4}\right)$ (sửa đề)}
...
\begin{ex}%[1D3H1-2]%[Dự án đề kiểm tra Toán 11 HKI NH23-24-Đợt 1- Phạm Phương]%[CTST-Đề số 5]
	%[TH]
	Giá trị của $A=\lim \limits_{n \to +\infty}\dfrac{2n+1}{n-2}$ bằng
	\choice
	{$+\infty$}
	{$-\infty$}
	{\True $2$}
	{$1$}
	\loigiai{
		Ta có $A=\lim \limits_{n \to +\infty}\dfrac{2n+1}{n-2}=\lim \limits_{n \to +\infty}\dfrac{\dfrac{2n}{n}+\dfrac{1}{n}}{\dfrac{n}{n}-\dfrac{2}{n}}=\lim \limits_{n \to +\infty}\dfrac{2+\dfrac{1}{n}}{1-\dfrac{2}{n}}=\dfrac{2+0}{1-0}=2$.
	}
\end{ex}

\begin{ex}%[1-HK1-CT-3-2324]%[VN-MT-9, Đỗ Nam]%[1H4H4-2]
	\immini[thm]{Cho hình hộp $ABCD.A'B'C'D'$. Mặt phẳng $\left(AB'D'\right)$ song song với mặt phẳng nào sau đây?
		\choice
		{$\left(BAC'\right)$}
		{$\left(BDA'\right)$}
		{$\left(ACD'\right)$}
	...
	\loigiai{
		...
		\begin{itemize}
			\item Ta có\\
			      \allowdisplaybreaks
			      $\begin{aligned}
					      \lim\limits_{x\to 1}\dfrac{\sqrt{2x-1}-1}{x-1}
					       & =\lim\limits_{x\to 1}\dfrac{\left( \sqrt{2x-1}-1 \right)\left( \sqrt{2x-1}+1 \right)}{\left( x-1 \right)\left( \sqrt{2x-1}+1 \right)} \\
			...
			\item Ta có\\
			      \allowdisplaybreaks
			      $\begin{aligned}
					      \lim\limits_{x\to 1}\dfrac{\sqrt[3]{3x-2}-1}{x-1}
					       & =\lim\limits_{x\to 1}\dfrac{\left( \sqrt[3]{3x-2}-1 \right)\left[ {{\left( \sqrt[3]{3x-2} \right)}^2}+\sqrt[3]{3x-2}+1 \right]}{\left( x-1 \right)\left[ {{\left( \sqrt[3]{3x-2} \right)}^2}+\sqrt[3]{3x-2}+1 \right]} \\
