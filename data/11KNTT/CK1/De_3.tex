...
\begin{ex}%[1D3N2-1]
	Giả sử $\lim\limits_{x \rightarrow x_0} f(x)=L$, $\lim\limits_{x \rightarrow x_0} g(x)=M\, (L, M \in \mathbb{R})$. Chọn đáp án \textbf{sai}.
	\choice
...
\begin{ex}%[1D3H1-2]%[Dự án EX-CK1 (10,11 mới)- Lâm Chính]
	...
	{$+\infty$}
	\loigiai{
	Ta có $\lim \limits_{n \to +\infty}\dfrac{2^{n+1}+4^n}{3^n+4^{n+1}}=\lim \limits_{n \to +\infty}\dfrac{2\cdot2^n+4^n}{3^n+4\cdot4^n}=\lim \limits_{n \to +\infty}\dfrac{2\cdot\left(\dfrac{2}{4} \right)^n+1}{\left(\dfrac{3}{4} \right)^n+4}=\dfrac{2\cdot0+1}{0+4}=\dfrac{1}{4}$.
...
\begin{ex}%[1D3H2-2]
	...
	{$1$}
	\loigiai{
		Ta có $\lim\limits \left(u_n-v_n\right)=\lim \limits_{n \to +\infty}u_n-\lim \limits_{n \to +\infty}v_n
			=-3-2=-5$.}
\end{ex}
...
\begin{ex}.%[1H4V1-3]
	...
	{ $A N=(A B M) \cap(S A C)$}
	\loigiai{
		\immini[thm]{Ta có $B \in(A B M) \cap(S B D).\quad(1)$\\
...
\begin{ex}%[1H4H3-2]
	...
	{\True Mặt phẳng $(ABCD)$}
	\loigiai{
		\immini[thm]{Ta có $M$ và $N$ lần lượt là trung điểm của $SA$ và $SC$ nên $MN$ là đường trung bình của $\triangle SAC$, suy ra $MN\parallel AC.$\\
...
\begin{ex}%[1H4H1-3]
	...
	{$AC$}
	\loigiai{
		\immini[thm]{$M$ là điểm chung thứ nhất của hai mặt phẳng $\left(ABM\right)$ và $\left(SCD\right).\quad (1)$\\
...
\begin{ex}%[1C4K46]
	...
	\loigiai{
		\begin{center}
			\begin{tikzpicture}[line cap = round, line join = round, scale =1.2, font = \small, declare function={a=5; b=0.5*a; h=4; d=1.5; bad=-60;}]
				\path
				(0,0) coordinate (A)
				(a,0) coordinate (D)
				(90:h) coordinate (S)
				...
				($(C)!.5!(D)$) coordinate (P)
				($(C)!.5!(S)$) coordinate (N)
				($(S)!.5!(C)!1/3!(D)$) coordinate (G)
				;

				\draw[blue] (E)--(Q) (F)--(P); \draw[blue, dashed] (E)--(F) (P)--(Q);
				\draw[green] (B)--(E) (F)--(C);
				\draw[red,dashed] (G)--(O); \draw[red] (N)--(B);
				\draw [dashed] (C)--(A)--(D)--(B) (C)--(E);

				\draw (B)--(S) (A)--(S)--(D)--(C)--(B)--cycle (S)--(C) (D)--(N);

				\foreach \x/\goc in {S/90,A/180,B/-135,C/-60,D/-45,O/-90,E/180,F/0,G/-50,P/-45,Q/-135,N/45}{
						\draw[fill] (\x) circle (1pt) node[shift={(\goc:8pt)}]{$\x$};
					}
			\end{tikzpicture}
		\end{center}
		\begin{enumerate}
			...
			\item Gọi $I=AB \cap SO$. Suy ra $ (SAB)\cap (ECD)=EI $. Suy ra $ M=SB\cap (ECD)=EI\cap SB$.
			\begin{center}
				\begin{tikzpicture}[line cap = round, line join = round, scale =1, font = \small, declare function={a=5; b=0.5*a; h=4; d=1.5; bad=-60;}]
					\path
					(0,0) coordinate (A)
					(a,0) coordinate (D)
					(90:h) coordinate (S)
					...
					(intersection of S--B and E--I) coordinate (M)
					;
	
					\draw [dashed] (A)--(D) (B)--(C) (M)--(F) (B)--(D);
	
					\draw (S)--(A)--(I)--(D)--cycle (B)--(S)--(C) (S)--(I)--(E);
	
					\foreach \x/\goc in {S/90,A/180,B/-135,C/-60,D/-45,O/-90,E/180,F/0,M/-135,I/-90}{
							\draw[fill] (\x) circle (1pt) node[shift={(\goc:8pt)}]{$\x$};
						}
				\end{tikzpicture}
			\end{center}
