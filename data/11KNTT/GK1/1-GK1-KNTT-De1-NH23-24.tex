...
\begin{bt}%[1D1B2-1]% ::Cau 31::
	\begin{enumEX}{1}
		\item Cho $\alpha \in (-\dfrac{\pi}{2};0)$ và $\sin \alpha = -\dfrac13$. Tìm $\cos \alpha$, $\tan \alpha$, $\cot \alpha$. 
		\item Giải phương trình $\cot \left( 2x-40^{\circ} \right)=-\sqrt{3}$.
	\end{enumEX}
	\loigiai{
		\begin{enumEX}{1}
		\item Ta có $\alpha \in (-\dfrac{\pi}{2};0)$ nên $\cos \alpha >0$. Suy ra $$\cos^2 \alpha = 1-\sin^2 \alpha = 1-\left(-\dfrac13\right)=\dfrac89 \Rightarrow \cos \alpha = \dfrac{2\sqrt{2}}{3}.$$ 
...
\end{bt}
\begin{bt}%[Nguyễn Tâm Phục]%[2D2T4-5]
