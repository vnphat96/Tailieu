...
\begin{vd}%[1H3K2]
	...
	\loigiai{\immini[thm]{Ta có $SAB$ và $SAC$ là tam giác đều, $ABC$ và $SBC$ là tam giác vuông cân cạnh huyền $BC$.\\
...
\begin{vd}%[1H3K2]
	...
	\loigiai{
	\immini[thm]{Ta có $ABCD$ là hình thoi, gọi $O$ là giao điểm của $AC$ và $BD$ suy ra $O$ là trung điểm của $AC, BD$.\\
...
\begin{bt}%[1H3B2]
	...
	\loigiai{
	\immini[thm]{Gọi $M,N$ lần lượt là trung điểm của $AB, CD$. Do $\Delta SAB=\Delta SCD$ nên ta suy ra $SM=SN$.\\
...
\begin{bt}%[1H3B2]
	...
	\loigiai{
	\immini[thm]{Gọi $K$ là trung điểm $CD$, khi đó $BK\parallel B'N$. Ta sẽ chứng minh $BK\perp AM$.\\
...
\begin{bt}%[1H3B2]
	...
	\loigiai{
	\immini[thm]{Từ giả thiết ta có $$\Delta SAC=\Delta DAC \Rightarrow \widehat{ASC}=\widehat{ADC}=90^{\circ}\Rightarrow SA\perp SC.$$
...
\begin{bt}%[1H3K2]
	...
	\loigiai{
	\immini[thm]{Ta có $AK\parallel HC$, do đó chỉ cần chứng minh $SH\perp HC$.\\
...
\begin{bt}%[1H3B2]
	...
	\loigiai{
	\immini[thm]{Gọi $I$ là trung điểm $AD\Rightarrow ABCI$ là hình vuông cạnh $a$, do đó $\Delta CID$ vuông cân tại $I$. Từ đó ta có $CD^2=2a^2$. \hfill (1)\\
...
\begin{bt}%[1H2K1]
	...
	\loigiai{
	\immini[thm]{Xét các tam giác $ACI$ và $BCI$, ta có:\\
...
% \end{bt}
% \begin{bt}%[1C8B1-2]
% 	\immini[thm]{Cho hình hộp $MNPQ.M'N'P'Q'$ có góc giữa hai đường thẳng $MN$ và $MQ$ bằng $70^{\circ}$.
% 	\begin{listEX}
...
% \end{bt}
% \begin{bt}%[1C8B1-3]
% 	\immini[thm]{Cho hình chóp $S.ABCD$ có đáy $ABCD$ là hình thoi. Gọi $M$, $N$ lần lượt là trung điểm của các cạnh $SB$ và $SD$. Chứng minh rằng $AC \perp MN$.}
% 	{
...
% 	\loigiai{\immini[thm]{Vì $B'C' \parallel BC$ nên $(AB,B'C')=(AB,BC)=\widehat{ABC}=60^\circ$.}{\begin{tikzpicture}[line cap=round,line join=round, >=stealth,font=\footnotesize, scale=0.9]
...
% 	\loigiai{\immini[thm]{Hình hộp đã cho có các cạnh bằng nhau nên tứ giác $ABCD$ là một hình thoi. Suy ra $AC \perp BD$. Mà $BD\parallel B'D'$ nên $AC \perp B'D'$.\\
...
% 	\end{enumerate}
% 	\loigiai{\immini[thm]{\begin{enumerate}[a)]
...
% %%%%%%%%%
% \begin{bt}%[1C8B1-3]
% 	\immini[thm]{Hình bên gợi nên hình ảnh $5$ cặp đường thẳng vuông góc. Hãy chỉ ra $5$ cặp đường thẳng đó.}
% 	{
...
% \end{bt} 
% \begin{bt}%[1C8B1-2]
% 	\immini[thm]{Cho hình chóp $S.ABCD$ có đáy $A B C D$ là hình bình hành và $\widehat{SAB}=100^{\circ}$ (Hình bên). Tính góc giữa hai đường thẳng:
% 	\begin{listEX}
% 	\item $SA$ và $AB$,
% 	\item $SA$ và $CD$.
% 	\end{listEX}}
...
% 	\immini[thm]{Ví dụ. Hình lập phương $ABCD.A'B'C'D'$ có $ AB $ và $ CD $ cùng vuông góc với $ BC $ nhưng $ AB $ và $ CD $ song song.}
% 	{
