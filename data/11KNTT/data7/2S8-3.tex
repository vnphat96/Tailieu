...
\centerline {
	...
	(.9,-1.65)coordinate(M)
	(0,-2)coordinate(N);
	\coordinate (I) at ($(E)!.4!(F)$);
	\coordinate (H) at ($(E)!.12!(F)$);
	\coordinate (L) at ($(M)!.38!(N)$);
	...
	(2.5,1)coordinate(J)
	(2.5,0)coordinate(Q);
	\coordinate (I) at ($(E)!.3!(F)$);
	\coordinate (H) at ($(E)!.2!(F)$);
...
%=============
\begin{bt}%[1C8K6-1]
	\immini[thm]{Cho hình chóp $S . A B C D$ có $S A \perp(A B C D)$, đáy $A B C D$ là hình chữ nhật (Hình bên). Chứng minh rằng:
	\begin{enumerate}
...
\end{bt}
\begin{bt}%[1C8B6-1]
	\immini[thm]{Cho hình chóp $S . A B C D$ có $(S A B) \perp(A B C D)$, đáy $A B C D$ là hình chữ nhật (Hình bên). Chứng minh rằng: $(S B C) \perp(S A B)$}{\begin{tikzpicture}[scale=.7,>=stealth, font=\footnotesize, line join=round, line cap=round]
...
% \end{bt}
% \begin{bt}%[1C8B4-2]
% 	\immini[thm]{Cho hình chóp $S . A B C D$ có đáy $A B C D$ là hình vuông cạnh $a$ với tâm $O, S O=\dfrac{a \sqrt{2}}{2}$. Hai mặt phẳng $(S A C)$ và $(S B D)$ cùng vuông góc với mặt phẳng $(A B C D)$.
% 	\begin{enumerate}
