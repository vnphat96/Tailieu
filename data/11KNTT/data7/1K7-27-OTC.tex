...
% \begin{ex}%[Đỗ Minh Phúc]%%
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7YL-3]%
		...
		{\True $DC'$}
		\loigiai{
			\immini[thm]{Ta có $ABA'B'$ là hình vuông nên $ A'B\perp AB'$\\
				mà $ AB' \parallel C'D$\\
				nên $ A'B\perp C'D.$}
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BO-2]%
		\immini[thm]{Cho hình chóp $S.ABC$ có đáy là tam giác vuông tại $A$ và cạnh bên $SA$ vuông góc với mặt phẳng đáy $(ABC)$ (minh họa hình bên). Khẳng định nào sau đây \textbf{sai}?
			\choice
			{$(SAB)\perp(SAC)$}
			{\True $(ABC)\perp(SBC)$}
			{$(SAC)\perp(ABC)$}
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BL-1]% 
		...
		\loigiai{
			\begin{itemize}
				...
				\item Góc giữa hai đường thẳng bằng góc giữa hai vectơ chỉ phương của hai đường thẳng đó là phát biểu sai vì góc giữa hai đường thẳng thuộc $\left[0^{\circ}; 90^{\circ}\right]$ còn góc giữa hai vectơ thuộc $\left[0^{\circ}; 180^{\circ} \right]$
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BM-2]%
		...
		{$BC \perp(SAC)$}
		\loigiai{
			\immini[thm]{Ta thấy $BC\perp AB$ và $BC\perp SA$ nên $BC\perp (SAB)$.}{
				\begin{tikzpicture}[scale=1.0, font=\footnotesize, line join=round, line cap=round, >=stealth]
					\def\a{2}
					\path
					(0,0) coordinate (A)
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BM-2]%
		...
		{\True $BC \perp(ADH)$}
		\loigiai{
			\immini[thm]{Ta có $\heva{&BC\perp AH\\&BC\perp DH}\Rightarrow BC\perp(ADH)$.}{\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
					\coordinate (B)at (0,0);
					\coordinate (D)at (5,0);
					\coordinate (C) at (2,-2);
					\coordinate (A) at (3,3);
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7KN-3]%
		...
		{\True $30^{\circ}$}
		\loigiai{
			\immini[thm]{Ta có $\heva{&CD\perp SA\\&CD\perp AD}\Rightarrow CD\perp(SAD)$.\\
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BN-3]%
		...
		{$\widehat{BAD}$}
		\loigiai{
			\immini[thm]{Góc giữa cạnh bên và mặt đáy là các góc $\widehat{SCA}$, $\widehat{SAC}$, $\widehat{SBD}$, $\widehat{SDB}$.}{
				\begin{tikzpicture}[font=\footnotesize, line join=round, line cap=round, >=stealth]%Choptugiacdeu
					\def\a{2}
					\def\b{0.5*\a}
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BP-3]%
		...
		{Đáp án khác}
		\loigiai{
			\immini[thm]{Gọi $O$ là tâm của hình vuông $ABCD$ và $H$ là hình chiếu vuông góc của $D$ lên $D'O$.\\
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7KP-4]%
		...
		{\True Đáp án khác}
		\loigiai{
			\immini[thm]{Do $SA=SB=22$ và $\widehat{SAB}=60^{\circ}$ nên $\triangle SAB$ đều $\Rightarrow AB=22$.\\
	...
	%	{$\dfrac{3 a}{4}$}
	%	\loigiai{
		%		\immini[thm]{Ta có $(ABC)\parallel (A'B'C')$ nên
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7BQ-1]%
		...
		\loigiai
		{
			\immini[thm]{Ta có $V=\dfrac{1}{3}{S_{ABCD}}\cdot SA=\dfrac{1}{3}a^2\cdot a\sqrt{3}=\dfrac{a^3\sqrt{3}}{3}$.}
	...
	\begin{ex}%[Đỗ Minh Phúc]%[1K7KQ-1]%
		...
		\loigiai{
			\immini[thm]{Gọi $I$ là trung điểm của $B'C'$\\
