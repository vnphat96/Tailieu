...
\begin{boxdn}\immini[thm]{Có duy nhất một mặt phẳng đi qua một điểm cho trước và vuông góc với một đường thẳng cho trước.}{
...
\end{note}
\begin{nx}
	\immini[thm]{Cho điểm $A$ có hình chiếu $H$ trên mặt phẳng $(P)$. Lấy điểm $O$ thuộc mặt phẳng $(P)$, $O$ không trùng $H$. Khi đó góc giữa đường thẳng $A O$ và mặt phẳng $(P)$ bằng góc $AOH$}{
	\vspace*{-3mm}
...
\begin{vd}%[1K7BM-2]
	...
	\loigiai{\immini[thm]{Vì $S A$ vuông góc với hai đường thẳng $AB$ và $AC$ nên $S A \perp(A B C)$. Suy ra $S A \perp B C$.\\
...
\begin{bt}
	...
	\loigiai{
		\immini[thm]{Ta có $SA \perp (ABCD)\Rightarrow SA\perp BD$.\hfill $(1)$\\
...
\begin{bt}%[1C8B2-2]
	...
	\loigiai{
	\immini[thm]{Vì $ABCD$ là hình bình hành nên $OA=OC$. Ta có $OM$ là đường trung bình của tam giác $SAC$ nên $OM\parallel SA$. Mà $SA\perp (ABCD)$ nên $OM\perp (ABCD)$.}{\begin{tikzpicture}[scale=0.7, line join=round, line cap=round]
	\tkzDefPoints{0/0/A,-1.3/-1.6/B,2.5/-1.6/C}
	\coordinate (D) at ($(A)+(C)-(B)$);
	\coordinate (S) at ($(A)+(0,3)$);
	\tkzDrawPolygon(S,B,C,D)
...
\end{bt}
\begin{bt}%[1C8B2-2]
	\immini[thm]{Cho hình hộp $ABCD.A'B'C'D'$ có $AA'\perp (ABCD)$. Chứng minh rằng $AA'\perp (A'B'C'D')$.}{\begin{tikzpicture}[line cap=round,line join=round,font=\footnotesize,>=stealth,scale=.7]
	\coordinate[label=left:$A$] (A) at (0,0);
...
\begin{bt}
	...
	\loigiai{
	\immini[thm]{Ta có hai mặt phẳng $(ABC)$ và $(P)$ cùng vuông góc với $SA$ nên chúng song song với nhau.\\
...
\end{bt}
\begin{bt}
	\immini[thm]{Trên một sân phẳng nằm ngang, tại các điểm $A$, $B$, $C$, $D$ người ta dựng các cột thẳng đứng $AM$, $BN$, $CP$, $DQ$ và nối các sợi dây thẳng giữa $M$ và $P$, $N$ và $Q$ như hình bên.
		\begin{enumerate}
...
\begin{vd}%[1C8B3-1]
	...
	\loigiai{
	\begin{listEX}[1]
	\item \immini[thm]{Do $SA \perp(ABC)$ nên $\alpha=\widehat{SBA}$. Tam giác $SAB$ vuông tại $A$ nên
	$$
...
\begin{ex}%[1H3B3-2]
	...
	{$BD\perp (SAC)$}
	\loigiai{
	\immini[thm]{Từ giả thiết, ta có : $SA\perp (ABC)$.\\
...
\begin{ex}%[1H3B3-2]
	...
	\end{ex}
\begin{ex}%[1H3B3-2]
	\immini[thm]{Cho hình lập phương $ABCD.A'B'C'D'$. Khi đó góc giữa hai đường thẳng $BD$ và $A'C'$ bằng
	\choice
	{\True $90^{\circ}$}
	{$30^{\circ}$}
	{$60^{\circ}$}
...
\begin{ex}%[1H3B3-2]
	...
	{$AH\perp AC$}
	\loigiai{
	\immini[thm]{Vì $SA\perp (ABC)$ nên $BC\perp SA$, kết hợp với $BC\perp AC$ suy ra $BC\perp (SAC)$, do đó $BC\perp AH$.
	}{
...
\begin{ex}%[1H3B3-2]
	...
	\loigiai{
	\immini[thm]{Do $SA=SB=SC$ nên $H$ là tâm đường tròn ngoại tiếp tam giác $ABC$, suy ra $H$ là trung điểm của $BC$.
	}{
	\begin{tikzpicture}[xscale=1,yscale=0.85]
...
\begin{ex}%[1H3B3-2]
	...
	{$CD\perp ( ABC)$}
	\loigiai{
	\immini[thm]{Theo giả thiết thì $A$ và $B$ cách đều $C, D$ nên $A,B$ nằm trên mặt phẳng trung trực của $CD$. Vậy $AB\perp CD$.
	}{
	\begin{tikzpicture}
...
\begin{ex}%[1H3B3-2]
	...
	{$AC \perp ( SBD) $}
	\loigiai{
	\immini[thm]{Theo giả thiết $AC,BD, SO$ đôi một vuông góc, do đó $SO\perp (ABCD), BD \perp ( SAC), AC \perp ( SBD)$, do $AB$ có thể không vuông góc với $BC$ nên ``$AB \perp ( SBC)$'' là \textbf{sai}.
	}{
	\begin{tikzpicture}
...
\begin{ex}%[1H3B3-2]
	...
	{$OS\perp (ABCD)$}
	\loigiai{
	\immini[thm]{Vì $S.ABCD$ là hình chóp đều nên $OS\perp (ABCD)$ nên phương án D đúng.\\
...
\begin{ex}%[1H3K3-2]
	...
	\loigiai{
	\immini[thm]{Ta có $CC_1\perp (ABCD)$ nên $CC_1\perp BD$.\\
...
\begin{ex}%[1H3K3-2]
	...
	{\True $SC\perp AD$}
	\loigiai{
	\immini[thm]{Do $SA=SC$ nên $AC\perp SO$, mặt khác do $ABCD$ là hình thoi nên $AC\perp BD$. Từ đó nhận được $AC\perp (SBD)$. Hiển nhiên $AC\perp SB$.\\
...
\begin{ex}%[1H3K3-2]
	...
	{$BC'\perp A'D$}
	\loigiai{
	\immini[thm]{Do $ABCD$, $A'B'BA$, $BB'C'C$ là các hình thoi nên \\
