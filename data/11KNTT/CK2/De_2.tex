...
\begin{ex}%[Tex hóa đề CK2 - form 2025 - đợt 1 - Phạm Hoài]%[1H8N1-3]
	\immini[thm]{Cho hình chóp $S.ABCD$ có đáy $ABCD$ là hình bình hành (hình vẽ minh hoạ). Góc giữa hai đường thẳng $SD$ và $BC$ bằng
	\choice
...
\end{ex}
\begin{ex}%[Dự án Toán Từ Tâm K11-Đợt 3]%[Nguyễn Tuấn]%[1H8V7-4]
Cho lăng trụ đứng $ABC.A'B'C'$ có đáy $ABC$ là tam giác vuông cân tại $B$ và $AC=2a$, biết rằng $(A'BC)$ hợp với đáy $(ABC)$ một góc $45^\circ$. Với $a=1$, thể tích khối lăng trụ $ABC.A'B'C'$ bằng
\shortans{$\sqrt{2}$}
\loigiai{
\immini[thm]{
Tam giác $ABC$ vuông cân tại $B$, suy ra $AB=BC=\dfrac{AC}{\sqrt{2}}=a\sqrt{2}$.\\
Vì $AA'\perp BC$ và $AB\perp BC$ suy ra $\left((A'BC),(ABC)\right)=\widehat{ABA'}=45^\circ$.\\ Suy ra tam giác $ABA'$ vuông cân tại $A$, nên $AA'=AB=a\sqrt{2}$.\\
Vậy $V=\dfrac{1}{2}AB^3=\left(a\sqrt{2}\right)^3=a^3\sqrt{2}$.
}{
...
($(A')+(C)-(A)$)coordinate (C')
;
\draw (A)--(B)--(C)--(C')--(B')--(A')--cycle (C')--(A')--(B)--(B');
\draw[dashed] (A)--(C)--(A');
\foreach \p/\q in {A/180,B/-90,C/0,A'/180,B'/-45,C'/0}
\fill[black] (\p) circle (1.0pt)node[shift={(\q:3mm)}]{$\p$};
\node at ($(A)!0.5!(A')$)[left]{$a\sqrt{2}$};
\node at ($(A)!0.5!(B)$)[left]{$a\sqrt{2}$};
\node at ($(B)!0.5!(C)$)[right]{$a\sqrt{2}$};
\node at ($(A)!0.5!(C)$)[above]{$2a$};
...
\loigiai{
\immini[thm]{Gọi $O$ là giao điểm của $AC$ và $BD$. \\
