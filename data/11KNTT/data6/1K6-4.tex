...
\end{ex}
\begin{ex}
	\immini[thm]{Cho hàm số bậc ba $y=f(x)$ có đồ thị như hình vẽ. Có bao nhiêu giá trị nguyên của tham số $m$ thuộc đoạn $[0; 9]$ sao cho bất phương trình $2^{f^2(x)+f(x)-m}-16\cdot 2^{f^2(x)-f(x)-m}-4^{f(x)}+16<0$ có nghiệm $x\in (-1;1)$?
		\choice
		{$8$}
		{$5$}
		{\True $6$}
