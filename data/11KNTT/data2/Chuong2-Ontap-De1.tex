\def\tenchude{ÔN TẬP KIỂM TRA CHƯƠNG II}
\begin{name}
	{\tenchude}
	{TOÁN 11}
	{LỚP TOÁN THẦY PHÁT}
...
\begin{ex}%[Hoàng Thanh Phương]%[1D2H1-5]
	...
	\loigiai{
		$\forall n \in \mathbb{N^*}$, $u_n = \dfrac{n+1}{n+2}= 1-\dfrac{1}{n+2}$
		Suy ra
		$$u_{n+1}-u_n=\left(1-\dfrac{1}{n+3}\right) - \left(1-\dfrac{1}{n+2}\right)=\dfrac{1}{n+2} - \dfrac{1}{n+3}=\dfrac{1}{(n+2)(n+3)}>0,\forall n\in\mathbb{N^*}$$
		Suy ra $\{u_n\}$ là dãy số tăng.\\
		Vì $\dfrac{2}{3} \le 1-\dfrac{1}{n+2}<1$ với mọi $n\in\mathbb{N}^*$.\\
...
\begin{ex}%[Hoàng Thanh Phương]%[1D2V2-7]
	...
	\loigiai{
		\begin{itemchoice}
			\itemch Sau mỗi năm, giá trị của chiếc xe bằng $96\%$ giá trị của năm ngoái, vậy giá trị chiếc xe lập thành một cấp số nhân với $u_1=800$ và $q=0{,}96$. \\
...
\end{ex}
\begin{ex}%[1D3G3-6]
Ba cạnh của một tam giác vuông có độ dài là các số nguyên dương lập thành một CSC. Độ dài cạnh dài nhất bằng $10$. Tổng độ dài hai cạnh góc vuông bằng
\shortans{14}
\loigiai{
...
\begin{eqnarray*}
&&a+c=2b\\
&\Leftrightarrow& a^2+2ac+c^2=4(c^2-a^2)\\
&\Leftrightarrow&-3c^2+2ac+5a^2=0\\
&\Leftrightarrow& (c+a)(5a-3c)=0\\
&\Leftrightarrow& \hoac{& c=-a \\ &3c= 5a.}
\end{eqnarray*}
$\bullet ~ c= -a$ vô lí vì $a$, $c>0$.\\
$\bullet ~ 3c=5a$, vì $c=10$ nên $a=6$ và $b=8$.\\
Vậy $a+b=14$.
}
\end{ex}
\begin{ex}%[Hoàng Thanh Phương]%[1D2V2-7]
	Từ độ cao $55{,}8$ (mét) của tháp nghiên Pisa nước Italia người ta thả một quả bóng cao su chạm xuống đất. Giả sử mỗi lần chạm đất bóng lại nảy lên độ cao bằng $\dfrac{1}{5}$ độ cao mà bóng đạt trước đó. Quãng đường của bóng di chuyển từ lúc được thả rơi cho đến khi nó nảy lên và rơi xuống lần thứ năm gần bằng bao nhiêu (làm tròn đến phần chục)?
	\shortans{83,7}
	\loigiai{
	Đặt $u_1=55{,}8$ (mét) là quãng đường bóng rơi khi thả xuống, $u_{n+1}=\dfrac{u_1}{5^{n}}, n\ge 1$ là quãng đường bóng rơi sau lần nảy lên thứ $n$. \\
	Ta có $(u_n)$ là dãy cấp số nhân với $u_1=55{,}8$ và công bội $q=\dfrac{1}{5}$.\\
	Suy ra quãng đường của bóng di chuyển từ lúc được thả rơi cho đến khi nó nảy lên và rơi xuống lần thứ 5 là
	$$S= u_1+2u_2+2u_3+2u_4+2u_5+2u_6=2\cdot \dfrac{u_1(1-q^6)}{1-q}-u_1 \approx 83{,}7 \text{ (mét).}$$
	...
	Vậy tổng quãng đường hành trình của quả bóng xấp xỉ $83{,}7$ (mét).
	}
\end{ex}
...
\TL
\begin{ex}%[1K2G7-7]
	Tính tổng $S=1+2\cdot3+3\cdot3^2+\cdots+11.3^{10}$.
	\loigiai{
...
\end{ex}
\begin{ex}%[1K2G5-5]%[Trương Đăng Khoa]%Ví dụ 6
	Chị Hương vay góp một khoản tiền $100$ triệu đồng và đồng ý trả dần $5$ triệu đồng mỗi tháng với lãi suất $0{,}8\%$ số tiền còn lại của mỗi tháng.
	Gọi $A_n$, ($n\in\mathbb{N^*}$) là số tiền còn nợ (triệu đồng) của chị Hương sau $n$ tháng.
	\begin{enumerate}
		\item Xác định công thức tổng quát của dãy số $(A_n)$.
		\item Sau 20 tháng chị Hương có trả hết nợ không? Nếu chưa hết thì sau bao nhiêu tháng chị Hương sẽ trả hết nợ?
		\item Giả sử chị Hương dự kiến trả góp hết nợ trong vòng $1$ năm. Hỏi mỗi tháng chị Hương phải trả bao nhiêu tiền (làm tròn đến hàng nghìn đồng)?
	\end{enumerate}
	\loigiai{
...
\begin{itemize}
	\item Đầu kỳ thứ nhất là $A$.
	\item Cuối kỳ thứ nhất là $A(1+ d) - B$.
	\item Cuối kỳ thứ hai là : $[A( 1+ d) -B] \cdot ( 1+ d) - B= A \cdot (1+ d)^2 - B \cdot [(1+ d) + 1]$.
	\item Cuối kỳ thứ ba là : $[ A \cdot (1+ d)^2 - B \cdot ( 1+ d) + 1] \cdot (1+ d) - B= A \cdot (1+ d)^3 - B \cdot [ (1+ d)^2 +(1+ d)+ 1]$.
	\item $\ldots$
	\item Cuối kỳ thứ n là : $A \cdot (1+ d)^n - B \cdot [ (1+ d)^{n-1} +(1+ d)^{n-2}+ \cdots + (1+ d)+ 1] = A \cdot (1+ d)^n - B \cdot \dfrac{(1+ d)^n - 1}{d}$.
\end{itemize}
\begin{enumerate}
