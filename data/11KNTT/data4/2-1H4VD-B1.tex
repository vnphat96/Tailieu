...
\begin{vd}
	\immini[thm]{Cho tứ giác $ABCD$ sao cho các cạnh đối không song song với nhau. Lấy một điểm $S$ không thuộc mặt phẳng $\left(ABCD\right)$. Xác định giao tuyến của 
		\begin{itemize}
...
\begin{vd}
	\immini[thm]{Cho tứ diện $ABCD$. Lấy các điểm $M$ thuộc cạnh $AB$, $N$  thuộc cạnh $AC$ sao cho $MN$ cắt $BC$. Gọi $I$ là điểm bên trong tam giác $BCD$. Tìm giao tuyến của mặt phẳng $\left(MNI\right)$ với các mặt phẳng $\left(ABC\right)$, $\left(BCD\right)$, $\left(ABD\right)$, $\left(ACD\right)$.
	}{
...
\end{tikzpicture}}
	\loigiai{
		\begin{itemize}
			...
			\end{tikzpicture}}
			\immini[thm]{\item [c)] Tìm $(MNI) \cap (ABD)$.
				\begin{itemize}
...
\begin{vd}
	\immini[thm]{Cho hình chóp $S.ABCD$ đáy là hình bình hành tâm $O$. Gọi $M$, $N$, $P$ lần lượt là trung điểm của cạnh $BC$, $CD$, $SA$. Tìm giao tuyến của 
		\begin{tasks}(1)
			\task $(MNP)$ và $(SAB)$.
			\task $(MNP)$ và $(SBC)$.
			\task $(MNP)$ và $(SAD)$.
	...
	\loigiai{
		\begin{itemize}
			\immini[thm]{\item[a)] Tìm $(MNP)\cap (SAB)$.
				\begin{itemize}
				...
				\begin{tikzpicture}
					...
					\tkzLabelPoints[above right](P)
			\end{tikzpicture}	}
			\immini[thm]{\item[c)] Tìm $(MNP)\cap (SAD)$.
				\begin{itemize}
...
\begin{vd}
	...
	\end{tasks}
	\loigiai{
		\immini[thm]{a) Gọi $I$ là giao điểm của $MN$ và $AB$, khi đó ta có $\heva{&I\in MN\\&I\in AB}\Rightarrow \heva{&I\in (MNP)\\&I\in (ABCD)}.$\hfill (1)\\
...
\begin{vd}
	...
	\loigiai{
		\begin{enumerate}[\faPencilSquareO]
			\immini[thm]{	\item Tìm giao điểm của $CD$ với mp$(MNK)$.
				\begin{note}
...
\begin{bt}
	...
	\loigiai{
		\begin{enumerate}[a)]	
			...
			\immini[thm]{Trong mặt phẳng $(SBD)$, gọi $I=SO\cap NP$, có
