...
\loigiai
{\immini[thm]{ Ta có $\heva{& M \in (SAB)\cap (\alpha)\\ &
...
\loigiai
{\immini[thm]{Ta chứng minh được $(MNP)\parallel  (AB'D')$.\\ Ta có
...
\loigiai
{\immini[thm]{Do mặt phẳng $(P)$ đi qua $M$ và song song với $(SAD)$ nên cắt các mặt của hình chóp bằng các giao tuyến đi qua $M$ và song song với $(SAD)$. Vì $ABCD$ là hình thoi và tam giác $SAD$ đều nên thiết diện thu được là hình thang cân $MNEFE$ $(MN\parallel  EF$, $MF=EN)$.
Khi đó Ta có
$$
MN=a,
...
\loigiai
{\immini[thm]{Mặt phẳng $(HKM)$ và $(ABCD)$ chứa hai đường thẳng song song $HK$ và $AB$ nên giao tuyến củAChúng là $MN$ cũng song song với $HK$ và $AB$. Xét hai tam giác $HAM$ và $KBN$ có:
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Ta có $\heva{& BN\parallel DM\\ & DM\subset (DPM)} \Rightarrow BN\parallel (DPM).\quad (1) $\\
...
\immini[thm]{\item  Ta có $\heva{& M \in (\beta)\cap (SAB)\\ &SA\parallel (\beta)\\ &SA\subset (SAB)} \Rightarrow (\beta)\cap (SAB)=MF\parallel SA,F \in SB$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Do $O$, $M$ lần lượt là trung điểm của $AC$, $SA$ nên $OM$ là đường trung bình của tam giác $SAC$ ứng với cạnh $SC \Rightarrow OM\parallel SC$.\\
...
\loigiai
{\immini[thm]{Kẻ $FI \parallel SA,I \in AB \Rightarrow IF \parallel (SAD)$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item  Ta có $\heva{& BE\parallel AF\\ &AF\subset (ADF)} \Rightarrow EB\parallel (ADF)$.\\
...
\loigiai{
\begin{enumerate}
\immini[thm]{\item  Do $\heva{& (\alpha)\parallel (SAB)\\ &(ABCD)\cap (SAB)=AB\\
...
\loigiai
{\immini[thm]{{\bfseries Bổ đề.} Cho tam giác $ABC$ và các điểm $M$, $N$ thuộc các cạnh $AB$, $AC$ sao cho $MN \parallel  BC$. Gọi $E$, $F$ lần lượt là trung điểm của $BC$, $MN$ và $I=MB \cap CN$ thì $A$, $F$, $I$, $E$ thẳng hàng.\\
{\itshape Chứng minh.} 
...
\immini[thm]{Gọi $M=AB'\cap BA'$, $P=AC'\cap CA'$, $N=BC'\cap CB'$ và
$I=CM\cap AN$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item  Gọi $O$, $O'$ lần lượt là trọng tâm các mặt $ABCD$ và $A'B'C'D'$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Dễ thấy $PN\parallel CD'$ và $QM\parallel A'B$ mà $A'B\parallel C'D$ nên $PN\parallel QM$ hay $M$, $N$, $P$, $Q$ đồng phẳng.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item  Ta có $\heva{& (\alpha)\parallel (SAB)\\ &(ABCD)\cap (\alpha)=MN\\ &(ABCD)\cap (SAB)=AB} \Rightarrow MN\parallel AB$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Trong $\left(ABB'A'\right)$ gọi $J=MN\cap AB$, trong $(ABC)$ gọi $Q=JP\cap AC$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Gọi $(\alpha)$ là mặt phẳng đi qua $M$ và song song với $\left(A'D'CB\right)$ và $N'=(\alpha)\cap BD$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item  Gọi $O$, $M$, $E$, $F$ lần lượt là trung điểm của $AC'$, $AC$, $BC$, $B'C'$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Ta có $MA\parallel NN'.\quad (1)$\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Ta có $(ABC)$, $(DBC)$, $(\alpha)$ đôi một cắt nhau theo các giao tuyến là $BC$, $MN$, $PQ$ nên theo định lí về giao tuyến thì $BC$, $MN$, $PQ$ hoặc đồng quy hoặc đôi một song song.\\
...
\immini[thm]{\item  Dễ thấy $MNPQ$ là hình thang cân có $MQ=a-x$, $NP=a-y$.\\
...
\loigiai
{\begin{enumerate}
\immini[thm]{\item Ta có $\left(ABB'A'\right)\parallel \left(CDD'C'\right)$, 
