...
\begin{vd}
	...
	\loigiai{
		...
		{Gọi $M$ là trung điểm của $AD$.\\
			Ta có $\dfrac{BG}{BM}=\dfrac{BI}{BC}=\dfrac{2}{3}$.\\
			Suy ra $IG \parallel CM$.\\
			Vì $\heva{&IG \not\subset (ACD)\\&AM\subset (ACD)\\&IG\parallel AM}$ nên $IG \parallel (ACD)$.	}
		{\begin{tikzpicture}[scale=1, fill=black]
			\coordinate (A) at (1,4);
			\coordinate (B) at (-1,0);
			\coordinate (D) at (4,-1);
			...
			\coordinate (G) at ($(B)!0.666!(M)$);
			\coordinate (I) at ($(B)!0.666!(C)$);
			\tkzLabelPoints[above](A,G)
			\tkzLabelPoints[left](B)
			\tkzLabelPoints[below left](I)
			\tkzLabelPoints[right](D,M)
			\tkzLabelPoints[below](C)
			\tkzDrawSegments(A,B A,C A,D B,C C,D C,M)
			\tkzDrawSegments[dashed](B,D G,I B,M)
			\end{tikzpicture}}
	}
\end{vd}
\begin{vd}
	...
	\loigiai{
		\immini
		{a) Hai mặt phẳng $(SAB)$ và $(SCD)$ có một điểm chung là $S$, lần lượt chứa hai đường thẳng $AB$ và $CD$ song song với nhau.\\
			Suy ra giao tuyến của hai mặt phẳng là đường thẳng qua $S$ song song với $AB$ và $CD$.\\
			b) \begin{itemize}
				\item Vì $\dfrac{AM}{AD}=\dfrac{1}{3}$ nên $MN \parallel DC$.\\
				Vì $\heva{&MN \not\subset (SCD)\\&DC\subset (SCD)\\&MN\parallel DC}$ nên $MN \parallel (SCD)$.
				\item Gọi $J$ là trung điểm của $AC$. Ta có $\dfrac{NG}{NJ}=\dfrac{2}{3}\Rightarrow NG \parallel SJ$.\\
				Vì $\heva{&NG \not\subset (SAC)\\&SJ\subset (SAC)\\&NG\parallel SJ}$ nên $NG \parallel (SAC)$.
			\end{itemize}	}
		{\begin{tikzpicture}[scale=1, fill=black]
			\coordinate (S) at (1,5);
			\coordinate (A) at (0,0);
			...
			\coordinate (C) at (3,-2);
			\coordinate (D) at (5,0);
			\coordinate (M) at ($(A)!0.333!(D)$);
			\coordinate (I) at ($(A)!0.5!(B)$);
			\coordinate (J) at ($(A)!0.5!(C)$);
			\coordinate (G) at ($(S)!0.666!(I)$);
			\coordinate (N) at ($(B)!0.666!(N)$);
			\tkzLabelPoints[above](S)
			\tkzLabelPoints[left](A,B,G)
			\tkzLabelPoints[right](D,N)
			\tkzLabelPoints[below](C,M,J,I)
			\tkzDrawSegments(S,B S,C S,D B,C C,D)
			\tkzDrawSegments[dashed](S,A B,A D,A G,J M,N B,D G,N C,I A,C S,I)
		\end{tikzpicture}}
	}
\end{vd}
\begin{vd}
...
\begin{vd}
\immini[thm]{Cho tứ diện $ABCD$ có $G$ là trọng tâm $\triangle ABC$, $M\in CD$ với $MC = 2MD$. 
	\begin{listEX}[1]
...
\begin{vd}
\immini[thm]{Cho hình chóp $S.ABCD$ có đáy $ABCD$ là hình bình hành. Gọi $I$, $K$ lần lượt là trung điểm của $BC$ và  $CD$.
	\begin{enumerate}
...
\begin{vd}
	...
	\loigiai{
		\immini[thm]{
			...
			\begin{tikzpicture}[scale=0.8, line cap =round, line join=round, font = \small]
				\path
				(0,0)coordinate(A)
				(7,0)coordinate(D)
				(2,-2)coordinate(B)
				...
				($(K)+(A)-(C)$)coordinate(L)
				(intersection of A--D and K--L)coordinate(E)
				(intersection of C--D and K--L)coordinate(F);
				
				\draw (S)--(A)--(B)--(C)--(D)--cycle (C)--(S)--(B);
				\draw[dashed](C)--(A)--(D)--(B) (E)--(F) (K)--(I);
				\foreach \d/\g in {S/45,A/-135,B/-90,C/-45,D/0,I/135,K/90,E/135,F/-45} \draw (\d) node [shift={(\g:10pt)}] {$\d$};
		\end{tikzpicture}}
	}
\end{vd}

\begin{vd}
	Cho tứ diện $ABCD$. Gọi $M$, $I$ lần lượt là trung điểm của $BC$, $AC$. Mặt phẳng $(P)$ đi qua điểm $M$, song song với $BI$ và $SC$. Xác định trên hình vẽ các giao điểm $H, K, N$ của $(P)$ với các cạnh $AC$, $SA$, $SB$. Tứ giác $MNKH$ là hình gì?
	\loigiai{
		\immini
		{
...
\begin{vd}%[1H2K3-4]
\immini[thm]{Cho hình chóp $S.ABCD$. Gọi $M$, $N$ thuộc cạnh $AB$, $CD$. Gọi $(\alpha)$ là mặt phẳng qua $MN$ và song song với $SA$. Tìm giao tuyến của $(\alpha)$ với các mặt của hình chóp.
...
\begin{vd}
	\immini[thm]{Cho hình chóp $S.ABCD$ có đáy là hình bình hành, $O$ là giao điểm của $AC$ và $BD$, $M$ là trung điểm của $SA$. 
	\begin{enumerate}
	...
	\loigiai{
		\begin{enumerate}
			\item 
			\immini[thm]{Ta có $M, O$ là trung điểm của $SA$ và $AC$, suy ra $MO \parallel SC$.\\
