...
\begin{ex}%[1K4BB-2]
	...
	\loigiai{
		\immini[thm]{Vì $N$, $K$ lần lượt là lượt là trung điểm của $BD$ suy ra $NK\parallel BD$.\\
...
\begin{ex}%[1K4K0-5]
	...
	{$1$}
	\loigiai{
		\immini[thm]{Gọi $O$ là giao điểm của hai đường chéo $AC$ và $BD$.\\
...
\begin{ex}%[1K4YB-1]
	...
	\loigiai{
		\immini[thm]{Dựa vào hình vẽ ta có\\ $\heva{&B'C' \parallel BC\\ &BC \cap (BDD')=B}$ \\
...
\begin{ex}%[1K4BA-3]
	...
	{một tam giác}
	\loigiai{
		\immini[thm]{Gọi $N$ là giao của $SD$ và mặt phẳng $(MBC)$.\\
...
\begin{ex}%[1K4BA-3]
	...
	\loigiai{
		\immini[thm]{Ta có $\heva{&S\in (SAB)\cap (SCD)\\&AB\parallel CD\\&AB\subset (SAB), CD\subset (SCD)}$\\
...
\begin{ex}%[1K4BB-5]
	...
	{Tam giác đều}
	\loigiai{
		\immini[thm]{Gọi $N$, $P$, $Q$ lần lượt là trung điểm các cạnh $AC$, $CD$, $BD$.\\
...
\begin{ex}%[1K4BA-3]
	...
	{$MN$ và $SD$ cắt nhau}
	\loigiai{
		\immini[thm]{Hai mặt phẳng $(SAB)$ và $(MCD)$ lần lượt chứa hai đường thẳng song song $AB$, $CD$ và $MN$ là giao tuyến của chúng nên $MN\parallel CD.$}
		{
		\begin{tikzpicture}[>=stealth,line join=round,line cap=round,font=\footnotesize,scale=1]
				\tikzset{
					pics/hinhChopTuGiacHinhThang/.style  n args={5}{
...
\begin{ex}%[1K4BB-5]
	...
	{Hình thoi}
	\loigiai{
		\immini[thm]{Ta có $\heva{(&\alpha)\parallel AB\\
...
\begin{ex}%[1K4B0-4]
	...
	\loigiai{
		\immini[thm]{Ta có $ MG\subset (ADN) $ và $ (ADN)\cap (ABC)=AN $.\\
