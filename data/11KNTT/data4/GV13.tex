...
\end{center}
\begin{tc}
	\immini[thm]{Cho hai mặt phẳng song song, nếu một đường thẳng nằm trong mặt phẳng này thì đường thẳng đó sẽ song song với mặt phẳng kia.
	\begin{center}
	\fbox{$\heva{&
		(\alpha)\parallel(\beta) \\&
		d \subset(\alpha)
...
\end{dl}
\begin{dl}
\immini[thm]{Cho hai mặt phẳng song song, nếu một mặt phẳng cắt mặt phẳng này thì cũng cắt mặt phẳng kia và hai giao tuyến song song với nhau.
\begin{center}
...
\immini[thm]{Ta chứng minh mặt phẳng $(\alpha)$ có hai đường thẳng \textbf{CẮT NHAU} và lần lượt song song với mặt phẳng $(\beta)$.\\
...
\end{listEX} 
	\loigiai{
	\begin{listEX}
	\immini[thm]{\item Chứng minh $(OMN) \parallel (SCD)$.\\
...
\loigiai{ 
\immini[thm]{ Ta có: $M$, $N$ lần lượt là trung điểm của $SA$ và $AD$.\\
...
\end{enumEX}
\loigiai{
\immini[thm]{a) Chứng minh: $(OMN) \parallel (SCD)$.\\
...
\begin{listEX}
\item Chứng minh: $(OMN) \parallel (SBC)$.
\immini[thm]{Ta có: $M$, $O$ lần lượt là trung điểm của $SA$ và $AC$.\\
$\Rightarrow OM $ là đường trung bình của $\triangle SAC \Rightarrow OM \parallel SC$.\\
...
\loigiai{
\begin{listEX}
\immini[thm]{\item Chứng minh: $GM \parallel (SBC)$.\\
...
\loigiai{
\begin{listEX}
\immini[thm]{\item 	Chứng minh $(IMC) \parallel (SAB)$.\\
...
\immini[thm]{\item Chứng minh $(MNM') \parallel (DCC'D')$.\\
...
\begin{ex}%[Nguyễn Trần Phong]%[1K4BC-2]
	...
	{$(BCD) $}
	\loigiai{
		\immini[thm]{Ta có $\heva{ &B'D'\parallel BD\\&AB'\parallel DC'\\&B'D',AB'\subset (AB'D')\\ &DC',BD\subset (BDC')\\& B'D' \cap AB' = B'} \Rightarrow (AB'D')\parallel (BDC').$
		}{\begin{tikzpicture}[scale=0.6, font=\footnotesize, line join=round, line cap=round, >=stealth]
%\draw[color=gray,dash pattern=on 1pt off 1pt,xstep=1.0cm,ystep=1.0cm] (-5.2,-5.2) grid (5.2,5.2);
\coordinate (A) at (0,0);
\coordinate (D) at (5,0);
