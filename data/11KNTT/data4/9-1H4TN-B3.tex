...
\begin{ex}%[1H2Y3-2]
	...
	{$MN \parallel (SBC)$}
	\loigiai{
		\immini[thm]{Xét tam giác $SAC$ có $M, N$ lần lượt là trung điểm của $SA, SC$.\\ Suy ra $MN \parallel AC$ nên $MN \parallel (ABCD)$.}{\begin{tikzpicture}[scale=0.8, line join=round, line cap=round,thick]
...
\begin{ex}%[1H2Y3-3]
	...
	\loigiai{
		\immini[thm]{Do hai mặt phẳng $(SAD)$ và $(SBC)$ có chung điểm $S$ và có hai đường thẳng $AD$, $BC$ song song với nhau nên giao tuyến của hai mặt phẳng $(SAD)$ và $(SBC)$ là đường thẳng đi qua đỉnh $S$ và song song với đường thẳng $BC$.
		}{
			\begin{tikzpicture}[scale=0.3,>=stealth]
				\tkzDefPoints{0/0/A, 8/0/B,-3/-3/D, 2/6/S}
				\tkzDefPointBy[translation = from A to B](D)\tkzGetPoint{C}
