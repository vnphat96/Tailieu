...
\begin{tomtat}
	...
	% $h$ được gọi là số gia của biến số tại điểm $x_0$.
	\end{note}
...
\begin{vd}%[1D5B1]
	...
	\end{aligned}
	.$\\
	Vậy $f'(x)=\left(\dfrac{2}{x}\right)'=-\dfrac{2}{x^2}$ trên các khoảng $(-\infty ; 0)$ và $(0 ;+\infty)$.
	\item %b
	...
	\end{aligned}
	.$\\
	Vậy $f'(x)=\left(\sqrt{x}\right)'=\dfrac{1}{2\sqrt{x}}$ trên $(0 ;+\infty)$.	
	\item %c
	Với bất kì $x_0 \neq 1$, ta có:
	$\begin{aligned}[t]
	f'\left(x_0\right)
	...
	\end{aligned}
	.$\\
	Vậy $f'(x)=\left(\dfrac{1}{x-1}\right)'=-\dfrac{1}{(x-1)^2}$ trên các khoảng $(-\infty ; 1)$ và $(1 ;+\infty)$.
	\item %d
	Với bất kì $x_0 \neq 1$, ta có:
	$\begin{aligned}[t]
	f'\left(x_0\right)
	...
	\end{aligned}
	.$\\
	Vậy $f'(x)=\left(\sqrt{x-1}\right)'=\dfrac{1}{2\sqrt{x-1}}$ trên $(1 ;+\infty)$.
	\end{listEX}
	}
\end{vd}
% \begin{vd}%[1K9KU3]
...
\begin{bt}%[1C7Y1-2]
	...
	\loigiai{
	% Xét $h$ là số gia của biến số tại điểm $x_0=2$.\\
	Đăt $h=x-2$.\\
	Ta có: $f(2+h)-f(2)=\dfrac{1}{2+h}-\dfrac{1}{2}=\dfrac{2-(2+h)}{2(2+h)}=\dfrac{-h}{2(2+h)}$.\\
	% Suy ra: $\dfrac{\Delta y}{h}=\dfrac{-1}{2(2+h)}$.\\
	Ta thấy: $\lim\limits_{h\to 0}\dfrac{f(2+h)-f(2)}{h}=\lim\limits_{h\to 0}\dfrac{-1}{2(2+h)}=\dfrac{-1}{4}$.\\
	Vậy $f'(2)=\dfrac{-1}{4}$.
	}
\end{bt}
\begin{bt}%[1D5B1]
	...
	Đặt $h=x-2$. Ta có
	\begin{align*}
	& f(2+h)-f(2)=[-(2+h)^2+3(2+h)-2]-(-2^2+3\cdot 2-2)=-\Delta^2 x-h;\\
	% & \dfrac{\Delta y}{h}=-h-1;\\
	& \lim\limits_{h\to 0}\dfrac{f(2+h)-f(2)}{h}=\lim\limits_{h\to 0}(-h-1)=-1.
	\end{align*}
	Vậy $y'(2)=-1$.
	}
\end{bt}
\begin{bt}%[1D5B1]
	...
	Đặt $h=x-4$. Ta có
	\begin{align*}
	& f(4+h)-f(4)=\dfrac{1}{1+h}-1=\dfrac{-h}{1+h};\\
	% & \dfrac{f(4+h)-f(4)}{h}=\dfrac{-1}{1+h};\\
	& \lim\limits_{h\to 0}\dfrac{f(4+h)-f(4)}{h}=\lim\limits_{h\to 0}\dfrac{-1}{1+h}=-1.
	\end{align*}
	Vậy $f'(4)=-1$.
	}
\end{bt}
\begin{bt}%[1D5G1]
	...
	\loigiai{
	Đặt $h=x-\dfrac{\pi}{6}$. Ta có
	\begin{align*}
	& f\left(\dfrac{\pi}{6}+h\right)-f\left(\dfrac{\pi}{6}\right)=\sin\left(\dfrac{\pi}{2}+3h\right)-\sin\dfrac{\pi}{2}=\cos(3h)-1;\\
	& \dfrac{f\left(\dfrac{\pi}{6}+h\right)-f\left(\dfrac{\pi}{6}\right)}{h}=\dfrac{\cos(3h)-1}{h}=-\dfrac{2\sin^2\tfrac{3h}{2}}{h};\\
	& \lim\limits_{h\to 0}\dfrac{f\left(\dfrac{\pi}{6}+h\right)-f\left(\dfrac{\pi}{6}\right)}{h}=\lim\limits_{h\to 0}\dfrac{-2\sin^2\tfrac{3h}{2}}{h}=0.
	\end{align*}
...
\begin{bt}%[1T7B1-1]
	...
	$$
	f'(x_0)=\lim\limits_{x \to x_0} \dfrac{f(x)-f\left(x_0\right)}{x-x_0}=\lim\limits_{x \to x_0} \dfrac{C-C}{x-x_0}=\lim\limits_{x \to x_0} 0=0 .
	$$
	Vậy $f'(x)=(C)'=0$ trên $\mathbb{R}$.
	\item Với bất kì $x_0 \neq 0$, ta có:
	$$
	f'\left(x_0\right)=\lim\limits_{x \to x_0} \dfrac{\dfrac{1}{x}-\dfrac{1}{x_0}}{x-x_0}=\lim\limits_{x \to x_0} \dfrac{x_0-x}{x x_0\left(x-x_0\right)}=\lim\limits_{x \to x_0} \dfrac{-1}{x x_0}=-\dfrac{1}{x_0^2} .
	$$
	Vậy $f'(x)=\left(\dfrac{1}{x}\right)'=-\dfrac{1}{x^2}$ trên các khoảng $(-\infty ; 0)$ và $(0 ;+\infty)$.
	\item Với $x_0$ bất kì, ta có 
	$$f'\left(x_0\right)=\lim\limits_{x \to x_0} \dfrac{f(x)-f\left(x_0\right)}{x-x_0}=\lim\limits_{x \to x_0} \dfrac{x^2-x_0^2}{x-x_0}=\lim\limits_{x \to x_0}\left(x+x_0\right)=2 x_0.$$
	\item Với $x_0$ bất kì, ta có:
	...
	\item Với bất kì $x_0$, ta có:\\
	$$f'(x_0)=\lim\limits_{x \to x_0}\dfrac{x^3-{x_0}^3}{x-x_0}=\lim\limits_{x \to x_0}(x^2+xx_0+{x_0}^2)=3{x_0}^2.$$
	Vậy $f'(x)=\left(x^3\right)'=3x^2$ trên $\mathbb{R}$.
...
\begin{bt}%[1T7Y1-4]
	...
	Ta có:
	\begin{eqnarray*}
	s(2+h)-s(2)&=&4{,}9\cdot(2+h)^2-4{,}9\cdot2^2=4{,}9\cdot\left[(2+h)^2-2^2\right]\\
	&=&4{,}9\cdot \left[h\cdot(h+4)\right].
	\end{eqnarray*}
	Vận tốc tức thời tại thời điểm $t=2$.\\
	Suy ra, $s'(2)=\lim\limits_{h\to 0}\dfrac{s(2+h)-s(2)}{h}=\lim\limits_{h\to 0}\dfrac{4,9\cdot \left[h\cdot(h+4)\right]}{h}=\lim\limits_{h\to 0}4,9\cdot (h+4)=19,6.$}
\end{bt}
%===================
...
\begin{vd}%[1T7B1-3]
	...
	\loigiai{
	Ở bài trước ta tính được $f'(x)=-\dfrac{1}{x^2}$.
	\begin{itemize}
	\item Hệ số góc của tiếp tuyến $(C)$ tại $M(1 ; 1)$ là $k=f'(1)=-1.$
...
\begin{bt}%[1T7B1-3]
	...
	\loigiai{
	Ở bài trước ta tính được $f'(x)=-\dfrac{1}{(x-1)^2}$.
	\begin{itemize}
	\item Hệ số góc của tiếp tuyến $(C)$ tại $M(2 ; 1)$ là $k=f'(2)=-1.$
...
\loigiai{
	...
	Xét $h=x_0-1$. Ta có: 
	$$\begin{aligned}[t]
	f(1+h)-f(1)=&3(1+h)^3-1-2\\
	=&9h+9(h)^2+3(h)^3\\
	=&h[9+9h+3(h)^2].\\
	\end{aligned}$$
	% Suy ra: $\dfrac{\Delta y}{h}=9+9h+3(h)^2$.\\
	Suy ra $\lim\limits_{h\to 0}\dfrac{f(1+h)-f(1)}{h}=\lim\limits_{h\to 0}\left(9+9h+3(h)^2\right)=9$.\\
	Vậy $f'(1)=9$.
	\end{enumerate}
}
\end{bt}
...
\loigiai{
	...
	\item Với bất kì $x_0$, ta có\\
	$f'(x_0)=\lim\limits_{x\to x_0}\dfrac{f(x)-f(x_0)}{x-x_0}=\lim\limits_{x\to x_0}\dfrac{-x^2+{x_0}^2}{x-x_0}=\lim\limits_{x\to x_0}\left[-(x+x_0)\right]=-2x_0$.\\
	Vậy $f'(x)=-2x$ trên $\mathbb{R}$.
	\item Với bất kì $x_0$, ta có\\
	$f'(x_0)=\lim\limits_{x\to x_0}\dfrac{f(x)-f(x_0)}{x-x_0}=\lim\limits_{x\to x_0}\dfrac{\left[\left(x^3-2x\right)-\left({x_0}^3-2x_0\right)\right]}{x-x_0}$\\
	$=\lim\limits_{x\to x_0}\left[x^2+xx_0+{x_0}^2-2\right]=3{x_0}^2-2$.\\
	Vậy $f'(x)=3x^2-2$ trên $\mathbb{R}$.
	\item Với mọi $x_0\ne 0$, ta có\\
	$f'\left(x_0\right)=\lim\limits_{x \to x_0} \dfrac{\dfrac{4}{x}-\dfrac{4}{x_0}}{x-x_0}=\lim\limits_{x \to x_0} \dfrac{4(x_0-x)}{x x_0\left(x-x_0\right)}=\lim\limits_{x \to x_0} \dfrac{-4}{x x_0}=-\dfrac{4}{x_0^2}.$\\
	Vậy $f'(x)=\left(\dfrac{4}{x}\right)'=-\dfrac{4}{x^2}$ trên các khoảng $(-\infty ; 0)$ và $(0 ;+\infty)$.
	\end{enumerate}}
\end{bt}
\begin{bt}%[1T7B1-4]
...
\begin{bt}%[1T7T1-4]
	...
	% }
	\loigiai{Vận tốc rơi tức thời tại thời điểm $t=2$ là $v(2)=h'(2)=1{,}62\cdot2=3{,}24$ m/s.
	}
\end{bt}
\begin{bt}%[1K9KU3]
...
\begin{bt}%[1C7K1-2]
	...
	Đặt $h=x-x_0$.\\
	Ta có: $f(0+h)-f(0)=|h|$.\\
	Suy ra: $\dfrac{f(0+h)-f(0)}{h}=\dfrac{|h|}{h}$.\\
	Vì: $\lim\limits_{h\to 0^+}\dfrac{f(0+h)-f(0)}{h}=1$ và $\lim\limits_{h\to 0^-}\dfrac{f(0+h)-f(0)}{h}=-1$.\\
	...
	\begin{eqnarray*}
		f(x+h)-f(x)=&f(x+h)-f(x)\\
	=&|x+h|-|x|\\
	=&\sqrt{\left(x+h\right)^2}-\sqrt{x^2}\\
	=&\dfrac{\left(x+h\right)^2-x^2}{\sqrt{\left(x+h\right)^2}+\sqrt{x^2}}\\
	=&\dfrac{2xh+(h)^2}{\sqrt{\left(x+h\right)^2}+\sqrt{x^2}}.
	\end{eqnarray*}
	Suy ra: $\dfrac{f(x+h)-f(x)}{h}=\dfrac{2x+h}{\sqrt{\left(x+h\right)^2}+\sqrt{x^2}}$.\\
	Ta thấy: $\lim\limits_{h\to 0}\dfrac{f(x+h)-f(x)}{h}=\lim\limits_{h\to 0}\dfrac{2x+h}{\sqrt{\left(x+h\right)^2}+\sqrt{x^2}}=\dfrac{2x}{2\sqrt{x^2}}=\dfrac{x}{|x|}$.\\
	Vậy $f'(x)=\dfrac{x}{|x|}$ với $x\neq0$.
	\end{itemize}
	}
\end{bt}
\begin{bt}%[1K9KU4]
...
\begin{bt}%[1C7K1-2]
	...
	$$\begin{aligned}[t]
	C(Q+h)-C(Q)
	=&\left(Q+h\right)^2+80\left(Q+h\right)+3500-\left(Q^2+80Q+3500\right)\\
	=&2Q\cdot(h)+(h)^2+80h.
	\end{aligned}$$
	Suy ra: $\dfrac{\Delta y}{h}=2Q+h+80$.\\
	Ta thấy: $\lim\limits_{h\to 0}\left(2Q+h+80\right)=2Q+80$.\\
	Vậy $C'(Q)=2Q+80$.
	\item $C'(90)=2\cdot90+80=260$.\\
