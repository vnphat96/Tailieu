\begin{dang}{Tìm tập xác định của hàm số lượng giác}
	...
	\begin{itemize}
		...
		\begin{enumEX}{2}
			...
			\end{aligned}\right.$
			\item  $\tan x=0\Leftrightarrow x=k\pi$.
			\item  $\cot x=0\Leftrightarrow x=\dfrac{\pi}{2}+k\pi$.
		\end{enumEX}
	\end{itemize}
\end{dang}
\viduminhhoa
...
\begin{tasks}(2)
	...
	% \task $y=\sqrt{\dfrac{2-\cos x}{1-\sin x}}$.
...
\loigiai{
	\begin{enumerate}
		\item $y=\dfrac{\tan 2x}{\cos x+1}+\sin x$.
		...
		\item $y=\dfrac{\cos 3x}{1-\sin x}+\tan x$.
		Hàm số đã cho xác định khi và chỉ khi\\
		...
		x\neq -\dfrac{\pi}{2} +k2\pi$.\\
		Tập xác định $\mathscr{D}=\mathbb{R}\setminus\left\lbrace -\dfrac{\pi}{2} +k2\pi, \, k\in \mathbb{Z} \right\rbrace$.\\
		% \item $y=\sqrt{\dfrac{2-\cos x}{1-\sin x}}$.
		% Hàm số đã cho xác định khi và chỉ khi $\dfrac{2-\cos x}{1-\sin x}\geq 0$.\\
		% Với mọi $x\in \mathbb{R}$, ta có
