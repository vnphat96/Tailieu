...
% 		\node[below] at (0,-1.5) {\fbox{$\sin x=1 \Leftrightarrow x=\dfrac{\pi}{2}+k2\pi$}};
% 		\end{tikzpicture}
% 		\hspace{0.5cm}
% 		\begin{tikzpicture}[smooth,samples=300,scale=0.8,>=stealth]
% 		\draw[->] (-1.5,0)--(1.5,0) node[below]{\footnotesize$cos$};
...
% 		\node[below left] at (0,-1) {$B'$};
% 		\node[below] at (0,-1.5) {\fbox{$\cos x=0 \Leftrightarrow x=\dfrac{\pi}{2}+k\pi$}};
% 		\end{tikzpicture}
% 	\end{note}
...
% \begin{enumerate}[\faCheckSquareO]
% 	\item Trường hợp $a \in \left\{\pm \dfrac{\sqrt{3}}{3};\pm 1; \pm \sqrt{3}\right\}$. Ta bấm máy \fbox{\textbf{SHIFT}} \fbox{\textbf{tan}} \fbox{$\dfrac{1}{a}$} để đổi số $a$ về góc $\alpha$  hoặc $\beta^\circ$ tương ứng. Riêng $a=0$ thì $\alpha=\dfrac{\pi}{2}$
% 	\begin{note}
% 		\begin{minipage}[b]{9cm}
% 			\begin{listEX}[1]
% 				\item [\ding{172}] Công thức theo đơn vị rad:
...
\begin{vd}
	...
	\begin{tasks}(1)
		\task $\sin x= \pi$ \dotfill
		\task $\cos x= -2$ \dotfill
		\task $\sin 3x=-\dfrac{\sqrt{3}}{2}$ \dotfill
		\task $2\sin \left( \dfrac{\pi }{5}-x \right)=1$ \dotfill
		\task $2\sin \left( x-45^\circ \right)-1=0$ \dotfill
		\task $\cos \left( x-\dfrac{2\pi }{3} \right)=1$ \dotfill
		\task $\sqrt{2}\cos 2x-1=0$ \dotfill
		\task $3\cos x-1=0$. \dotfill
	\end{tasks}
	\loigiai{
		\begin{enumerate}[a)]
			\item $\sin x = \pi>1$ vô nghiệm.
			\item $\cos x = -2<-1$ vô nghiệm.
...
\begin{vd}
	Giải các phương trình sau:
	\begin{tasks}(1)
		\task $\tan 3x=-\dfrac{\sqrt{3}}{3}$ \dotfill
		\task $\sqrt{3}\tan \left( \dfrac{\pi }{6}-x \right)=1$ \dotfill
		\task $\tan \left( x-45^\circ \right)-1=0$\dotfill
		\task $\sin x -\sqrt{3} \cos x=0$ \dotfill
		\task $\sqrt{3}\cot x-1=0$ \dotfill
	\end{tasks}
	\loigiai{
		\begin{enumerate}[a)]
...
\begin{vd}
	...
	\begin{tasks}(1)
		\task $ \sqrt{3}\tan x-3=0 $ trên $ (0,3\pi) $.\dotfill
		\task $\sqrt{2}\sin (x - 1) =  - 1$ trên $\left( - \dfrac{{7\pi }}{2} , \dfrac{\pi }{2} \right) $.\dotfill
		\task $2\cos \left( 3x - \dfrac{\pi }{3}\right) -1 =0$ trên $ \left(-\pi, \pi\right) $.\dotfill
		\task $\tan (3x + 2) - \sqrt 3  = 0$ trên $ \left( -\dfrac{\pi}{2}, \dfrac{\pi}{2}\right) $.\dotfill
	\end{tasks}
	\loigiai{
		\begin{enumerate}[a)]
...
\begin{vd}
	Giải các phương trình sau:
	\begin{tasks}(1)
		\task $\sin 3x=\sin 2x$ \dotfill
		\task $\sin 2x-\sin x=0$ \dotfill
		\task $\sin 5x+\sin x=0$ \dotfill
		\task $\cos 2x-\cos x=0$ \dotfill
		\task $\cos 8x+\cos x=0$ \dotfill
		\task $\cos 4x-\sin x=0$ \dotfill
		\task $\sin 5 x+2 \cos ^{2} x=1$ \dotfill
	\end{tasks}
	\loigiai{
		\begin{enumerate}[a)]
			...
			\item \begin{eqnarray*}
				      ...
				      &\Leftrightarrow&
				      \cos 2x = -\sin 5x = \cos\left(\dfrac{\pi}{2}+5x \right) \\
				      &\Leftrightarrow&
...
\begin{vd}
	Giải các phương trình sau
	\begin{tasks}(1)
		\task $\cot 3x=\cot x$.\dotfill\\
		.\dotfill\\
		.\dotfill
		\task $\tan 5x-\tan x=0$.\dotfill\\
		.\dotfill\\
		.\dotfill
		\task $\tan\left(3x-20^\circ\right)-\cot\left(2x+15^\circ\right)=0$.\dotfill\\
		.\dotfill\\
		.\dotfill
		\task $\cot\left(x+30^\circ\right)=\cot\dfrac{x}{2}$ \dotfill\\
		.\dotfill\\
		.\dotfill
	\end{tasks}
	\loigiai{
		\begin{enumerate}
			...
			\item $\tan 5x-\tan x=0$.
			      ...
			      Kết hợp với điều kiện, phương trình đã cho có các nghiệm là $x=\dfrac{\pi}{4}+k\dfrac{\pi}{2}$ và $x=k\pi$, ($k\in \mathbb{Z}$).
			\item $\tan\left(3x-20^\circ\right)-\cot\left(2x+15^\circ\right)=0$
			      \begin{eqnarray*}
			      ...
			      \end{eqnarray*}
			\item $\cot\left(x+30^\circ\right)=\cot\dfrac{x}{2}$\\
...
\begin{vd}
	Giải các phương trình sau
	\begin{tasks}(1)
		\task $\cos^2x-\sin x\cos x=0$.\dotfill
		\task $\sin^22x+\cos^23x=1$.\dotfill
		\task $2\sin x+\cos x-\sin 2x-1=0$.\dotfill
		\task $\sin x+4 \cos x=2+\sin 2 x$.\dotfill
	\end{tasks}
	\loigiai{
		\begin{enumerate}
			\item $\cos^2x-\sin x\cos x=0$.
			      \begin{eqnarray*}
...
\begin{vd}% [1K1B4-7]
	Giải các phương trình sau
	\begin{listEX}[1]
		\item $2\cos^2x - 3\cos x + 1 = 0$. \dotfill
		\item $\sin^2x + 3\sin x + 2 = 0$.\dotfill
		\item $\tan^2x + (\sqrt{3}  - 1)\tan x - \sqrt{3}  = 0$.  \dotfill
...
\begin{vd}
	...
	$$
		d(t)=3\sin \left[\dfrac{\pi}{182}(t-80)\right]+12 \text { với } t \in \mathbb{Z} \text { và } 0<t \leq 365.
	$$
	\begin{tasks}
	...
	\loigiai{
		\begin{enumerate}
			\item 	 Vận tốc của con lắc đơn thứ nhất bằng $2$ (cm/s) khi và chỉ khi
			      \allowdisplaybreaks
			      \begin{eqnarray*}
				      -4 \cos \left(\dfrac{2 t}{3}+\dfrac{\pi}{4}\right)=2 &\Leftrightarrow& \cos \left(\dfrac{2 t}{3}+\dfrac{\pi}{4}\right)=-\dfrac{1}{2} \\
				      & \Leftrightarrow& \cos \left(\dfrac{2 t}{3}+\dfrac{\pi}{4}\right)=\cos \dfrac{2 \pi}{3} \\ & \Leftrightarrow&\hoac{&\dfrac{2 t}{3}+\dfrac{\pi}{4}=\dfrac{2 \pi}{3}+k 2 \pi \\& \dfrac{2 t}{3}+\dfrac{\pi}{4}=\dfrac{2 \pi}{3}+k 2 \pi}, k \in \mathbb{N}\\
				      & \Leftrightarrow&\hoac{&t=\dfrac{5 \pi}{8}+k 3 \pi \\& t=\dfrac{13 \pi}{8}+k 3 \pi}, k \in \mathbb{N}.
			      \end{eqnarray*}
			...
			\item
			      ...
			      \allowdisplaybreaks
			      \begin{eqnarray*}
				      -4 \cos \left(\dfrac{2 t}{3}+\dfrac{\pi}{4}\right)=2 \cdot 2 \sin \left(2 t+\dfrac{\pi}{6}\right)& \Leftrightarrow& \cos \left(\dfrac{2 t}{3}+\dfrac{\pi}{4}\right)=\sin \left(-2 t-\dfrac{\pi}{6}\right) \\
				      & \Leftrightarrow& \cos \left(\dfrac{2 t}{3}+\dfrac{\pi}{4}\right)=\cos \left(\dfrac{2 \pi}{3}+2 t\right) \\
				      & \Leftrightarrow &\hoac{&\dfrac{2 t}{3}+\dfrac{\pi}{4}=\dfrac{2 \pi}{3}+2 t+k 2 \pi \\& \dfrac{2 t}{3}+\dfrac{\pi}{4}=-\dfrac{2 \pi}{3}-2 t+k 2 \pi}, k \in \mathbb{Z} \\
				      & \Leftrightarrow&\hoac{&t=-\dfrac{5 \pi}{6}+k \dfrac{3 \pi}{2} \\& t=-\dfrac{11 \pi}{32}+k \dfrac{3 \pi}{4}}, k \in \mathbb{Z}.
			      \end{eqnarray*}
...
\begin{bt}%[1D1B2-1]
	Giải phương trình
	\begin{tasks}(1)
		\task $(2\cos x-1)(2\cos 2x+2\cos x+3)=3-4\sin^2x$.
		\task $\tan\left(2x+\dfrac{\pi}{6}\right)+\tan\left(\dfrac{\pi}{3}-x\right)=0$.
		\task $\dfrac{\sin 2x +2\cos x-\sin x-1}{\sqrt{3}+\tan x}=0$.
		\task $2\sqrt{3}\sin 5x\cos 3x=\sin 4x+2\sqrt{3}\sin 3x\cos 5x$.
	\end{tasks}
	\loigiai{
		\begin{enumerate}[a)]
			\item $(2\cos x-1)(2\cos 2x+2\cos x+3)=3-4\sin^2x$
			      $\begin{aligned}
			...
			\item $2\sqrt{3}\sin 5x\cos 3x=\sin 4x+2\sqrt{3}\sin 3x\cos 5x$.
...
\end{bt}
\begin{bt}
	Một quả đạn pháo được bắn ra khỏi nòng pháo với vận tốc ban đầu có độ lớn $v_0$ không đổi, nếu bỏ qua sức cản của không khí và coi quả đạn pháo được bắn ra từ mặt đất là một chất điểm thì quỹ đạo của quả đạn tuân theo phương trình $y=\dfrac{-g}{2 v_0^2 \cos ^2 \alpha} x^2+x \tan \alpha$, ở đó $g=9,8 \mathrm{~m} / \mathrm{s}^2$ là gia tốc trọng trường.\\
	Tìm góc bắn $\alpha$ để quả đạn pháo bay xa nhất, bỏ qua sức cản của không khí và coi quả đạn pháo được bắn ra từ mặt đất.
	\loigiai{
		\begin{itemize}
