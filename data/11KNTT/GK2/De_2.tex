...
\begin{ex}%[1H8H4-3]
Cho hình chóp $S.ABC$ có đáy là tam giác đều cạnh $a$. Hình chiếu vuông góc của $S$ lên $\left( ABC \right)$ là trung điểm của cạnh $BC$. Biết $\Delta SBC$ đều, góc giữa hai mặt phẳng $\left( SAB \right)$ và $\left( ABC \right)$ gần bằng
\choice
{\True $63^\circ $}
{$45^\circ $}
{$90^\circ $}
...
Tam giác $ABC$ đều nên $CI=\dfrac{a\sqrt{3}}{2}$, suy ra $HM=\dfrac12 CI=\dfrac{a\sqrt{3}}{4}$\\
}
{
...
\coordinate[label=below left:$H$] (H) at ($(A)!3/4!(B)$);
\coordinate[label=above :$S$] (S) at ($(M)+(90:3)$);
\draw (A)--(B)--(C)--(S)--cycle (S)--(B)  (H)--(S)--(M);
\draw[dashed] (M)--(A)--(C) (M)--(H) (C)--(I);
\fill (A)circle(2pt) (B)circle(2pt) (C)circle(2pt) (S)circle(2pt) (H)circle(2pt) (M)circle(2pt);
\end{tikzpicture}
}

Tam giác $SHM$ vuông tại $M$ có $\tan \widehat{SHM}=\dfrac{SM}{HM}=2$.\\
$\Rightarrow \widehat{SHM}\approx 63^\circ $ nên góc giữa hai mặt phẳng $\left( SAB \right)$ và $\left( ABC \right)$ xấp xỉ $63^\circ $.
}
\end{ex}
...
\TL
\begin{ex}%[Dự án 17 - CK2 NH23-24 - TeamTeXHoa - Nguyễn Sơn Thành]%[1D6H2-2]
Cho  $\log_25=a$,  $\log_47=b$  và  $\log_23=c$. Biểu diễn $\log_{24}175$ theo $a$, $b$, $c$.
% \shortans[0]{$12$}
\loigiai{
Ta có $\log_47=b \Rightarrow \log_27=2b$.\\ 
$\log_{24}175=\dfrac{\log_2175}{\log_224}=\dfrac{\log_2\left(5^2\cdot7\right)}{\log_2\left(2^3\cdot3\right)}=\dfrac{2\log_25+\log_27}{3+\log_23}=\dfrac{2a +2b}{3+c}$
}
\end{ex}
...
\begin{ex}%[1H8V2-2]
Cho tứ diện $ABCD$ có $AD\perp (ABC)$ với $\triangle ABC$ là tam giác đều cạnh bằng $2a$ và $AD=\alpha\sqrt{3}$. Gọi $M$ là trung điểm của $BC$. Điểm $I$ thuộc $AM$ thỏa $\dfrac{AI}{MI}=\dfrac{1}{2}$, $G$ là trọng tâm tam giác $BCD$. Chứng minh rằng $IG\perp (BCD)$.
\loigiai{
\begin{center}
\begin{tikzpicture}
\def\a{6} %Khai báo cạnh
\def\h{4}
\path 	(0:0) coordinate (A)
++(0:\a) coordinate (B)
++(-150:3*\a/5) coordinate (C)
($(A)+(90:\h)$) coordinate (D)
($(C)!0.5!(B)$) coordinate (M)
($(A)!1/3!(M)$) coordinate (I)
($(D)!2/3!(M)$) coordinate (G)
($(D)!0.5!(M)$) coordinate (H);
\draw	(A)--(C)--(B)
(A)--(D)	(B)--(D)	(C)--(D)--(M);
\draw[dashed] 	(A)--(B) (A)--(M) (I)--(G) (A)--(H);
\foreach \x /\goc in {A/180,B/0,C/-135,D/90,M/-30,I/-120,G/60,H/60}
\fill[black] (\x) circle (1.5pt)
($(\x)+(\goc:3mm)$) node {$\x$};
\draw pic[draw,angle radius=2mm]{right angle=B--A--D};%Theo chiều dương
\end{tikzpicture}
\end{center}
Ta có $\heva{&AD\perp (ABC)\\&BC\subset (ABC)}\Rightarrow AD\perp BC$.\\
Và $\heva{&\triangle ABC~\text{đều}\\&AM~\text{là đường trung tuyến}}\Rightarrow AM\perp BC$.\\
Khi đó $\heva{&BC\perp AD\\&BC\perp AM\\&AM\cap AD=A}\Rightarrow BC\perp (ADM)$.\\
Trong $(ADM)$, kẻ $AH\perp DM~(1)$.\\
Ta có $\heva{&BC\perp (ADM)\\&AH\subset (ADM)}\Rightarrow AH\perp BC~(2)$.\\
Từ $(1)$ và $(2)$ suy ra $AH\perp (BCD)~(3)$.\\
Vì $\triangle ABC$ đều nên $AM=a\sqrt{3}=AD$. Suy ra tam giác $ADM$ cân tại $M$.\\
Mặt khác, $AH\perp DM$ nên $H$ là trung điểm của $DM$.\\
Vì $G$ là trọng tâm tam giác $BCD$ nên $\dfrac{GM}{AM}=\dfrac{1}{3}\Rightarrow \dfrac{GM}{HM}=\dfrac{2}{3}$.\\
Trong tam giác $AHM$ có $\dfrac{GM}{AM}=\dfrac{IM}{AM}=\dfrac{2}{3}\Rightarrow IG\parallel AH~(4)$.\\
Từ $(3)$ và $(4)$ ta suy ra được $IG\perp (BCD)$.
}
\end{ex}
