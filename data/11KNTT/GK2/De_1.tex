...
\begin{ex}%[1H8H2-2]
	\immini[thm]{Cho hình chóp $S. ABCD$ có $SA\perp AB$, $SA\perp AC$. Khẳng định nào dưới đây đúng?
		\choice
		{$SA\perp(SBC)$}
		{$SA\perp(SCD)$}
		{\True $SA\perp(ABCD)$}
...
\begin{ex}%[1H8N6-2]%[Dự án đề kiểm tra Toán 11 HK2 NH23-24- Nguyễn Hữu Duy]%[THPT Chuyên Hùng vương - Phú Thọ]
	...
	\loigiai{
		\immini[thm]{Vì $AC \perp BD$, $SA \perp BD$ nên $BD \perp (SAC)$. Vậy $AC$ và $SO$ vuông góc với $BD$. Suy ra $\widehat{SOC}$ là một góc phẳng của góc nhị diện $[S,BD,C]$.
		}
		{
			\begin{tikzpicture}[scale=0.7, font=\footnotesize,line join=round, line cap=round, >=stealth]
				\path
...
\begin{ex}%[BG - 10-11 New - 4in1, Phan Anh]%[1H8H4-2]
	\immini[thm]{Cho hình chóp đều $S.ABCD$ (minh họa như hình bên). Khẳng định nào sau đây đúng?
		\choice
		{\True $(SBD) \perp(ABCD)$}
		{$(SAB) \perp(ABCD)$}
		{$(SAD) \perp(ABCD)$}
...
\begin{ex}%[Chuẩn hóa M25, Nguyễn Văn Nay]%[1H8H6-1]
	\immini[thm]{Cho hình chóp $S.ABCD$ có đáy $ABCD$ là hình vuông tâm $O$ cạnh $2a$. Biết cạnh bên $SA$ vuông góc với đáy và $SA=a\sqrt{6}$.
		\choiceTF[t]
		{\True $BC \perp (SAB)$}
...
\TL
\begin{ex}%[1D6H2-1]
Cho số thực $a$ thõa mãn $0<a \neq 1$. Tính giá trị của biểu thức $T=\log _a\left(\dfrac{a^2 \cdot \sqrt[3]a^2 \cdot \sqrt[5]a^4}{\sqrt[15]a^7}\right)$.
% \shortans{$3$}
\loigiai{
Ta có $T=\log _a\left(\dfrac{a^2 \cdot \sqrt[3]a^2 \cdot \sqrt[5]a^4}{\sqrt[15]a^7}\right)=\log _a \dfrac{a^{2+\frac{2}{3}+\frac{4}{5}}}{a^{\frac{7}{15}}}=\log _a a^{2+\frac{2}{3}+\frac{4}{5}-\frac{7}{15}}=\log _a a^3=3$.}
\end{ex}
...
\begin{ex}%[1D6H4-6]
	...
	\loigiai{
		...
		\[
			\begin{aligned}
				5 \cdot 10^{-13}=1,2 \cdot 10^{-12} \cdot\left(\frac{1}{2}\right)^{\frac{t}{T}}
				 & \Leftrightarrow\left(\frac{1}{2}\right)^{\frac{t}{T}}=\frac{5}{12} \Leftrightarrow \frac{t}{T}=\log _{\frac{1}{2}} \frac{5}{12} \\
...
\begin{ex}%[CTST, Mức độ 3]giảng K10-11, Nguyễn Đắc Giáp]%[1H8V2-2]
