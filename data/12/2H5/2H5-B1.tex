...
\subsubsection{Vectơ pháp tuyến của mặt phẳng}
	\immini{
		\khung{\textbf{Vectơ pháp tuyến} của mặt phẳng $(P)$ là những vectơ khác $\vec{0}$ và có giá \text{vuông góc} với $(P)$.}
		\iconMT \indam{Chú ý:} Nếu $\vec{n}$ và $\vec{n'}$ cùng là vectơ pháp tuyến của $(P)$ thì $\vec{n'} = k \cdot \vec{n}$ (tọa độ tỉ lệ nhau).
	}{
...
\immini{\khung{Hai vectơ $\vec u$, $\vec v$ được gọi là \textbf{cặp vectơ chỉ phương} của mặt phẳng $(P)$ nếu chúng không cùng phương và có giá nằm trong hoặc song song với mặt phẳng $(P)$.}
	\iconMT \indam{Chú ý:}  Nếu $\vec u$, $\vec v$ là cặp vectơ chỉ phương của $(P)$ thì $\vec{n}=[\vec u,\vec v]$ là một vectơ pháp tuyến của $(P)$.}{
...
\khung{Mặt phẳng $(P)$ đi qua điểm $M(x_0;y_0;z_0)$ và nhận $\vec{n}=(a;b;c)$ làm vectơ pháp tuyến có phương trình là 
...
Gọi $\vec{n_1}=(a_1;b_1;c_1)$, $\vec{n_2}=(a_2;b_2;c_2)$ lần lượt là vectơ pháp tuyến của $(P)$ và $(Q)$.
		\khung{
			\begin{itemize}
			\item Nếu $\heva{&\vec{n_1}= k \cdot \vec{n_2}\\& d_1 =k\cdot d_2}$ thì $(P)$ trùng $(Q)$.
			\item Nếu $\heva{&\vec{n_1}= k \cdot \vec{n_2}\\& d_1 \ne k\cdot d_2}$ thì $(P)$ song song $(Q)$.
			\item Nếu $\vec{n_1}$ không cùng phương với $\vec{n_2}$ thì $(P)$ cắt $(Q)$.
			\item Nếu $\vec{n_1} \perp \vec{n_2}$ hay $a_1a_2+b_1b_2+c_1c_2=0$ thì $(P) \perp (Q)$.
		\end{itemize}
		}

\subsubsection{Khoảng cách từ một điểm đến mặt phẳng}
Cho điểm $M(x_0;y_0;z_0)$ và mặt phẳng $(P) \colon ax+by+cz+d=0$.
		\immini{\boxmini{$\mathrm{d}\left(M,(P) \right)=\dfrac{\bigg|ax_0+by_0+cz_0+d\bigg|}{\sqrt{a^2+b^2+c^2}}$}
	}{
...
\begin{dang}{Xác định vectơ pháp tuyến và điểm thuộc mặt phẳng}
	Cho mặt phẳng $(\alpha)$.
	\begin{itemize}
		\item  [\ding{172}] Nếu vectơ $\overrightarrow{n}$ khác $\overrightarrow{0}$ và có giá vuông góc với $(\alpha)$ thì $\overrightarrow{n}$ được gọi là vectơ pháp tuyến của $(\alpha)$.
		\item  [\ding{173}] Nếu hai vectơ $\overrightarrow{a}, \overrightarrow{b}$ không cùng phương, có giá song song hoặc nằm trong $(\alpha)$ thì $\overrightarrow{a}, \overrightarrow{b}$ được gọi là cặp vectơ chỉ phương của $(\alpha)$. Khi đó, nếu $\vec{a}=(a_1;a_2;a_3)$, $\vec{b}=(b_1;b_2;b_3)$ thì
...
\begin{dang}{Lập phương trình mặt phẳng khi biết các yếu tố liên quan}
	\begin{itemize}
		\item [\iconCV] \indam{Công thức:} Cho $(P)$ qua điểm $M(x_0,y_0,z_0)$ và một vectơ pháp tuyến $\overrightarrow{n_P}=(a,b,c)$. Khi đó, phương trình $(P)$ là
		\begin{align*}
			\boxed{(P):a(x-x_0)+b(y-y_0)+c(z-z_0)=0}
		\end{align*}
		\item [\iconCV] \indam{Một số cách xác định vectơ pháp tuyến thường gặp:}
		\begin{listEX}[1]
		...
		\end{listEX}
	\item [\iconCV] \indam{Phương trình theo đoạn chắn:}
...
\begin{vd}%[Đề tập huấn, Sở GD - ĐT tỉnh Quảng Bình, 2019]%[Nguyễn Tiến, dự án 12EX5]%[2H3K2-3]%
	...
	\loigiai{
		$\overrightarrow{AB}=(3;-3;4)$, $\overrightarrow{n}_{(P)}=(1;-3;2)\Rightarrow\left[\overrightarrow{AB},\overrightarrow{n}_{(P)}\right]=(6;-2;-6)=2(3;-1;-3)$.\\
