...
\immini[thm]{Hình phẳng $(H)$ được giới hạn bởi đồ thị hàm số bậc ba và trục hoành được chia thành hai phần có diện tích lần lượt là $S_1$ và $S_2$ (như hình vẽ).\\ 
...
\begin{ex}%[Vovanle]%[2D4H3-1]
\immini[thm]{Diện tích phần hình phẳng phần gạch sọc trong hình vẽ được tính theo công thức nào dưới đây? 
	\choice
...
Cho $(H)$ là hình phẳng giới hạn bởi các đường $y=\sqrt{x}$, $y=x-2$ và trục hoành. Biết diện tích của $(H)$ bằng $\dfrac{a}{b}$. Tính giá trị biểu thức $T=a+b$.
	...
	{$T=19$}
	\loigiai{
\immini[thm]{Diện tích của $(H)$ bằng 
...
\setcounter{ex}{0}
\begin{ex}%[Vovanle]%[2D4H3-1]
\immini[thm]{Cho đồ thị hàm số $y=\left(\dfrac{1}{2}\right)^x$, $y=x+1$ và hình phẳng được gạch sọc như hình vẽ.
}{
\begin{tikzpicture}[line join=round,line cap=round, font=\footnotesize,scale=0.75,>=stealth]
	\draw[-stealth](-0.5,0)--(4.6,0)node[below]{$x$};
