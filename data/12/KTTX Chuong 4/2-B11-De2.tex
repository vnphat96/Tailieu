...
\begin{ex}%[2D4V1-4]
	...
	\choiceTF
{$F(x)=\dfrac{3^{2x}\ln3}{2}$  là một nguyên hàm của hàm số $f(x)$ trên $\mathbb{R}$}
...
{\True $\displaystyle\int [f(x)+g(x)^2]\mathrm{d}x =\dfrac{9^x}{2\ln3}-x+\tan x+C$, ($C$ là một hằng số)}
\loigiai{
	\begin{itemchoice}
