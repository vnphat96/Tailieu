...
\begin{vd}%[2D1K3-6]
	\immini[thm]{Một người muốn xây một cái bể chứa nước, dạng một khối hộp chữ nhật không nắp có thể tích
...
\begin{vd}%[2D1T3-2]
	\immini[thm]{Một nhà sản xuất cần làm ra những chiếc bình có dạng hình trụ với dung tích $1000\mathrm{~cm}^3$. Mặt trên và mặt dưới của bình được làm bằng vật liệu có giá 1,2 nghìn đồng$/\mathrm{cm}^2$, trong khi mặt bên của bình được làm bằng vật liệu có giá $0{,}75$ nghìn đồng$/\mathrm{cm}^2$. Tìm các kích thước của bình để chi phí vật liệu sản xuất mỗi chiếc bình là nhỏ nhất.}{\hspace{1cm}
...
\end{vd}
\begin{vd}
	Một công ty tiến hành khai thác 12 giếng dầu trong khu vực được chỉ định. Trung bình mỗi giếng dầu chiết xuất được 133 thùng dầu mỗi ngày. Công ty có thể khai thác nhiều hơn 12 giếng dầu nhưng cứ khai thác thêm một giếng thì lượng dầu mỗi giếng chiết xuất được hằng ngày sẽ giảm 4 thùng. Để công ty có thể quyết định số giếng cần thêm cho phù hợp với tài chính, hãy chỉ ra số giếng công ty có thể khai thác thêm để sản lượng dầu chiết xuất đạt lớn nhất.
	\loigiai{ 
		Gọi ${x}$ là số giếng dầu khai thác thêm.
		
		Sản lượng dầu khi khai thác thêm ${x}$ giếng là: $(12+x)(133-4x)$ (thùng).
		
		Xét hàm số $f(x)=(12+x)(133-4x)=- 4 x^{2} + 85 x + 1596$ mô tả sản lượng thùng dầu.
		
		Ta có: $f'(x)=0\Leftrightarrow 85 - 8 x=0 \Leftrightarrow x=\frac{85}{8}$.
		...
		\begin{center}
			\begin{tikzpicture}
				...
				{$0$,$\frac{85}{8}$,$+\infty$}
				\tkzTabLine{,+,0,-}
				\tkzTabVar{-/,+/$\frac{32761}{16}$,-/}
			\end{tikzpicture} 
		\end{center}
		Ta có: $10<\frac{85}{8}<11$ và $f(10)=2046<f(11)=2047$.
		
		Do đó công ty có thể khai thác thêm 11 giếng dầu.
...
\begin{ex}
	...
	{\True $\dfrac{25}{4}\,\text{cm}^2$}
	{$125 \text{cm}^2$}
	\loigiai{Gọi một cạnh góc vuông là $x$ ($0<x<5$) thì cạnh góc vuông còn lại là $\sqrt{25-x^2}$.\\ Như vậy, diện tích tam giác là $S=\dfrac{x\cdot\sqrt{25-x^2}}{2}=\dfrac{1}{2}\sqrt{25x^2-x^4}$.
		Đặt $f(x)=25x^2-x^4$. 
		...
		Vậy tam giác vuông có cạnh huyền bằng $5 \mathrm{~cm}$ có thể có diện tích lớn nhất bằng $\dfrac{25}{4}$ cm$^2$.}
\end{ex}
...
\begin{ex}%[2H1K3-6]
	...
	\loigiai{
		\immini[thm]{
			Gọi $x$ là chiều rộng của bể cá. Suy ra chiều dài là $2x$ và diện tích đáy là $2x^2$.\\
			Vì tổng diện tích các mặt bên và đáy của bể cá là $5,5\,\mathrm{m^2}$ nên diện tích các mặt bên là $5,5-2x^2$.\\
			Chiều cao của bể cá là $h=\dfrac{5,5-2x^2}{6x}$.\\
			Thể tích của bể cá là $V=2x \cdot x \cdot h= 2x^2 \cdot \dfrac{5,5-2x^2}{6x}=-\dfrac{2}{3}x^3+\dfrac{11}{6}x$.\\			
			}{
			\begin{tikzpicture}
				\def\tls{.4}
				\path
				...
				;
		\end{tikzpicture}}
		\noindent Khảo sát hàm số $V(x)=-\dfrac{2}{3}{x^3}+\dfrac{11}{6}x$ trên khoảng $\left( 0;\dfrac{\sqrt{11}}{2} \right) $
			\begin{itemize}
...
\begin{ex}%[2D1T3-6]
	...
	\loigiai{
		...
		\immini[thm]{Với giả thiết của bài toán, thể tích bể cá là $$V=2x^2y=\dfrac {500}{3}\Rightarrow y=\dfrac {250}{3x^2}.$$
...
\Opensolutionfile{ans}[ans/2D1-B2-d2-3]
...
\begin{ex}
	...
	\loigiai
	{
		\begin{enumerate}[$\bullet$]
			...
			\begin{center}
				\begin{tikzpicture}[scale=1]
					\tkzTabInit[nocadre=false, lgt=1.2, espcl=2.5, deltacl=0.6]{$x$/1.2, $G(x)$/2}{$0$, $\dfrac{150-70\sqrt{3}}{3}$, $50$, $\dfrac{150+70\sqrt{3}}{3}$, $120$, $140$}
					\tkzTabVar{+/, -/, R, +/, R, -/}
				\end{tikzpicture}
			\end{center}
...
\begin{ex}%[Đề Ôn Tập số 2, phát triển theo ĐMH TNPT 2024-2025 ]%[2PhatTrien-DMH-2024-2025, GV: Nguyễn Cường]%[2D1V3-6]
	...
	\shortans[oly]{$210$}
	\loigiai{
		Gọi $x$ là số lần giảm giảm giá vé, với $0\le x\le 26$.\\
		Lợi nhuận đạt được của rạp chiếu phim là
		\allowdisplaybreaks
		\begin{eqnarray*}
			R(x)&=&(1\,000+50x)(260-10x)+(1\,000+50x)\cdot 40\\
			&=&260\,000-10\,000x+13\,000x-500x^2+40\,000+2\,000x\\
			&=&-500x^2+5\,000x+300\,000.
		\end{eqnarray*}
		Ta có $R'(x)=-1\,000x+5\,000$ và $R'(x)=0\Leftrightarrow x=5$.
		\begin{center}
			\begin{tikzpicture}
				\tkzTabInit[nocadre=true,lgt=1.4,espcl=4,deltacl=0.6]
				{$x$ /1, $R'(x)$ /0.7, $R(x)$ /2}
				{$0$,$5$,$26$}
				\tkzTabLine{,+,$0$,-,}
				\tkzTabVar{-/$R(0)$,+/$R(5)$,-/$R(26)$}
			\end{tikzpicture}
		\end{center}
		Vậy lợi nhuận lớn nhất là $R(5)=312\,500$ (nghìn đồng) khi giá vé là $260-50=210$ (nghìn đồng).
	}
\end{ex}
\dongcham{10}
\begin{ex}
	\immini[thm]{Vào lúc 12 giờ 7 phút anh Hùng chạy xe xuất phát từ điểm $A$ và đi đến điểm $C$ với vận tốc là $60(\mathrm{km} / \mathrm{h})$. Biết rằng 7 phút trước đó anh Quang chạy xe xuất phát từ điểm $B$ với vận tốc là $30(\mathrm{km} / \mathrm{h})$ sao cho hai quãng đường của hai xe hợp nhau một góc $60^{\circ}$ như hình vẽ dưới đây. Cho biết $A B=42(\mathrm{km})$ và vào lúc $a$ giờ $b$ phút thì anh Quang và anh Hùng ở vị trí có cự li gần nhất để vẫy tay chào nhau $\left(a, b \in \mathbb{N}^*\right)$. Tính $a+b$ ? Xem như hai xe đều chuyển động đều và không có tác dụng ngoại lực hay yếu tố nào khác.\\
		\shortans[oly]{$41$}
	}{
		\begin{tikzpicture}[scale=1, font=\footnotesize,>=stealth]
			...
			;
			\draw (B)--(A)--(C);
			\draw pic["$60^\circ$",draw,angle eccentricity=1.6,angle radius=0.5cm]{angle=C--A--B};
			\foreach \x/\g in {B/90,A/-90,C/0}\draw[fill=black] (\x) circle (.05) +(\g:.4)node{\footnotesize$\x$};
	\end{tikzpicture}}
	\loigiai{
...
\begin{ex}%[2D1V3-6]
	\immini[thm]{Hai nhà máy được đặt tại các vị trí $A$ và $B$ cách nhau $4$ km. Nhà máy xử lí nước thải được đặt ở vị trí $C$ trên đường trung trực của đoạn thẳng $A B$, cách trung điểm $M$ của đoạn thẳng $A B$ một khoảng là $3$ km. Người ta muốn làm đường ống dẫn nước thải từ hai nhà máy $A, B$ đến nhà máy xử lí nước thải $C$ gồm các đoạn thẳng $A I$, $B I$ và $I C$, với $I$ là vị trí nằm giữa $M$ và $C$ (Hình bên). Cần chọn vị trí điểm $I$ cách điểm $M$ bao nhiêu km để tổng độ dài đường ống nhỏ nhất? (\textit{kết quả làm tròn đến hàng phần trăm})\\
		\shortans[oly]{$1{,}15$}	
			...
			($(M)!3/2!90:(B)$) coordinate (C)
			($(C)!2/3!(M)$) coordinate(I)
			($(M)+(-1,0)$) coordinate (u)
			($(C)+(-1,0)$) coordinate (v)
			($(u)!1/2!(v)$) coordinate (u')
			($(A)+(0,-1)$) coordinate (x)
			($(B)+(0,-1)$) coordinate (y)
			($(x)!1/2!(y)$) coordinate (x')
			;
			\draw(C)--(I)--(A) (I)--(B);
			\draw[dashed](I)--(M) (A)--(B);
			\draw[dashed](C)to [bend left=-20] (M);
			\draw[dashed](A)to [bend left=-20] (B);
			\foreach \x/\y in {A/180,B/0,C/90,I/20,M/28}
			\draw[fill=black] (\x) circle (1.1pt) + (\y:0.5cm) node{$\x$};
			\draw (1.75,2.7)node[below]{$3$\,\text{km} } ;
			\draw (3.25,-0.5)node[below]{$4$\,\text{km} } ;
	\end{tikzpicture}}
	\loigiai{
		Gọi độ dài đoạn $MI$ là $x$ (km). Điều kiện $x>0$.\\
		Khi đó $IC=3-x$ (km).\\
		Xét tam giác $IAM$ vuông tại $M$, áp dụng định lý Py-ta-go ta được $$IA=IB=\sqrt{2^2+x^2}.$$.\\
		Suy ra tổng độ dài đường ống là $$IA+IB+IC=3-x+2\sqrt{4+x^2} \quad (\text{km}).$$\\
		Đặt $f(x)=3-x+2\sqrt{4+x^2}$, suy ra $f'(x)=-1+\dfrac{2x}{\sqrt{4+x^2}}$.\\
		$f'(x)=0\Leftrightarrow\sqrt{4+x^2}=2x\Leftrightarrow x=\pm \dfrac{2\sqrt{3}}{3}$.\\
		Bảng biến thiên
		\begin{center}
			\begin{tikzpicture}
				\tkzTabInit[lgt=1, espcl=2]
				{$x$/1,$y'$/0.6,$y$/2.5}{$0$,$\dfrac{2\sqrt{3}}{3}$,$3$}
				\tkzTabLine{,-,0,+,}
				\tkzTabVar{+/$7$,-/$3+2\sqrt{3}$,+/$2\sqrt{13}$}
			\end{tikzpicture}
		\end{center}
		Vậy cần chọn điểm $I$ cách vị trí $M$ một khoảng xấp xỉ bằng $
		1{,}1547$ thì tổng độ dài đoạn đường là nhỏ nhất.
	}
\end{ex}
\dongcham{10}
\begin{ex}
	\immini[thm]{Để làm một cái hộp đựng quà tặng bạn, từ một tấm bài hình chữ nhật với kích thước $40$ cm $\times$ $20$ cm, bạn Hoa cắt bỏ hai hình vuông cạnh là $x$ (cm) và hai hình chữ nhật (phần gạch sọc như hình bên) rồi gấp theo đường nét đứt và dán các mép để được một cái hộp kín có dạng hình hộp chữ nhật. Hỏi có thể tạo được cái hộp có thể tích lớn nhất bằng bao nhiêu? (\textit{kết quả tính theo đơn vị $cm^3$ và làm tròn đến hàng đơn vị})\\
		\shortans[oly]{$770$}
	}{
		\begin{tikzpicture}[smooth,font=\footnotesize,scale=0.7,thick]
			...
			(0,4) coordinate (D)
			;
			\fill[pattern = north east lines] (0,3)--(1,3)--(1,4)--(0,4);
			\fill[pattern = north east lines] (0,0)--(1,0)--(1,1)--(0,1);
			\fill[pattern = north east lines] (4,0)--(8,0)--(8,1)--(4,1);
			\fill[pattern = north east lines] (4,3)--(8,3)--(8,4)--(4,4);
			\draw 
			(A)--(B)--(C)--(D)--(A)
			(4,0)--(8,0)--(8,1)--(4,1)--(4,0)
			(4,4)--(4,3)--(8,3)--(8,4)--(4,4)
			(1,0)--(1,1)--(0,1)--(0,0)node[left,midway]{$x$}
			(1,4)--(1,3)--(0,3)--(0,4)node[left,midway]{$x$}
			;
			\draw[dashed] (1,1)--(4,1)--(4,3)--(1,3)--(1,1) (5,1)--(5,3);
			\draw[<->] (9,0)--(9,4) node[above,midway,sloped]{$20$cm};
			\draw[<->] (0,-1)--(8,-1) node[above,midway]{$40$cm};
	\end{tikzpicture}}
	\loigiai{ 
		Kết quả $770$
}\end{ex}
...
\begin{ex}
	\immini[thm]{
		Từ một tấm bìa hình vuông $ABCD$ có cạnh bằng $5 $ dm, người ta cắt bỏ bốn tam giác bằng nhau là $AMB,$ $BNC,$ $CPD$ và $DQA.$ Với phần còn lại, người ta gấp lên và ghép lại để thành hình chóp tứ giác đều. Hỏi cạnh đáy của khối chóp bằng bao nhiêu dm để thể tích của nó là lớn nhất? (kết quả làm tròn đến hàng phần trăm)\\
		\shortans[oly]{$2,83$}
	}{
		\begin{tikzpicture}[line join=round, line cap=round,>=stealth,thick]
			...
			\coordinate (N1) at ($(B)!0.5!(C)$);
			\coordinate (P1) at ($(D)!0.5!(C)$);
			\coordinate (Q1) at ($(A)!0.5!(D)$);
			\tkzDefPointBy[homothety = center O ratio 2/5](M1)\tkzGetPoint{M}
			\tkzDefPointBy[homothety = center O ratio 2/5](N1)\tkzGetPoint{N}
			\tkzDefPointBy[homothety = center O ratio 2/5](P1)\tkzGetPoint{P}
			\tkzDefPointBy[homothety = center O ratio 2/5](Q1)\tkzGetPoint{Q}
			\tkzInterLL(A,O)(Q,M) \tkzGetPoint{I}
			\tkzDrawPoints[fill=black,size=2pt](A,B,C,D,O,M,N,P,Q,I)
			\tkzDrawSegments(A,B B,C C,D D,A A,M M,B B,N N,C C,P P,D D,Q Q,A A,I)
			\tkzDrawSegments[dashed](M,N N,P P,Q Q,M I,O)
			\tkzLabelPoints[left](D,Q)
			\tkzLabelPoints[above](A,B,M,I)
			\tkzLabelPoints[right](C,O,N)
	...
	\loigiai{
		\begin{center}
			...
			\begin{tikzpicture}[scale=0.7,line join=round, line cap=round,thick]
				...
				\coordinate (N) at (-2,-2);
				\coordinate (Q) at (5,0);
				\coordinate (M) at ($(N)+(Q)-(P)$);
				\tkzInterLL(P,M)(N,Q) \tkzGetPoint{O}
				\coordinate (S) at ($(O)+(0,5)$);
				\coordinate (I) at ($(M)!0.5!(Q)$);
				\tkzDrawPoints[fill=black,size=2pt](M,N,P,Q,S,O,I)
				\tkzDrawSegments(N,M M,Q S,N S,M S,Q S,I)
				\tkzDrawSegments[style=dashed ](P,N P,Q S,P P,M N,Q S,O O,I)
			...
			\end{tikzpicture}
		\end{center}
		Đặt $MQ=x$ dm $\left(0<x<5\sqrt{2}\right).$\\
		Ta có $AO= \dfrac{AC}{2}=\dfrac{5\sqrt{2}}{2},$ \quad $OI=\dfrac{MQ}{2}=\dfrac{x}{2},$ \quad  $SI=AI=AO-IO=\dfrac{5\sqrt{2}-x}{2}.$\\
		Chiều cao hình chóp là  $$h=SO=\sqrt{SI^2-OI^2}=\sqrt{{\left(\dfrac{5\sqrt{2}-x}{2}\right)}^2-{\left(\dfrac{x}{2}\right)}^2}=\sqrt{\dfrac{50-10x\sqrt{2}}{4}}.$$
		Thể tích khối chóp là 
		$$V=\dfrac{1}{3}S_{MNPQ}\cdot SO =\dfrac{1}{3}\cdot x^2\sqrt{\dfrac{50-10x\sqrt{2}}{4}}=\dfrac{1}{6}\sqrt{50x^4-10x^5\sqrt{2}}.$$
		Xét hàm số $f(x)=\sqrt{50x^4-10x^5\sqrt{2}}$ trên khoảng $\left(0;5\sqrt{2}\right).$ Ta có
		\begin{itemize}
			\item [$\bullet$] $f'(x)=\dfrac{100x^3-25x^4\sqrt{2}}{\sqrt{50x^4-10x^5\sqrt{2}}}.$
			\item [$\bullet$] Khi đó $f'(x)=0 \Leftrightarrow 100x^3-25x^4\sqrt{2}=0 \Leftrightarrow \hoac{&x=0 \notin \left(0;5\sqrt{2} \right)\\& x=2\sqrt{2}.}$
		\end{itemize}
		Bảng biến thiên
		\begin{center}
			\begin{tikzpicture}
				\tkzTabInit[lgt=1.2,espcl=3]{$x$ /1,$f'(x)$ /0.8,$f(x)$ /1.7}
				{$0$,$2\sqrt{2}$,$5\sqrt{2}$}%
				\tkzTabLine{ , + , 0 ,-, }
				\tkzTabVar{-/ , +/  ,  -/ }
			\end{tikzpicture}
		\end{center}
		Hàm số $f(x)$ đạt giá trị lớn nhất khi $x=2\sqrt{2}.$\\
		Vậy thể tích hình chóp lớn nhất khi $x=2\sqrt{2}\approx 2,83$ (dm).
	}
\end{ex}
\dongcham{10}
\Closesolutionfile{ans}
