...
\begin{khung4}{GHI NHỚ}
	...
	% $$\heva{&a \ne 0\\&b^2-3ac >0.}$$
	\ding{172} Liên hệ tổng tích hai nghiệm của $y'$
	$\heva{&x_1+x_2=-\dfrac{2b}{3a}\\&x_1x_2=\dfrac{c}{3a}}$\\
	\ding{173} Tâm đối xứng của đồ thị là trung điểm của đoạn nối 2 điểm cực trị. Hoành độ tâm đối xứng là nghiệm phương trình $y''=0 \Leftrightarrow x=-\dfrac{b}{3a}$.
	% \ding{174} Tiếp tuyến tại tâm đối xứng sẽ có hệ số góc nhỏ nhất nếu $a>0$ và lớn nhất nếu $a<0$.
\end{khung4}
...
\begin{enumerate}[\iconCH]
	\item Tập xác định $\mathscr{D}=\mathbb{R}\backslash \left\{-\dfrac{d}{c}\right\}$; Đạo hàm $y'=\dfrac{ad-cb}{(cx+d)^2}$.
...
\subsubsection{Hàm số $\bm{y = }\dfrac{\bm{a{x^2} + bx + c}}{\bm{dx + e}}\bm{\left( {a \ne 0,e \ne 0} \right)}$ (đa thức tử không chia hết cho đa thức mẫu)}
\begin{enumerate}[\iconCH]
	\item Tập xác định $\mathscr{D}=\mathbb{R}\backslash \left\{-\dfrac{e}{d}\right\}$; Đạo hàm $y'=\dfrac{adx^2+2aex + be - cd}{(dx+e)^2}$.
	...
	\item Hình dạng đồ thị:
\end{enumerate}
\begin{tikzpicture}[line cap=butt,line join=miter,>=stealth,scale=0.3,font=\footnotesize]
		...
		\begin{enumerate}[a)]
			\item Tập xác định $\mathbb{R}$.\\
			      ...
			      \begin{itemize}
				      ...
				      \item [$\bullet$] \immini[thm]{Bảng biến thiên như hình bên:\\
