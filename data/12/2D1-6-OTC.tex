...
\end{center}
\begin{ex}%[2D1Y1-2]
	\immini[thm]{Cho hàm số $y=f(x)$ xác định và liên tục trên khoảng $(-\infty;+\infty),$ có bảng biến thiên như hình bên.
...
\end{ex}
\begin{ex}%[2D1B5-1] 
	\immini[thm]{Đường cong trong hình sau là đồ thị của một hàm số trong bốn hàm số được liệt kê ở bốn phương án A, B, C, D dưới đây. Hỏi hàm số đó là hàm số nào?}
...
\end{ex}
\begin{ex}%[2D1B5-3]
	\immini[thm]{Đồ thị sau đây là của hàm số $y=x^4-3x^2-3$. Với giá trị nào của $m$ thì phương trình $x^4-3x^2+m=0$ có ba nghiệm phân biệt?
	\choice
	{$m=-3$}
	{$m=-4$}
	{\True $m=0$}
...
\end{ex}
\begin{ex}%[2D1Y2-2]
	\immini[thm]{Cho hàm số $y=f(x)$ có bảng biến thiên như hình vẽ bên. Mệnh đề nào dưới đây đúng?
	\choice
	{$y_{\text{CT}}=0$}
...
\end{ex}
\begin{ex}%[2D1Y2-2]
	\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình bên. Mệnh đề nào dưới đây đúng?
	\choice
...
\end{ex}
\begin{ex}%[2D1B5-1]
	\immini[thm]{Cho hàm số $y=f(x)=ax^3+bx^2+cx+d$ có đồ thị như hình vẽ ở bên. Mệnh đề nào.
	\choice
	{$a>0$, $b>0$, $c<0$, $d>0$}
	{$a>0$, $b>0$, $c>0$, $d>0$}
...
\begin{ex}%Câu 21.%[2D1Y2-2]
	\immini[thm]{Cho hàm số $y=f(x)$ xác định trên $\mathbb{R}$ và có đồ thị hàm số $y=f'(x)$ là đường cong ở hình bên. Hỏi hàm số $y=f(x)$ có bao nhiêu điểm cực trị?
	\choice
	{$6$}
	{$5$}
	{$4$}
...
\end{ex}
\begin{ex}%[2D1K3-6] 
	\immini[thm]{Tính diện tích lớn nhất $S_{\max}$ của một hình chữ nhật nội tiếp trong nửa đường tròn bán kính $R=6 \;\mathrm{cm}$ nếu một cạnh của hình chữ nhật nằm dọc theo đường kính của hình tròn mà hình chữ nhật đó nội tiếp.} 
	...
	\loigiai{
		\immini[thm]{Gọi hình chữ nhật cần tính diện tích là $ABCD$ có $OC=x (0<x<6)$, $OB=6$.\\
...
\end{ex}
\begin{ex}%[2D1B5-2]
	\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị trong hình vẽ bên. Tìm tất cả các giá trị của $m$ để phương trình $\left|f(x)\right|=m$ có đúng hai nghiệm phân biệt.
	\choice
	{\True $m>5$, $0<m<1$}
	{$m<1$}
	{$m=1$, $m=5$}
	...
	{\begin{tikzpicture}[scale=.6, line join=round, line cap=round,>=stealth]		
		...
		\end{tikzpicture}}
	\loigiai{
		\immini[thm]{Từ đồ thị $(C)$ của hàm số $y=f(x)$ ta suy ra đồ thị $(C')$ của hàm số $y=\left|f(x)\right|$ như sau:\\
...
\end{ex}
\begin{ex}%[2D1B5-1]
	\immini[thm]{Cho hàm số $y=\dfrac{x-a}{bx+c}$ có đồ thị như hình vẽ bên dưới. Tính giá trị của biểu thức $P=a+b+c$.
	\choice
	{\True $P=-3$}
	{$P=1$}
	{$P=5$}
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D1G5-3]%Câu 38.
	\immini[thm]{Cho hàm số $f(x)=x^3-3x^2+2$ có đồ thị là đường cong trong hình bên.
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D1K5-5]%Câu 42.
	\immini[thm]{Cho hàm số $y=f(x)$. Đồ thị của hàm số $y=f'(x)$ như hình vẽ. Đặt $h(x)=f(x)-x$. Mệnh đề nào dưới đây đúng?
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D1G3-6]%Câu 44.
	\immini[thm]{Một cái ao hình $ABCDE$ (như hình vẽ), ở giữa ao có một mảnh vườn hình tròn có bán kính $10$ m. Người ta muốn bắc một câu cầu từ bờ $AB$ của ao đến vườn. Tính gần đúng độ dài tối thiếu $l$ của cây cầu biết:\\
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D1G5-5]%Câu 45.
	\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị $f'(x)$ như hình vẽ.	
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D1G5-5]%Câu 47.
	\immini[thm]{Cho hàm số $y=f(x)$ có đạo hàm liên tục trên đoạn $[-3;3]$ và đồ thị hàm số $y=f'(x)$ như hình vẽ bên. Biết $f(1)=6$ và $g(x)=f(x)-\dfrac{(x+1)^2}{2}$. Kết luận nào sau đây là đúng?
		\choice
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D1G5-5]%Câu 50.
	\immini[thm]{Cho hàm số $y=f(x)$. Hàm số $y=f'(x)$ có đồ thị như hình bên. Hàm số $y=f\left(x-x^2\right)$ nghịch biến trên khoảng nào dưới đây.	
		\choice
