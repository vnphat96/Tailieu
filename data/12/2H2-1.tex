...
\immini[thm]{\textbf{a) Diện tích xung quanh của hình nón.}\\
...
\draw[cyan] (S)--(A) (S)--(B);
\end{tikzpicture}}
\immini[thm]{\textbf{b) Diện tích toàn phần của hình nón.}\\
...
\immini[thm]{– Thể tích của khối nón tròn xoay là giới hạn của thể tích khối chóp đều nội tiếp khối nón khi đó số cạnh tăng lên vô hạn.\\
...
\immini[thm]{– Hình nón cụt là phần nón giới hạn bởi mặt đáy và một thiết diện song song với đáy.\\
– Công thức.\\
...
\immini[thm]{Trong mp$(P)$ cho hai đường thẳng $\Delta$ và $l$ song song nhau, cách nhau một khoảng bằng $r$. Khi quay $(P)$ xung quanh $\Delta$ thì $l$ sinh ra một mặt tròn xoay được gọi là mặt trụ tròn xoay. $\Delta$ gọi là trục, $l$ gọi là đường sinh, $r$ là bán kính của mặt trụ đó.} 
{
...
\begin{vd}%[TLDH4-Nguyễn Cao Cường]%[2H2B1-1]%Ví dụ 2.
	...
	\end{itemize}
	\loigiai{
\immini[thm]{Quay tam giác $SOA$ xung quanh cạnh $SO$ được hình nón hình bên.
	\begin{itemize}
	\item[a)]Diện tích xung quanh $S_{\text{xq}}=\pi rl=15\pi$ (cm$^2$).\\
	Diện tích toàn phần $S_{\text{tp}}=\pi rl+\pi r^2=24\pi$ (cm$^2$). 
...
\begin{vd}%[TLDH4-Nguyễn Cao Cường]%[2H2B1-1]%Ví dụ 3.
	...
	\end{itemize}
	\loigiai{
\immini[thm]{Quay tam giác $SAB$ xung quanh cạnh $SO$ được hình nón như hình vẽ. Ta có $r=\dfrac{AB}{2}=\dfrac{a}{2}$.
	\begin{itemize}
...
\begin{vd}%[TLDH4-Nguyễn Cao Cường]%[2H2K1-5]%Ví dụ 5.
	...
	\loigiai{
\immini[thm]{Ta có $\heva{&AC=AB\cdot\cos\alpha=2R\cdot\cos\alpha\\&CH=AC\cdot\sin\alpha=2R\cdot\cos\alpha\cdot\sin\alpha\\&AH=AC\cdot\cos\alpha=2R\cdot\cos^2\alpha.}$ \\
...
\begin{ex}%[TLDH4-Nguyễn Cao Cường]%[2H2Y1-2]%Câu 5.
	...
	{$2\pi a^2\sqrt{3}$}
	\loigiai{
	\immini[thm]{Diện tích xung quanh $S_{\text{xq}}=\pi rl=2\pi a^2$.}
	{\begin{tikzpicture}[scale=.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
	\def\x{3} % Bán kính trụ lớn
	\pgfmathsetmacro{\y}{\x/4} % Bán kính trục bé
	\def\h{5} % Chiều cao
...
\begin{ex}%[TLDH4-Nguyễn Cao Cường]%[2H2B1-1]%Câu 14.
	...
	{$4\pi$}
	\loigiai{
	\immini[thm]{Ta có $CB=AB\tan 60^{\circ}=\sqrt{3}$.\\ Vậy $V=\dfrac{1}{3}\pi R^2h=\dfrac{1}{3}\pi\cdot 3\cdot 1=\pi$.}
	{\begin{tikzpicture}[scale=.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
	\def\x{3} % Bán kính trụ lớn
	\pgfmathsetmacro{\y}{\x/4} % Bán kính trục bé
	\def\h{5} % Chiều cao
...
\immini[thm]{	Có một chiếc cốc có dạng như hình vẽ, biết chiều cao của chiếc cốc là $8$ cm, bán kính đáy cốc là $3$ cm, bán kính miệng cốc là $6$ cm. Tính thể tích $V$ của chiếc cốc. 
	\choice
	{$72\pi$ cm$^3$}
	{$48\pi$ cm$^3$}
	{\True$168\pi$ cm$^3$}
...
\begin{ex}%[TLDH4-Nguyễn Cao Cường]%[2H2B1-3]%Câu 16.
	...
	\loigiai{
	\immini[thm]{Dựa vào giả thiết ta có bán kính đáy hình nón là bán kính đường tròn nội tiếp hình vuông nên $r=\dfrac{a}{2}$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2B1-1]%Câu 20.
	\immini[thm]{Tam giác vuông cân $ABC$ có $AB=AC=a\sqrt{2}$ và hình chữ nhật $MNPQ$ với $MQ=2MN$ được xếp chồng lên nhau sao cho $M$, $N$ lần lượt là trung điểm của $AB$, $AC$ (như hình vẽ). Tính thể tích $V$ của vật thể tròn xoay khi quay mô hình trên quanh trục $AI$, với $I$ là trung điểm $PQ$. 	
		\choice
		{$V=\dfrac{11\pi a^3}{6}$}
		{$V=\dfrac{5\pi a^3}{6}$}
		{$V=\dfrac{11\pi a^3}{8}$}
	...
	\loigiai{
		\immini[thm]{Ta có $BC=\sqrt{AB^2+AC^2}=2a\Rightarrow MN=a$, $MQ=2a$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2B1-1]%Câu 21.
	...
	\loigiai{
		\immini[thm]{Gọi là $O$ tâm của tam giác đều $BCD$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2B1-2]%Câu 22.
	...
	\loigiai{
		\immini[thm]{Gọi $I$ là tâm đường tròn $(ABC)$, suy  ra $ IA=r=\dfrac{a\sqrt{3}}{3}$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2G2-5]%Câu 23.
	...
	\loigiai{
		\immini[thm]{Rõ ràng trong hai khối nón cùng bán kính đáy nội tiếp trong một khối cầu thì khối nón có chiều cao lớn hơn thì thể tích lớn hơn, nên ta chỉ xét khối nón có chiều cao lớn hơn trong hai khối nón đó.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2G1-5]%Câu 24.
	...
	{\True $2$}
	\loigiai{
		\immini[thm]{Ta có thể tích khối nón là $V_1=\dfrac{1}{3}\pi r^2h$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2G1-5]%Câu 25.
	...
	{$r=3$}
	\loigiai{
		\immini[thm]{Gọi bán kính và thể tích của hình cầu là $R$ và $V_C$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2B1-1]%Câu 26.
	\immini[thm]{Hình bên cho ta hình ảnh của một đồng hồ cát với các kích thước kèm theo $OA=OB$. Khi đó tỉ số tổng thể tích của hai hình nón $(V_n)$ và thể tích hình trụ $(V_t)$ bằng 		
		\choice
		{$\dfrac{1}{4}$}
		{$\dfrac{2}{5}$}
		{$\dfrac{1}{2}$}
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2K1-1]%Câu 27.
	\immini[thm]{Cho một đồng hồ cát như hình bên (gồm 2 hình nón chung đỉnh ghép lại), trong đó đường sinh bất kỳ của hình nón tạo với đáy một góc $60^{\circ}$ như hình bên. Biết rằng chiều cao của đồng hồ là $30$ cm và tổng thể tích của đồng hồ là $1000\pi$ cm$^3$. Hỏi nếu cho đầy lượng cát vào phần trên thì khi chảy hết xuống dưới, khi đó tỉ lệ thể tích lượng cát chiếm chỗ và thể tích phần phía dưới là bao nhiêu?
		\choice
		{$\dfrac{1}{3\sqrt{3}}$}
		{\True $\dfrac{1}{8}$}
		{$\dfrac{1}{64}$}
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2G1-5]%Câu 29.
	...
	\loigiai{
		\immini[thm]{Khi xoay tam giác $OAB$ quanh trục $Ox$ tạo thành hình nón có đường cao là $OA=2017\cdot\cos\alpha$ và bán kính đáy là\break $AB=OB\cdot\sin\alpha=2017\cdot\sin\alpha$.\\
			Thể tích khối nón 
			\begin{eqnarray*}
...
\begin{vd}%[Nguyen Tam Phuc]%[2H2Y1-2]%Ví dụ 2.
	...
	\loigiai{
		\immini[thm]{Gọi thiết diện qua trục là hình vuông $ABB'A'$ với $AB$, $A'B'$ lần lượt là đường kính 2 đường tròn đáy $\Rightarrow AB=A'B'=2r=2a$, do đó $h=AA'=BB'=2a$.\\
...
\begin{vd}%[Nguyen Tam Phuc]%[2H2K1-2]%Ví dụ 4.
	...
	\loigiai{		
		\immini[thm]{Gọi $M$, $N$ trung điểm $AB$, $CD\Rightarrow MN$ là trục hình vuông $ABCD$ và $MN$ qua trung điểm $I$ của $OO'$.\\
...
\begin{vd}%[Nguyen Tam Phuc]%[2H2K1-2]%Ví dụ 5.
	...
	\loigiai{
		\immini[thm]{Đặt độ dài cạnh tam giác đều $ABC$ là $x$.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[ID6]%Câu 9.
	...
	{$126\pi$ (cm$^2$)}
	\loigiai{
		\immini[thm]{Quay hình vuông $ABCD$ xung quanh $MN$ ta được hình trụ như hình vẽ.\\
...
\begin{ex}%[Nguyen Tam Phuc]%[2H2K1-2]%Câu 12.
	...
	{$\pi a^2$}
	\loigiai{
		\immini[thm]{Khi quay hình chữ nhật này xung quanh cạnh $AD$ ta được hình trụ như hình vẽ. \\
