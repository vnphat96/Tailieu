...
\begin{ex}%[EX-Ôn Tập TN 2025, PT Sinh]%[2D1N2-2]
	\immini[thm]{Cho hàm số $y = f(x)$ có đồ thị như hình vẽ. Điểm cực tiểu của đồ thị hàm số đã cho là
		\choice[2]
		{$(-1; 0)$}
		{$(1; 0)$}
		{$(2; 0)$}
...
\begin{ex}%[2D1H5-1]%[KNTT - Lớp 12 - Ôn tập cuối học kì 1 - Đề 4]%[Phúc Hậu]
	\immini[thm]{Đồ thị như hình vẽ là đồ thị của hàm số nào dưới đây?
		...
		{$y=-x^3-3x^2-4$}
		{$y=x^3-3x^2+4$}
		{$y=x^3-3x^2-4$}}{\begin{tikzpicture}[scale=0.6,>=stealth, font=\footnotesize, line join=round, line cap=round]
...
\begin{ex}%[2H2N1-3]
	Cho hình chóp $S.ABCD$ có $ABCD$ là hình vuông cạnh $ a $. Cạnh bên $ SA $ vuông góc với mặt đáy $ (ABCD) $ và $ SA=a\sqrt{3} $. Tích vô hướng của hai vectơ $\overrightarrow{CB} $ và $\overrightarrow{DS}$ bằng
	\choice
	{$a^2$}
	{$2a^2$}
	{$a^2\sqrt{2}$}
...
\begin{bt}%[Lê Vũ Hải]%[2H2V2-6]
	...
	\loigiai{
		...
		Do ba điểm $A$, $B$, $C$ thẳng hàng nên hai vectơ $\vec{AB}$ và $\vec{AC}$ cùng phương, suy ra $\heva{&x-1 = k \cdot (-0,5)\\ & y-3=k\cdot (-2,25) \\& -2=k\cdot (-2,5)} \Leftrightarrow \heva{& k=0,8 \\& x=0,6 \\& y=1,2}$. \\
