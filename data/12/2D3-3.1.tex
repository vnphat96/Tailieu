...
\begin{vd}%[2D3Y3-1]%Ví dụ 1: 
	\immini[thm]{Cho hàm số $y=f(x)$ liên tục trên đoạn $[a;b]$. Gọi $D$ là hình phẳng giới hạn bởi đồ thị $(C)\colon y=f(x)$, trục hoành, hai đường thẳng $x=a$, $x=b$ (như hình vẽ bên). Giả sử $S_D$ là diện tích hình phẳng $D$. Chọn công thức \textbf{đúng} trong các phương án $A,B,C,D$ cho dưới đây?
	}{
		\begin{tikzpicture} [scale=1,yscale=.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
		\def \xmin{-3} \def \xmax{3}
...
\begin{ex}%[2D3Y3-1]%Câu 5.
	\immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình vẽ dưới đây. Diện tích hình phẳng (phần tô màu trong hình vẽ) được tính bằng công thức nào?
		\choice
...
\begin{ex}%[2D3Y3-1]%Câu 7.
	\immini[thm]{Cho đồ thị hàm số $y=f(x)$. Diện tích hình phẳng (phần có đánh dấu gạch trong hình) là 
	}{	
...
\begin{ex}%[2D3Y3-1]%Câu 8.
	\immini[thm]{Cho đồ thị hàm số $y=f(x)$ trên đoạn $[0; 4]$ như hình vẽ và có diện tích $S_1=\dfrac{11}{6}, S_2=\dfrac{9}{2}$. Tính tích phân $I=\displaystyle\int\limits_0^4 f(x)\mathrm{\,d}x$. 
		\choice
		{\True $I=-\dfrac{8}{3}$}
		{$I=\dfrac{19}{3}$}
		{$I=\dfrac{8}{3}$}
...
\begin{ex}%[2D3K3-1]%Câu 29.
	...
	\loigiai{
		\immini[thm]{
			...
			PTĐT $(d)$ đi qua điểm $A(0;4)$ có hệ số góc $k$ có dạng: $y=kx+4$.\\
		}{
...
\begin{ex}%Câu 37.%[2D3B3-1]%[Don Lee]
	...
	\loigiai{
		...
		{\begin{tikzpicture}[>=stealth, color=black, line width = 1pt, scale=.7]
			...
			\fill(-4/3,0) node[below]{\tiny $-\ln 2$};
...
\begin{dang}{Ứng dụng tích phân tính diện tích hình phẳng giới hạn bởi $y=f(x),y=g(x)$.}
	\textbf{Phương pháp giải:}\\
	\immini[thm]{\textbf{Dạng:} Cho hai hàm số $y=f(x)$ và $y=g(x)$ liên tục trên đoạn $[a; b]$. Diện tích hình phẳng giới hạn bởi các đường $y=f(x)$ và $y=g(x)$ là 
...
\begin{vd}%[Nguyễn Tâm Phục]%[2D3G3-1]%Ví dụ 4.
	\immini[thm]{Cho parabol $(P)\colon y=-x^2+2x$, có đỉnh $S$ và $A$ là giao điểm khác $O$ của $(P)$ và trục hoành. $M$ là điểm di động trên đường cong $SA$ của $(P)$, tiếp tuyến của $(P)$ tại $M$ cắt $Ox,Oy$ lần lượt tại $E$ và $F$. Tìm giá trị nhỏ nhất của tổng diện tích $2$ tam giác cong $MOE$ và $MAF$ thì giá trị của m là 
		\choice
		{$\dfrac{5}{3}$}
		{$4$}
		{\True $\dfrac{4}{3}$}
...
\begin{ex}%[Nguyễn Tâm Phục]%[2D3G3-1]%Câu 84.
	...
	\loigiai{
		...
		Khi đó PTĐT $AB$ là $$y-a^2=\dfrac{b^2-a^2}{b-a}(x-a)\Rightarrow y=(a+b)x-ab.$$
