...
\begin{ex}
	\immini[thm]{Cho hàm số $y=f(x)$ có bảng biến thiên như hình bên. Xét tính đúng, sai của các khẳng định sau:
	\choiceTF
	{\True $f(-5)<f(4)$}
...
\begin{ex}
	...
	{$\lim _{x \rightarrow-\infty} f(x)=2$ ; $\lim _{x \rightarrow 1^{-}} f(x)=-\infty$}
...
\end{tikzpicture}}
	...
	\begin{enumerate}[a)]
		...
		\item Theo hình vẽ thì $\lim _{x \rightarrow-\infty} f(x)=2$ ; $\lim _{x \rightarrow 1^{-}} f(x)=-\infty$.
