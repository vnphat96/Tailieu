...
\immini[thm]{	Giá trị nhỏ nhất trên tập xác định của hàm số có đồ thị sau là 
...
\immini[thm]{	Cho đồ thị hàm số $y=f'(x)$ như hình vẽ. 
...
\immini[thm]{	Cho hàm số $y=f(x)$ có đạo hàm là $f'(x)$. Đồ thị hàm số $y=f'(x)$ được cho như hình vẽ bên. Biết $f(0)+f(3)=f(2)+f(5)$. Giá trị nhỏ nhất và giá trị lớn nhất của $y=f(x)$ trên đoạn $[0;5]$ lần lượt là
	\choice
	{\True $f(2),f(5)$}
	{$f(0),f(5)$}
	{$f(0),f(2)$}
...
\immini[thm]{	Giá trị nhỏ nhất của hàm số $y=x^3-3x+5$ có đồ thị sau trên đoạn $[0;2]$ là 
	\choice
	{$\min\limits_{[0;2]} y=0$}
	{\True $\min\limits_{[0;2]} y=1$}
	{$\min\limits_{[0;2]} y=3$}
	{$\min\limits_{[0;2]} y=5$}}{	{\color{black} \begin{tikzpicture}[scale=0.8,>=stealth, font=\footnotesize, line join=round, line cap=round]
		%	\draw[color=gray!50,dashed] (-4,-6) grid (4,5);
...
\begin{ex}%Câu 10%[Nguyễn Diệu Linh]%[2D1Y3-1]
	...
	\begin{center}
	{\color{black} \begin{tikzpicture}[scale=1.0,>=stealth, font=\footnotesize, line join=round, line cap=round]
		%	\draw[color=gray!50,dashed] (-4,-6) grid (4,5);
...
\immini[thm]{	Hàm số $y=x^3-3x+1$ có đồ thị như hình vẽ đạt giá trị lớn nhất và giá trị nhỏ nhất trên đoạn $\left[-\dfrac{3}{2};\dfrac{3}{2}\right]$ tại điểm có hoành độ lần lượt là $x_1;x_2$. Khi đó tổng $x_1+x_2$ bằng 
	\choice
	{$2$}
	{$1$}
	{$3$}
...
\immini[thm]{	Cho đồ thị hàm số $y=f'(x)$ như hình vẽ. 
	...
	{$x=4$}
	{$x=-1$}}
{{\color{black} \begin{tikzpicture}[>=stealth,x=1cm,y=1cm,scale=0.8]
...
\immini[thm]{	Cho đồ thị hàm số $y=f'(x)$ như hình vẽ. 
	...
	{$x=0$}
	{$x=2$}
	{\True $x=1$}}{{\color{black} \begin{tikzpicture}[>=stealth,x=1cm,y=1cm,scale=0.8]
