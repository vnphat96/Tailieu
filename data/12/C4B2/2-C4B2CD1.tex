...
\begin{note}Chú ý:
	\begin{itemize}
		...
		\item Quy ước: $\int\limits_a^af(x)\mathrm{d}x=0$; $\int\limits_a^bf(x)\mathrm{d}x=-\int\limits_b^af(x)\mathrm{d}x$.
		% \item Tích phân của hàm số $f$ từ $a$ đến $b$ chỉ phụ thuộc vào $f$ và các cận $a$, $b$ mà không phụ thuộc vào biến $x$ hay $t$, nghĩa là $\int\limits_a^bf(x)\mathrm{d}x=\int\limits_a^bf(t)\mathrm{d}t$.
	\end{itemize}
\end{note}
\begin{nx}~
	\begin{itemize}
		\item Nếu hàm số $f(x)$ có đạo hàm $f'(x)$ và $f'(x)$ liên tục trên đoạn $\left[a;b\right]$ thì $f(b)-f(a)=\int\limits_a^bf'(x)\mathrm{d}x$.
		\item Ý nghĩa hình học của tích phân.\\
		\immini{
			Nếu hàm số $f(x)$ liên tục và \textbf{không âm} trên đoạn $\left[a;b\right]$ thì $\int\limits_a^bf(x)\mathrm{d}x$ là diện tích $S$ của hình thang cong giới hạn bởi đồ thị $y=f(x)$, trục hoành và hai đường thẳng $x=a$, $x=b$.
			$$S=\int\limits_a^bf(x)\mathrm{\,d}x.$$}{\begin{tikzpicture}[>=stealth,scale=0.8]
		...
		\item Đạo hàm của quãng đường di chuyển của vật theo thời gian bằng tốc độ của chuyển động tại mọi thời điểm $v(t)=s'(t)$. Do đó, nếu biết tốc độ $v(t)$ tại mọi thời điểm $t\in\left[a;b\right]$ thì tính được quãng đường di chuyển trong khoảng thời gian từ $a$ đến $b$ theo công thức: $s=s(b)-s(a)=\int\limits_a^bv(t)\mathrm{d}t$.
	\end{itemize}
\end{nx}
\subsubsection{Tính chất của tích phân}
Cho hai hàm số $f(x)$, $g(x)$ liên tục trên đoạn $\left[a;b\right]$. Khi đó:
\begin{enumerate}
	\item $\int\limits_a^bkf(x)\mathrm{d}x=k\int\limits_a^bf(x)\mathrm{d}x$, với $k$ là hằng số.
	\item $\int\limits_a^b\left[f(x)\pm g(x)\right]\mathrm{\,d}x=\int\limits_a^b{f(x)\mathrm{\,d}x}\pm\int\limits_a^bg(x)\mathrm{\,d}x$.
	\item $\int\limits_a^bf(x)\mathrm{\,d}x=\int\limits_a^cf(x)\mathrm{\,d}x+\int\limits_c^bf(x)\mathrm{\,d}x$ với $c\in\left(a;b\right)$.
\end{enumerate}
...
\begin{ex}%[Câu 23]%[2D4H2-1]
	...
	\choiceTF
	{\True $\int\limits_a^b\left[f(x)+2g(x)\right]\mathrm{\,d}x=\int\limits_a^bf(x)\mathrm{\,d}x\text{+2}\int\limits_a^bg(x)\mathrm{\,d}x$}
	{$\int\limits_a^b\dfrac{f(x)}{g(x)}\mathrm{\,d}x=\dfrac{\int\limits_a^bf(x)\mathrm{\,d}x}{\int\limits_a^bg(x)\mathrm{\,d}x}$}
	{$\int\limits_a^b\left[f(x)\cdot g(x)\right]\mathrm{\,d}x=\int\limits_a^bf(x)\mathrm{\,d}x \int\limits_a^bg(x)\mathrm{\,d}x$}
	{$\int\limits_a^bf^2(x)\mathrm{\,d}x=\left[\int\limits_a^bf(x)\mathrm{\,d}x\right]^2$}
	\loigiai{
		\begin{itemchoice}
			\itemch Đúng. Theo tính chất tích phân ta có
			$\int\limits_a^b\left[f(x)+g(x)\right]\mathrm{\,d}x=\int\limits_a^bf(x)\mathrm{\,d}x+\int\limits_a^bg(x)\mathrm{\,d}x;\int\limits_a^bkf(x)\mathrm{\,d}x=k\int\limits_a^bf(x)\mathrm{\,d}x$, với $k\in \mathbb{R}$.
			\itemch Sai. Cho $a=1,b=2$ và $f(x)=x+1, g(x)=x$. Khi đó
			$$VT=\int\limits_{1}^2\dfrac{x+1}{x}\mathrm{\,d}x==\int\limits_{1}^2\left(1+\dfrac{1}{x}\right)\mathrm{\,d}x=\left(x+\ln x\right)\bigg|_1^2=1+\ln 2.$$
			và $$VP=\dfrac{\int\limits_1^2(x+1)\mathrm{\,d}x}{\int\limits_1^2x\mathrm{\,d}x}=\dfrac{\left(\dfrac{x^2}{2}+x\right)\bigg|_1^2}{\dfrac{x^2}{2}\bigg|_1^2}=\dfrac{1}{3}.$$
			Do đó $VT\neq VP$.
			\itemch Sai. Cho $a=1, b=2$ và $f(x)=x, g(x)=\dfrac{1}{x}$. Khi đó
			$$VT=\int\limits_1^2\left[x\cdot \dfrac{1}{x}\right]\mathrm{\,d}x=x\bigg|_1^2=1.$$
			và $$VP=\int\limits_1^2x\mathrm{\,d}x\cdot \int\limits_1^2\dfrac{1}{x}\mathrm{\,d}x=\left(\dfrac{x^2}{2}\right)\bigg|_1^2\cdot \ln x\bigg|_1^2=\dfrac{3}{2}\ln 2.$$
			Do đó $VT\neq VP$.
			\itemch Sai. Cho $a=1,b=2$ và $f(x)=x$. Khi đó
			$$VT=\int\limits_1^2x^2\mathrm{\,d}x=\left(\dfrac{x^3}{3}\right)\bigg|_1^2=\dfrac{7}{3}.$$
			và $$VP=\left(\int\limits_1^2x\mathrm{\,d}x\right)^2=\left(\dfrac{x^2}{2}\bigg|_1^2\right)^2=\dfrac{9}{4}.$$
			Do đó $VT\neq VP$.
		\end{itemchoice}
	}
\end{ex}
\begin{ex}%[Câu 24]%[2D4H2-1]
	...
	\choiceTF
	{\True Nếu $\int\limits_0^2f(x)\mathrm{\,d}x=4$ thì $\displaystyle\int\limits_0^2\left[\dfrac{1}{2}f(x)+2\right]\mathrm{\,d}x=6$}
	{\True Nếu $\int\limits_2^5f(x)\mathrm{\,d}x=3$ và $\int\limits_2^5g(x)\mathrm{\,d}x=-2$ thì $\int\limits_2^5\left[f(x)+g(x)\right]\mathrm{\,d}x=1$}
	{Nếu $\int\limits_1^4f(x)\mathrm{\,d}x=6$ và $\int\limits_1^4g(x)\mathrm{\,d}x=-5$ thì $\int\limits_1^4\left[f(x)-g(x)\right]\mathrm{\,d}x=1$}
	{\True Nếu $\int\limits_2^3f(x)\mathrm{\,d}x=4$ và$\int\limits_2^3g(x)\mathrm{\,d}x=1$ thì $\int\limits_2^3\left[f(x)-g(x)\right]\mathrm{\,d}x=3$}
	\loigiai{
		\begin{itemchoice}
			\itemch Đúng. Ta có $\int\limits_0^2\left[\dfrac{1}{2}f(x)+2\right]\mathrm{\,d}x=\dfrac{1}{2}\int\limits_0^2f(x)\mathrm{\,d}x+\int\limits_0^22\mathrm{\,d}x=\dfrac{1}{2}\cdot 4+4=6$.
			\itemch Đúng. Ta có $\int\limits_2^5\left[f(x)+g(x)\right]\mathrm{\,d}x=\int\limits_2^5f(x)\mathrm{\,d}x+\int\limits_2^5g(x)\mathrm{\,d}x=3+(-2)=1$.
			\itemch Sai. Ta có $\int\limits_1^4\left[f(x)-g(x)\right]\mathrm{\,d}x=\int\limits_1^4f(x)\mathrm{\,d}x-\int\limits_1^4g(x)\mathrm{\,d}x=6-(-5)=11$.
			\itemch Đúng. Ta có $\int\limits_2^3\left[f(x)-g(x)\right]\mathrm{\,d}x=\int\limits_2^3f(x)\mathrm{\,d}x-\int\limits_2^3g(x)\mathrm{\,d}x=4-1=3$.
		\end{itemchoice}
	}
\end{ex}
\begin{ex}%[Câu 25]%[2D4H2-1]
	Cho hàm số $f(x),g(x)$ liên tục trên $\mathbb{R}$.
	\choiceTF
	{Biết $\int\limits_2^3f(x)\mathrm{\,d}x=3$ và $\int\limits_3^2g(x)\mathrm{\,d}x=1$. Khi đó $\int\limits_2^3\left[f(x)+g(x)\right]\mathrm{\,d}x=4$}
	{\True Biết $\int\limits_1^3f(x)\mathrm{\,d}x=2022$ và $\int\limits_3^1g(x)\mathrm{\,d}x=1$. Khi đó $\int\limits_1^3\left[f(x)+g(x)\right]\mathrm{\,d}x=2021$}
	{\True Biết $\int\limits_1^2f(x)\mathrm{\,d}x=3$ và $\int\limits_1^2g(x)\mathrm{\,d}x=2$. Khi đó $\int\limits_1^2\left[f(x)-g(x)\right]\mathrm{\,d}x=1$}
	{Biết $\int\limits_2^5f(x)\mathrm{\,d}x=2$. Khi đó $\int\limits_2^53f(x)\mathrm{\,d}x=2$}
	\loigiai{
		\begin{itemchoice}
			\itemch Sai. Ta có
			$\int\limits_2^3\left[f(x)+g(x)\right]\mathrm{\,d}x=\int\limits_2^3f(x)\mathrm{\,d}x+\int\limits_2^3g(x)\mathrm{\,d}x=\int\limits_2^3f(x)\mathrm{\,d}x-\int\limits_3^2g(x)\mathrm{\,d}x=2$.
			\itemch Đúng. Ta có $\int\limits_3^1g(x)\mathrm{\,d}x=1\Leftrightarrow \int\limits_1^3g(x)\mathrm{\,d}x=-1$. Do đó 
			$$\int\limits_1^3\left[f(x)+g(x)\right]\mathrm{\,d}x=\int\limits_1^3f(x)\mathrm{\,d}x+\int\limits_1^3g(x)\mathrm{\,d}x=2022+(-1)=2021.$$
			\itemch Đúng. Ta có $\int\limits_1^2\left[f(x)-g(x)\right]\mathrm{\,d}x=\int\limits_1^2f(x)\mathrm{\,d}x-\int\limits_1^2g(x)\mathrm{\,d}x=3-2=1$.
			\itemch Sai. Ta có $\int\limits_2^53f(x)\mathrm{\,d}x=3\int\limits_2^5f(x)\mathrm{\,d}x=3\cdot 2=6$.
		\end{itemchoice}
	}
\end{ex}
\begin{ex}%[Câu 26]%[2D4H2-1]
	...
	\choiceTF
	{\True Nếu $\int\limits_0^3f(x)\mathrm{\,d}x=3$ thì $\int\limits_0^32f(x)\mathrm{\,d}x=6$}
	{\True Nếu $\int\limits_1^4f(x)\mathrm{\,d}x=2024$ thì $\int\limits_4^1f(x)\mathrm{\,d}x=-2024$}
	{Nếu $\int\limits_6^0f(x)\mathrm{\,d}x=12$ thì $\int\limits_0^62022f(x)\mathrm{\,d}x=24264$}
	{\True Nếu $\int\limits_0^1f(x)\mathrm{\,d}x=4$ thì $\int\limits_0^12f(x)\mathrm{\,d}x=8$}
	\loigiai{
		\begin{itemchoice}
			\itemch Đúng. Ta có $\int\limits_0^32f(x)\mathrm{\,d}x=2\int\limits_0^3f(x)\mathrm{\,d}x=2\cdot 3=6$.
			\itemch Đúng. Ta có $\int\limits_4^1f(x)\mathrm{\,d}x=-\int\limits_1^4f(x)\mathrm{\,d}x=-2024$.
			\itemch Sai. Ta có $\int\limits_0^62022f(x)\mathrm{\,d}x=2022\int\limits_0^6f(x)\mathrm{\,d}x=2022\cdot (-12)=-24264$.
			\itemch Đúng. Ta có $\int\limits_0^12f(x)\mathrm{\,d}x=2\int\limits_0^1f(x)\mathrm{\,d}x=2\cdot 4=8$.
		\end{itemchoice}
	}
\end{ex}
...
% 	Cho hàm số $f(x),g(x)$ liên tục trên $\mathbb{R}$.
% 	\choiceTF
% 	{Nếu $\int_0^2 f(x)d x=6$ thì $\int_0^2\left[2f(x)-1\right]\mathrm{\,d}x=-10$}
% 	{\True Nếu $\int\limits_0^2f(x)\mathrm{\,d}x=4$ thì $\int\limits_0^2\left[2f(x)-1)\right]\mathrm{\,d}x=6$}
% 	{\True Nếu $\int_0^2f(x)\mathrm{\,d}x=3$ và $\int_0^2g(x)\mathrm{\,d}x=7$ thì $\int_0^2\left[f(x)+3g(x)\right]\mathrm{\,d}x=24$}
% 	{\True Nếu $\int\limits_0^1\left[f(x)+2x\right]\mathrm{\,d}x=3$ thì $\int\limits_0^1f(x)\mathrm{\,d}x=2$}
% 	\loigiai{
% 		\begin{itemchoice}
% 			\itemch Sai. Ta có $\int _0^2\left[2f(x)-1\right]\mathrm{\,d}x=2\int _0^2f(x)\mathrm{\,d}x-\int _0^2\mathrm{\,d}x=2\cdot 6-2=10$.
% 			\itemch Đúng. Ta có $\int\limits_0^2\left[2f(x)-1)\right]\mathrm{\,d}x=\int\limits_0^22f(x)\mathrm{\,d}x-\int\limits_0^2\mathrm{\,d}x=2\cdot 4-2=6$.
% 			\itemch Đúng. Ta có $\int_0^2\left[f(x)+3g(x)\right]\mathrm{\,d}x=\int_0^2f(x)\mathrm{\,d}x+3\int_0^2g(x)\mathrm{\,d}x=3+3\cdot 7=24$.
% 			\itemch Đúng. Ta có:\\ $\int\limits_0^1\left[f(x)+2x\right]\mathrm{\,d}x=3\Leftrightarrow \int\limits_0^1f(x)\mathrm{\,d}x+2\int\limits_0^1x\mathrm{\,d}x=3\Leftrightarrow \int\limits_0^1f(x)\mathrm{\,d}x+2\cdot \dfrac{x^2}{2}\bigg|_0^1=3$.\\
% 			Suy ra $\int\limits_0^1f(x)\mathrm{\,d}x=3-x^2\bigg|_0^1=3-(1-0)=2$.
% 		\end{itemchoice}
% 	}
% \end{ex}
...
% 	Cho hàm số $f(x),g(x)$ liên tục trên $\mathbb{R}$.
% 	\choiceTF
% 	{\True Nếu $\int\limits_{-1}^5f(x)\mathrm{\,d}x=-3$ thì $\int\limits_5^{-1}f(x)\mathrm{\,d}x=3$}
% 	{\True Nếu $\int\limits_2^3f(x)\mathrm{\,d}x=-6$ thì $\int\limits_3^22f(x)\mathrm{\,d}x=12$}
% 	{Nếu $\int\limits_1^2f(x)\mathrm{\,d}x=2$ và $\int\limits_1^2g(x)\mathrm{\,d}x=6$ thì $\int\limits_2^1\left[f(x)-g(x)\right]\mathrm{\,d}x=-4$}
% 	{Nếu $\int\limits_0^1f(x)\mathrm{\,d}x=3$ và $\int\limits_0^1g(x)\mathrm{\,d}x=-4$ thì $\int\limits_1^0\left[f(x)+g(x)\right]\mathrm{\,d}x=-1$}
% 	\loigiai{
% 		\begin{itemchoice}
% 			\itemch Đúng. Ta có $\int _5^{-1}f(x)\mathrm{\,d}x=-\int _{-1}^5f(x)\mathrm{\,d}x=-(-3)=3$.
% 			\itemch Đúng. Ta có $\int\limits_3^22f(x)\mathrm{\,d}x=-\int\limits_2^32f(x)\mathrm{\,d}x=-2\int\limits_2^3f(x)\mathrm{\,d}x=-2\cdot (-6)=12$.
% 			\itemch Sai. Ta có \\
% 			$\int\limits_2^1\left[f(x)-g(x)\right]\mathrm{\,d}x=-\int\limits_1^2\left[f(x)-g(x)\right]\mathrm{\,d}x=-\int\limits_1^2f(x)\mathrm{\,d}x+\int\limits_1^2g(x)\mathrm{\,d}x=-2+6=4$.
% 			\itemch Sai. Ta có\\
% 			$\int\limits_1^0\left[f(x)+g(x)\right]\mathrm{\,d}x=-\int\limits_0^1\left[f(x)+g(x)\right]\mathrm{\,d}x=-\int\limits_0^1f(x)\mathrm{\,d}x-\int\limits_0^1g(x)\mathrm{\,d}x=-3+4=1$.
% 		\end{itemchoice}
% 	}
% \end{ex}
...
% 	Cho hàm số $f(x),g(x)$ liên tục trên $\mathbb{R}$.
% 	\choiceTF
% 	{\True Nếu $\int\limits_0^1f(x)\mathrm{\,d}x=-1$ và $\int\limits_0^3f(x)\mathrm{\,d}x=5$ thì $\int\limits_1^3f(x)=6$}
% 	{Nếu $\int\limits_1^2f(x)\mathrm{\,d}x=-3$ và $\int\limits_2^3f(x)\mathrm{\,d}x=4$ thì $\int\limits_1^3f(x)\mathrm{\,d}x=-1$}
% 	{Nếu $\int\limits_{-1}^0f(x)\mathrm{\,d}x=3, \int\limits_{0}^3f(x)\mathrm{\,d}x=1$ thì $\int\limits_{-1}^3f(x)\mathrm{\,d}x=-4$}
% 	{Nếu $\int\limits_{-2}^{5}f(x)\mathrm{\,d}x=8$ và $\int\limits_5^{-2}g(x)\mathrm{\,d}x=3$ thì $\int\limits_{-2}^5\left(f(x)-4g(x)-1\right)\mathrm{\,d}x=-13$}
% 	\loigiai{
% 		\begin{itemchoice}
% 			\itemch Đúng. Ta có 
% 			$\int\limits_0^3f(x)\mathrm{\,d}x =\int\limits_0^1f(x)\mathrm{\,d}x +\int\limits_1^3f(x)\mathrm{\,d}x$.\\
% 			Do đó $\int\limits_1^3f(x)\mathrm{\,d}x =\int\limits_0^3f(x)\mathrm{\,d}x-\int\limits_0^1f(x)\mathrm{\,d}x = 5+ 1= 6$.
% 			\itemch Sai. Ta có $\int\limits_1^3f(x)\mathrm{\,d}x=\int\limits_1^2f(x)\mathrm{\,d}x+\int\limits_2^3f(x)\mathrm{\,d}x=-3+4=1$.
% 			\itemch Sai. Ta có $\int\limits_{-1}^0f(x)\mathrm{\,d}x=3;\int\limits_{0}^3f(x)\mathrm{\,d}x=1;\int\limits_{-1}^3f(x)\mathrm{\,d}x=\int\limits_{-1}^0f(x)\mathrm{\,d}x+\int\limits_{0}^3f(x)\mathrm{\,d}x=3+1=4$.
% 			\itemch Sai. Ta có 
% 			\begin{eqnarray*}
% 				&&\int\limits_{-2}^5\left[f(x)-4g(x)-1\right]\mathrm{\,d}x\\
% 				&&=\int\limits_{-2}^5f(x)\mathrm{\,d}x-\int\limits_{-2}^54g(x)\mathrm{\,d}x-\int\limits_{-2}^5\mathrm{\,d}x\\
% 				&&=\int\limits_{-2}^5f(x)\mathrm{\,d}x-4\int\limits_{-2}^5g(x)\mathrm{\,d}x-\int\limits_{-2}^5\mathrm{\,d}x\\
% 				&&=\int\limits_{-2}^5f(x)\mathrm{\,d}x+4\int\limits_5^{-2}g(x)\mathrm{\,d}x-\int\limits_{-2}^5\mathrm{\,d}x\\
...
% 	Cho hàm số $f(x),g(x)$ liên tục trên $\mathbb{R}$.
% 	\choiceTF
% 	{\True Biết $\int\limits_1^2f(x)\mathrm{\,d}x=2$. Giá trị của  $\int\limits_2^13f(x)\mathrm{\,d}x=-6$}
% 	{Biết $\int\limits_1^2f(x)\mathrm{\,d}x=-1$ và $\int\limits_1^2g(x)\mathrm{\,d}x=3$, khi đó $\int\limits_2^1\left[f(x)-g(x)\right]\mathrm{\,d}x=5$}
% 	{\True Nếu $\int\limits_1^2f(x)\mathrm{\,d}x=-2$ và $\int\limits_2^3f(x)\mathrm{\,d}x=1$ thì $\int\limits_1^3f(x)\mathrm{\,d}x=-1$}
% 	{\True Nếu $\int\limits_0^2(f(x)+3x^2)\mathrm{\,d}x=10$ thì $\int\limits_0^2f(x)\mathrm{\,d}x=2$}
% 	\loigiai{
% 		\begin{itemchoice}
% 			\itemch Đúng. Biết $\int\limits_1^2f(x)\mathrm{\,d}x=2$. Giá trị của $\int\limits_2^13f(x)\mathrm{\,d}x=-6$.\\
% 			Ta có $\int\limits_2^13f(x)\mathrm{\,d}x=-\int\limits_1^23f(x)\mathrm{\,d}x=-3\int\limits_1^2f(x)\mathrm{\,d}x=-3\cdot 2=-6$.
% 			\itemch Sai. Biết $\int\limits_1^2f(x)\mathrm{\,d}x=-1$ và $\int\limits_1^2g(x)\mathrm{\,d}x=3$.\\
% 			Ta có $\int\limits_1^2f(x)\mathrm{\,d}x=-1\Leftrightarrow \int\limits_2^1f(x)\mathrm{\,d}x=1$ và $\int\limits_1^2g(x)\mathrm{\,d}x=3\Leftrightarrow \int\limits_2^1g(x)\mathrm{\,d}x=-3$.\\
% 			Do vậy,  $\int\limits_2^1\left[f(x)-g(x)\right]\mathrm{\,d}x=\int\limits_2^1f(x)\mathrm{\,d}x-\int\limits_2^1g(x)\mathrm{\,d}x=1-(-3)=4$.
% 			\itemch Đúng. Nếu $\int\limits_1^2f(x)\mathrm{\,d}x=-2$ và $\int\limits_2^3f(x)\mathrm{\,d}x=1$ thì $\int\limits_1^3f(x)\mathrm{\,d}x=-1$.\\
% 			Ta có $\int\limits_1^3f(x)\mathrm{\,d}x=\int\limits_1^2f(x)\mathrm{\,d}x+\int\limits_2^3f(x)\mathrm{\,d}x=-2+1=-1$.
% 			\itemch Đúng. Ta có
% 			\begin{eqnarray*}
% 				&&\int\limits_0^2(f(x)+3x^2)\mathrm{\,d}x=10\\
% 				&\Leftrightarrow&\int\limits_0^2f(x)\mathrm{\,d}x+\int\limits_0^23x^2\mathrm{\,d}x=10\\
% 				&\Leftrightarrow&\int\limits_0^2f(x)\mathrm{\,d}x=10-\int\limits_0^23x^2\mathrm{\,d}x\\
% 				&\Leftrightarrow&\int\limits_0^2f(x)\mathrm{\,d}x=10-x^3\bigg|_0^2\\
% 				&\Leftrightarrow&\int\limits_0^2f(x)\mathrm{\,d}x=10-8=2.	\end{eqnarray*}
% 		\end{itemchoice}
% 	}
% \end{ex}
\Closesolutionfile{ans}
...
\begin{ex}%[2D4H2-1]
	...
	\shortans{$12$}
	\loigiai{
		Ta có $\int\limits_0^3 3 f(x){d}x=3\int\limits_0^3 f(x){d}x=12$.}
\end{ex}
\begin{ex}%[2D4H2-1]
	Cho $\int\limits_1^3f(x)\mathrm{\,d}x=2$. Tính $I=\int\limits_1^3\left[f(x)+2x\right]\mathrm{\,d}x$.\\
	\shortans{$10$}
	\loigiai{
		Ta có$\colon $ $\int\limits_1^3\left[f(x)+2x\right]\mathrm{\,d}x=\int\limits_1^3f(x)\mathrm{\,d}x+\int\limits_1^32x\mathrm{\,d}x=2+\left.x^2\right|_1^3=2+3^2-1^2=10$.}
\end{ex}
...
\begin{ex}%[2D4H2-1]
	...
	\shortans{$2{,}5$}
	\loigiai{
		Ta có $\int\limits_{-1}^2\left[x+2f(x)+3g(x)\right]\mathrm{\,d}x=\int\limits_{-1}^2x\mathrm{\,d}x+2\int\limits_{-1}^2f(x)\mathrm{\,d}x+3\int\limits_{-1}^2g(x)\mathrm{\,d}x=\dfrac{3}{2}+4-3=\dfrac{5}{2}=2{.}5$.}
\end{ex}
...
\begin{ex}%[2D4H2-1]
	...
	\shortans{$1$}
	\loigiai{
		$\int\limits_0^1\left[2f(x)-3x^2\right]\mathrm{\,d}x=2\int\limits_0^1f(x)\mathrm{\,d}x-3\int\limits_0^1x^2\mathrm{\,d}x=2-1=1$.}
\end{ex}
...
\begin{ex}%[2D4H2-1]
	...
	\shortans{$-6$}
	\loigiai{
		Ta có $\int\limits_3^12f(x)\mathrm{\,d}x=-\int\limits_1^32f(x)\mathrm{\,d}x=-2\int\limits_1^3f(x)\mathrm{\,d}x=-2\cdot3=-6$.}
\end{ex}

% \begin{ex}%[2D4H2-1]
% 	Biết $\int\limits_0^1f(x)\mathrm{\,d}x=-2$ và $\int\limits_1^0g(x)\mathrm{\,d}x=-3$.. Tính $ I=\int\limits_0^1\left[f(x)-g(x)\right]\mathrm{\,d}x$.\\
% 	\shortans{$-5$}
% 	\loigiai{
% 		$\int\limits_0^1\left[f(x)-g(x)\right]\mathrm{\,d}x=\int\limits_0^1f(x)\mathrm{\,d}x-\int\limits_0^1g(x)\mathrm{\,d}x=-2-3=-5$.}
% \end{ex}

% \begin{ex}%[2D4H2-1]
% 	Biết $\int\limits_1^2f(x)\,\mathrm{\,d}x=3$ và $\int\limits_1^2g(x)\mathrm{\,d}x=2$ và $\int\limits_1^2h(x)\mathrm{\,d}x=2022$. Tính $\linebreak I=\int\limits_1^2\left[f(x)-g(x)+h(x)\right]\mathrm{\,d}x$.\\
% 	\shortans{$2023$}
% 	\loigiai{
% 		Ta có $\int\limits_1^2\left[f(x)-g(x)+h(x)\right]\,\mathrm{\,d}x=\int\limits_1^2f(x)\,\mathrm{\,d}x-\int\limits_1^2g(x)\mathrm{\,d}x+\int\limits_1^2h(x)\mathrm{\,d}x$\\
% 		$=3-2+2022=2023$.}
% \end{ex}

% \begin{ex}%[2D4H2-1]
% 	Cho $\int\limits_{-1}^2f(x)\mathrm{\,d}x=2$ và $\int\limits_2^5f(x)\mathrm{\,d}x=-5$. Tính $ I=\int\limits_{-1}^5f(x)\mathrm{\,d}x$.\\
% 	\shortans{$-3$}
% 	\loigiai{
% 		Ta có $\int\limits_{-1}^5f(x)\mathrm{\,d}x=\int\limits_{-1}^2f(x)\mathrm{\,d}x+\int\limits_2^5f(x)\mathrm{\,d}x=2-5=-3$.}
% \end{ex}

% \begin{ex}%[2D4H2-1]
% 	Cho $ f$, $ g$ là hai hàm liên tục trên đoạn $\left[1;\,3\right]$ thoả$\colon $ $\int\limits_1^3\left[f(x)+3g(x)\right]\mathrm{\,d}x=10$, $\int\limits_1^3\left[2f(x)-g(x)\right]\mathrm{\,d}x=6$. Tính $I=\int\limits_1^3\left[f(x)+g(x)\right]\mathrm{\,d}x$.\\
% 	\shortans{$6$}
% 	\loigiai{
% 		Đặt $ a=\int\limits_1^3f(x)\mathrm{\,d}x$ và $ b=\int\limits_1^3g(x)\mathrm{\,d}x$.\\
% 		Khi đó, $\int\limits_1^3\left[f(x)+3g(x)\right]\mathrm{\,d}x=a+3b$, $\int\limits_1^3\left[2f(x)-g(x)\right]\mathrm{\,d}x=2a-b$.\\
...
% \begin{ex}%[2D4H2-1]
% 	Cho hàm số $ f(x)$ liên tục trên $\mathbb{R}$ thoả mãn $\int\limits_1^8f(x)\,\mathrm{\,d}x=9$, $\int\limits_4^{12}{f(x)}\,\mathrm{\,d}x=3$, $\int\limits_4^8f(x)\,\mathrm{\,d}x=5$. Tính $ I=\int\limits_1^{12}{f(x)}\,\mathrm{\,d}x$.\\
% 	\shortans{$7$}
% 	\loigiai{
% 		Ta có $ I=\int\limits_1^{12}{f(x)}\,\mathrm{\,d}x=\int\limits_1^8f(x)\,\mathrm{\,d}x+\int\limits_8^{12}{f(x)}\,\mathrm{\,d}x$ $=\int\limits_1^8f(x)\,\mathrm{\,d}x+\int\limits_4^{12}{f(x)}\,\mathrm{\,d}x-\int\limits_4^8f(x)\,\mathrm{\,d}x$\\$=9+3-5=7$.}
% \end{ex}

% \begin{ex}%[2D4H2-1]
% 	Cho hàm số $ f(x)$ liên tục trên $\left[0;10\right]$ thỏa mãn $\int\limits_0^{10}{f(x)\mathrm{\,d}x}=7$, $\int\limits_2^6f(x)\mathrm{\,d}x=3$. Tính $ P=\int\limits_0^2f(x)\mathrm{\,d}x+\int\limits_6^{10}{f(x)\mathrm{\,d}x}$.\\
% 	\shortans{$4$}
% 	\loigiai{
% 		Ta có $\int\limits_0^{10}{f(x)\mathrm{\,d}x}=\int\limits_0^2f(x)\mathrm{\,d}x+\int\limits_2^6f(x)\mathrm{\,d}x+\int\limits_6^{10}{f(x)\mathrm{\,d}x}$\\
% 		Suy ra $\int\limits_0^2f(x)\mathrm{\,d}x+\int\limits_6^{10}{f(x)\mathrm{\,d}x}=\int\limits_0^{10}{f(x)\mathrm{\,d}x}-\int\limits_2^6f(x)\mathrm{\,d}x=7-3=4$.}
% \end{ex}

% \begin{ex}%[2D4H2-1]
% 	Giả sử $\int\limits_0^1f(x)\mathrm{\,d}x=3$ và $\int\limits_0^5f(z)\mathrm{\,d}z=9$. Tổng $I=\int\limits_1^3f(t)\mathrm{\,d}t+\int\limits_3^5f(t)\mathrm{\,d}t$ bằng\\
% 	\shortans{$6$}
% 	\loigiai{
% 		$\int\limits_0^1f(x)\mathrm{\,d}x=3\Leftrightarrow\int\limits_0^1f(t)\mathrm{\,d}t=3\Leftrightarrow\int\limits_1^0f(t)\mathrm{\,d}t=-3.$\\
% 		$\int\limits_0^5f(z)\mathrm{\,d}z=9\Leftrightarrow\int\limits_0^5f(t)\mathrm{\,d}t=9.$\\
% 		$\Rightarrow\int\limits_1^0f(t)\mathrm{\,d}t+\int\limits_0^5f(t)\mathrm{\,d}t=6\Leftrightarrow\int\limits_1^5f(t)\mathrm{\,d}t=6.$\\
% 		$I=\int\limits_1^3f(t)\mathrm{\,d}t+\int\limits_3^5f(t)\mathrm{\,d}t=\int\limits_1^5f(t)\mathrm{\,d}t=6.$}
% \end{ex}
\Closesolutionfile{ans}
...
\begin{ex}%[2D4H2-4]%Câu 6
	Biết $\displaystyle\int\limits_0^1\dfrac{\mathrm{e}^x}{2^x}\mathrm{\,d}x=\dfrac{\mathrm{e-2}}{a-\ln b }$, $\left(a,b\in\mathbb{Z}\right)$. Khi đó giá trị của $ P=a+b$ là
	\choice
	{$ P=-3$}
	{\True $ P=6$}
	{$ P=-1$}
	{$ P=3$}
	\loigiai{
		$ I=\displaystyle\int\limits_0^1\dfrac{\mathrm{e}^x}{2^x}\mathrm{\,d}x=\displaystyle\int\limits_0^1\left(\dfrac{\mathrm{e}}{2}\right)^x\mathrm{\,d}x=\left[\left(\dfrac{\mathrm{e}}{2}\right)^x\cdot\dfrac{1}{1-\ln 2}\right]\Big|_0^1=\dfrac{\mathrm{e}-2}{2-\ln 4}$.}
\end{ex}
...
\begin{ex}%[2D4H2-1]%Câu 15
	...
	\choiceTF
	{$\int\limits_a^b{f(x)\mathrm{\,d}x}=\int\limits_b^a{f(x)\mathrm{\,d}x}$}
	{\True $\int\limits_a^b{f(x)\mathrm{\,d}x}=-\int\limits_b^a{f(x)\mathrm{\,d}x}$}
	{$\int\limits_a^bf(x)\mathrm{\,d}x=2\int\limits_a^bf(x)\mathrm{\,d}\left(2x\right)$}
	{\True $\int\limits_a^a{2024f(x)\mathrm{\,d}x=0}$}
	\loigiai{
		\begin{itemchoice}
			\itemch Sai. Vì
			$\int\limits_a^b{f(x)\mathrm{\,d}x}=-\int\limits_b^a{f(x)\mathrm{\,d}x}$.
			\itemch Đúng. Vì $\int\limits_a^b{f(x)\mathrm{\,d}x}=-\int\limits_b^a{f(x)\mathrm{\,d}x}$.
			\itemch Sai. Vì $2\int\limits_a^bf(x)\mathrm{\,d}\left(2x\right)=4\int\limits_a^bf(x)\mathrm{\,d}\left(x\right)$.
			\itemch Đúng. 
			$\int\limits_a^a2024f(x)\mathrm{\,d}x=0.$
		\end{itemchoice}
	}
\end{ex}
%
\begin{ex}%[2D4H2-1]%Câu 16
	...
	\choiceTF
	{\True $\int\limits_a^b{\left[f(x)+g(x)\right]\mathrm{\,d}x}=\int\limits_a^b{f(x)}\mathrm{\,d}x+\int\limits_a^b{g(x)\mathrm{\,d}x}$}
	{$\int\limits_a^b{f(x)\cdot g(x)\mathrm{\,d}x}=\int\limits_a^b{f(x)\mathrm{\,d}x}\cdot\int\limits_a^b{g(x)\mathrm{\,d}x}$}
	{\True $\int\limits_a^b{kf(x)\mathrm{\,d}x=k\int\limits_a^b{f(x)\mathrm{\,d}x}}$}
	{$\int\limits_a^b{\dfrac{f(x)}{g(x)}\mathrm{\,d}x}=\dfrac{\int\limits_a^bf(x)\mathrm{\,d}x}{\int\limits_a^bg(x)\mathrm{\,d}x}$}
	\loigiai{
		\begin{itemchoice}
			\itemch Đúng.
			...
			\itemch Đúng.
			$\int\limits_a^b{kf(x)\mathrm{\,d}x=k\int\limits_a^b{f(x)\mathrm{\,d}x}}$.
			\itemch Sai.
	\end{itemchoice}}
\end{ex}
%
...
% 	\choiceTF
% 	{$\int\limits_a^c{f(x)\mathrm{\,d}x=\int\limits_a^b{f(x)\mathrm{\,d}x}}\cdot \int\limits_b^c{f(x)\mathrm{\,d}x}$}
% 	{\True $\int\limits_a^c{f(x)\mathrm{\,d}x=\int\limits_a^b{f(x)\mathrm{\,d}x}}+\int\limits_b^c{f(x)\mathrm{\,d}x}$}
% 	{$\int\limits_a^c{f(x)\mathrm{\,d}x=\int\limits_a^b{f(x)\mathrm{\,d}x}}-\int\limits_b^c{f(x)\mathrm{\,d}x}$}
% 	{$\int\limits_a^c{f(x)\mathrm{\,d}x=\int\limits_a^b{f(x)\mathrm{\,d}x}}+\int\limits_c^b{f(x)\mathrm{\,d}x}$}
% 	\loigiai{\begin{itemchoice}
% 			\itemch Sai. Không đúng với lý thuyết.
% 			\itemch Đúng. $\int\limits_a^c{f(x)\mathrm{\,d}x=\int\limits_a^b{f(x)\mathrm{\,d}x}}+\int\limits_b^c{f(x)\mathrm{\,d}x}$.
% 			\itemch Sai.
% 			\itemch Sai.
% 	\end{itemchoice}}
% \end{ex}
...
% 	\choiceTF
% 	{\True $\int\limits_a^bf(x)\mathrm{\,d}x=\int\limits_a^bf(y)\mathrm{\,d}y$}
% 	{\True $\int\limits_a^b{\left(f(x)+g(x)\right)\mathrm{\,d}x}=\int\limits_a^b{f(x)\mathrm{\,d}x+\int\limits_a^b{g(x)\mathrm{\,d}x}}$}
% 	{$\int\limits_a^b{f(x)\mathrm{\,d}x=\int\limits_a^b{f(t)\mathrm{\,d}x}}$}
% 	{$\int\limits_a^b{\left(f(x)g(x)\right)\mathrm{\,d}x}=\int\limits_a^b{f(x)\mathrm{\,d}x\int\limits_a^b{g(x)\mathrm{\,d}x}}$}
% 	\loigiai{
% 		\begin{itemchoice}
% 			\itemch Đúng. $\int\limits_a^b{f(x)\mathrm{\,d}x=\int\limits_a^b{f(y)\mathrm{\,d}}y}$
% 			\itemch Đúng. $\int\limits_a^b{\left(f(x)+g(x)\right)\mathrm{\,d}x}=\int\limits_a^bf(x)\mathrm{\,d}x+\int\limits_a^b g(x)\mathrm{\,d}x$.
% 			\itemch Sai. Không đúng với lý thuyết.
% 			\itemch Sai. Không đúng với lý thuyết.
% 		\end{itemchoice}
% 	}
...
% 	\choiceTF
% 	{\True $\int\limits_{-2024}^{2024}\mathrm{\,d}x=4048$}
% 	{$\int\limits_a^bf_1(x)\cdot f_2(x)\mathrm{\,d}x=\int\limits_a^bf_1(x)\mathrm{\,d}x\cdot\int\limits_a^bf_2(x)\mathrm{\,d}x$}
% 	{\True Cho hàm số $f(x)$ liên tục trên đoạn $\left[a;b\right]$. Khi đó $\dfrac{1}{b-a}\int\limits_a^bf(x)\mathrm{\,d}x$ được gọi là giá trị trung bình của hàm số $f(x)$ trên đoạn $\left[a;b\right]$}
% 	{\True Nếu hàm số $f(x)$ có đạo hàm $f'(x)$ và $f'(x)$ liên tục trên đoạn $\left[a;b\right]$ thì $f(b)-f(a)=\int\limits_a^bf'(x)\mathrm{\,d}x$}
% 	\loigiai{\begin{itemchoice}
% 			\itemch Đúng.
% 			\itemch Sai. 
% 			\itemch Đúng.
...
\begin{ex}%[2D4H2-1]%Câu 20
	...
	\choiceTF
	{\True $\int\limits_a^b{kf(x)\mathrm{\,d}x}=k\left[F(b)-F(a)\right]$}
	{$\int\limits_b^af(x)\mathrm{\,d}x=F(b)-F(a)$}
	...
	{$\int\limits_a^b{f\left(2x+3\right)\mathrm{\,d}x}=F\left(2x+3\right)\big|_a^b$}
	\loigiai{
		\begin{itemchoice}
			\itemch Đúng.
			\itemch Sai. $\int\limits_b^a{f(x)\mathrm{\,d}x}=F(a)-F(b)$. 
			...
			\itemch Sai. $\int\limits_a^bf\left(2x+3\right)\mathrm{\,d}x=\dfrac12 F\left(2x+3\right)\big|_a^b$
		\end{itemchoice}
	}
\end{ex}
%
...
\begin{dang}{Tích phân hàm chứa trị tuyệt đối}
	Tính tích phân $I=\int\limits_a^b|f(x)| \mathrm{\,d}x$?\\
	\textbf{Phương pháp}
	\begin{itemize}
		\item \textbf{Bước 1.} Xét dấu $f(x)$ trên đoạn $[a ; b]$.
		\item \textbf{Bước 2.} Dựa vào bảng xét dấu trên đoạn $[a ; b]$ để khử $|f(x)|$. Sau đó sử dụng các phương pháp tính tích phân đã học để tính $I=\int\limits_a^b|f(x)| \cdot \mathrm{\,d}x$.
	\end{itemize}
\end{dang}
