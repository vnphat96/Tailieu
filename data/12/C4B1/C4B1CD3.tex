\newpage
\subsection{Nguyên hàm hàm ẩn biến đổi phức tạp}

\begin{khung4}{Công thức đạo hàm của hàm hợp}
	\begin{listEX}[2]%[\faPencilSquare]
...
\begin{dang}{Nguyên hàm của một tích, thương}
	$\bullet$ $f'(x)=g(x) \cdot h[f(x)] \Leftrightarrow \displaystyle\int \dfrac{f'(x)}{h[f(x)]}\mathrm{\,d}x =\int  {g(x)}\mathrm{\,d}x \Leftrightarrow \int\dfrac{\mathrm{\,d}\left[ f(x) \right]}{h\left[ f(x) \right]} =\int  {g(x)\mathrm{\,d}x}$.\\
	$\bullet$ $f'(x) = \dfrac{g(x)}{h[f(x)]} \Leftrightarrow \displaystyle \int f'(x)h[f(x)]\mathrm{\,d}x=\int g(x)\mathrm{\,d}x \Leftrightarrow \int h[f(x)]\mathrm{\,d}\left[ f(x) \right]=\int g(x)\mathrm{\,d}x$.\\
	$\bullet$ $u(x)f'(x)+u'(x)f(x)=h(x) \Leftrightarrow [u(x)f(x)]'=h(x) \Leftrightarrow \displaystyle\int h(x)\mathrm{\,d}x=u(x)f(x)+C$.
\end{dang}
...
%%%==============EX_1============%%%
\begin{ex}%[2D4V1-3]
	Cho hàm số $f(x)$ thỏa mãn $f\left(\dfrac{\pi}{4} \right)=0$ và $f'(x) \cdot \sin^2\dfrac{x}{2}\cos^2\dfrac{x}{2}=1$. Tính $f\left(\dfrac{\pi}{2} \right)$.
	\choice
	...
	\loigiai{
		$f'(x) \cdot \sin^2\dfrac{x}{2}\cos^2\dfrac{x}{2}=1
			\Rightarrow f'(x)=\dfrac{1}{\sin^2\dfrac{x}{2}\cos^2\dfrac{x}{2}} =\dfrac{1}{\dfrac{1}{4}\sin^2x} $\\
		$\Rightarrow f(x)=4\displaystyle\int  \dfrac{1}{\sin^2x}\mathrm{\,d}x=-4\cot x+C$.\\
...
\begin{ex}%[2D4V1-2]
	...
	\loigiai{
		...
		$\begin{aligned}[t]
			...
			\end{aligned}$\\
		Do $f(0)=2 \Rightarrow \dfrac{f^2(0)}{2}=\dfrac{0^5}{5}+\dfrac{0^3}{3}+C \Rightarrow C=2$.\\
		...
		Do đó $f(x)$ là hàm không giảm trên đoạn $[1;2]$, ta có $f(x)\le f(2) < 0$ với mọi $x\in [1;2]$\\
			      $\begin{aligned}[t]
					      f'(x)=x\left[f(x)\right]^2
					       ...
					       & \Rightarrow \left(\dfrac{1}{f(x)} \right)'=-x       \\
					       & \Rightarrow \dfrac{1}{f(x)}=\displaystyle\int(-x)dx =-\dfrac{x^2}{2}+C.
				      \end{aligned}$\\
			      Mà
			      $\begin{aligned}[t]
					      f(2)=-\dfrac{1}{3}
...
\begin{dang}{Phương trình vi phân \boldmath $A(x)f(x)+B(x)f'(x)=h(x)$ \unboldmath}
