...
\begin{ex}%[2D4H2-4]
	Biết rằng $\displaystyle\int (3^{x}+5^{x})^2\mathrm{\,d}x=\dfrac{9^x}{a\ln 3}+2\cdot \dfrac{15^x}{b\ln 5+c\ln 2+d\ln 3}+\dfrac{25^x}{e\ln 5}+C$. Tính giá trị của biểu thức $P=a+b+c+d+e$.
	\shortans[4]{$7$}
	\loigiai{
		\begin{eqnarray*}
			\displaystyle\int (3^{x}+5^{x})\mathrm{\,d}x&=&\displaystyle\int (9^{x}+2 \cdot 15^{x}+25^{x})\mathrm{\,d}x\\
			&=&
			\dfrac{9^x}{\ln 9}+2\cdot \dfrac{15^x}{\ln 30} +\dfrac{25^x}{\ln 25}+C\\
			&=&\dfrac{9^x}{2\ln 3}+2\cdot \dfrac{15^x}{\ln 5+\ln 2+\ln 3}+\dfrac{25^x}{2\ln 5}+C.
		\end{eqnarray*}
