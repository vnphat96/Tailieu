...
\begin{enumerate}[\iconMT]
	...
	\item \indam{Định lý:} Giả sử hàm số $y=f(x)$ liên tục trên khoảng $(a ; b)$ chứa điểm $x_0$ và có đạo hàm trên các khoảng $\left(a ; x_0\right)$ và $\left(x_0 ; b\right)$. Khi đó:
	      \begin{boxdn}
		      \begin{itemize}
			      \item [$\bullet$] Nếu $f'(x)<0$ với mọi $x \in\left(a ; x_0\right)$ và $f'(x)>0$ với mọi $x \in\left(x_0 ; b\right)$ thì $x_0$ là một điểm cực tiểu của hàm số $f(x)$.
			      \item [$\bullet$] Nếu $f'(x)>0$ với mọi $x \in\left(a ; x_0\right)$ và $f'(x)<0$ với mọi $x \in\left(x_0 ; b\right)$ thì $x_0$ là một điểm cực đại của hàm số $f(x)$.
		      \end{itemize}
	      \end{boxdn}
\newpage
...
% \begin{note}
	\begin{enumerate}[\iconCH]
		...
		\end{itemize}
		\item Khi xét dấu $f'(x)$ thì $f'(x)$ sẽ không đổi dấu khi qua nghiệm kép (và nghiệm bội chẵn) và đổi dấu khi qua nghiệm đơn (và nghiệm bội lẻ).
	\end{enumerate}
...
\begin{vd}
	...
	\loigiai{
		\begin{enumEX}[a)]{1}
			...
			$y'=\dfrac{x^2+2x}{(x+1)^2}$, $y'=0\Leftrightarrow \hoac{& x=-2 \\ & x=0.}$\\
...
\begin{ex}
	...
	{$6$}
	\loigiai{
	Ta có $y'=\dfrac{x^2+4x+3}{(x+2)^2}$; $y'=0 \Leftrightarrow \hoac{&x=-1 \\ &x=-3} $. \\
	Bảng biến thiên
	\begin{center}
		\begin{tikzpicture}
			\tkzTab
