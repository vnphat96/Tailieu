...
\end{ex}
\begin{ex}%[2D1Y2-2]
    \immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình. Mệnh đề nào {\bf đúng}?
        \choice
...
\end{ex}
\begin{ex}%[2D1K2-2]
    \immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình vẽ bên dưới. Hỏi đồ thị hàm số $y=\left|f(x)\right|$ có bao
        nhiêu điểm cực trị?
        \choice
        {\True $5$}
        {$2$}
...
\end{ex}
\begin{ex}%[2D1K2-2]
    \immini[thm]{Đồ thị hàm số $y=f(x)$ liên tục trên $\mathbb{R}$ và có đồ thị như hình vẽ. Hỏi đồ thị hàm số $y=f\left(|x|\right)$ có tất cả bao nhiêu điểm cực trị?
        \choice
        {$2$}
        {$3$}
        {$4$}
...
\end{ex}
\begin{ex}%[2D1K2-2]
    \immini[thm]{Cho hàm số bậc bốn $y=f(x)$ có đồ thị hình vẽ bên. Số điểm cực trị của hàm số $g(x)=f(x^3-3x)$ là
        \choice
        {$7$}
        {\True $9$}
        {$11$}
...
\end{ex}
\begin{ex}%[2D1K2-6]
    \immini[thm]{Cho hàm số $f=f(x)$ có đạo hàm trên $\mathbb{R}$. Đồ thị hàm số $y=f'(x)$ như hình vẽ. Tìm tất cả các giá trị của tham số $m$ để hàm số $y=f(x)-mx$ có $3$ điểm cực trị?
        \choice
        {\True $0<m<4$}
        {$0\le m\le4$}
        {$m>4$}
...
\end{ex}
\begin{ex}%[2D1K2-2]
    \immini[thm]{Cho hàm số $y=f(x)$ có đạo hàm trên $\mathbb{R}$, $f(-2)=0$. Biết  hàm $y=f'(x)$ có đồ thị như hình vẽ bên. Số điểm cực tiểu của hàm  số $y=g(x)=\left|4f\left(x \right)-x^2+4  \right| $  là
        \choice
        {$1$}
        {\True $3$}
        {$4$}
        {$2$}}{\begin{tikzpicture}[>=stealth,line join=round,line cap=round,font=\footnotesize,scale=.5]
    ...
    \end{tikzpicture}}
    \loigiai{
        \immini[thm]{Đặt $h(x)=4f\left(x \right)-x^2+4$, ta có $h'(x)=4f'(x)-2x$;
            ...
            Dựa vào đồ thị ta có phương trình $f'(x)=\dfrac{x}{2}$ có $3$ nghiệm phân biệt là $-2$, $0$, $4$ và vị trí tương đối giữa hai đồ thị ta suy ra bảng biến thiên của $h(x)$ như sau}{\begin{tikzpicture}[>=stealth,line join=round,line cap=round,font=\footnotesize,scale=1]
                \begin{scope}[xscale=0.7]
                    \draw[->] (-3.4,0)--(6,0)node[below]{$x$};
                    \draw[->](0,-1.5)--(0,3)node[right]{$y$};
                    \draw[domain=-3.5:5.5,samples=100] plot (\x,{-.07*(\x)^3+.14*(\x)^2+ 1.08*(\x) });
