...
\begin{ex}%[2D1Y5-1]
    \immini[thm]{
        ...
        {\True $y=x^3-3x+1$}
        {$y=-x^3+3x+1$}
        {$y=x^3-3x^2+1$}}{\begin{tikzpicture}[scale=0.7, font=\footnotesize, line join=round, line cap=round, >=stealth]
...
\end{ex}
\begin{ex}%[2D1B5-1]
    \immini[thm]{Cho hàm số $ y=ax^3+bx^2+cx+d$ có đồ thị như hình bên. Trong các giá trị $ a$, $b$, $c$, $d$ có bao nhiêu giá trị âm.
        \choice
        {\True $ 2 $}
        {$ 1 $}
        {$ 4 $}
...
\end{ex}
\begin{ex}%[2D1K5-1]
    \immini[thm]{Cho hàm số $y=(a-1)x^4+(b+2)x^2+c-1$ có đồ thị như hình vẽ bên. Mệnh đề nào dưới đây đúng?
        \choice
        {$a>1,b>-2,c>1$}
        {\True $a>1,b<-2,c>1$}
        {$a<1,b>-2,c>1$}
...
\end{ex}
\begin{ex}%[2D1B5-3]
    \immini[thm]{Cho hàm số $y=f(x)$ có bảng biến thiên như hình bên. Số nghiệm của phương trình $f(x)=f(0)$ là
        \choice
        {$2$}
        {\True $3$}
        {$1$}
...
\end{ex}
\begin{ex}%[2D1B5-3]
    \immini[thm]{	Cho đồ thị hàm số $y=f(x)$ như hình bên dưới. Tìm tất cả các giá trị của tham số $m$ để phương trình $f(x)=m$ có đúng $3$ nghiệm phân biệt thuộc đoạn $[-2;1]$.
        \choice
        {$-2<m<0$}
        {$-2\le m<1$}
        {\True $-2<m\le 1$}
    ...
    \loigiai{
        \immini[thm]{	Phương trình $f(x)=m$ có đúng $3$ nghiệm phân biệt thuộc đoạn $[-2;1]$ khi và chỉ khi đường thẳng $y=m$ cắt phần đồ thị ứng với $x\in [-2;1]$ của đồ thị hàm số $y=f(x)$ tại đúng $3$ điểm phân biệt.\\
...
\end{ex}
\begin{ex}%[2D1B5-3]
    \immini[thm]{Cho hàm số bậc ba $f(x)$ có đồ thị như hình vẽ. Số giá trị nguyên của tham số $m$ để phương trình $f(x)+1=m$ có ba nghiệm phân biệt là
        \choice
        {\True $3$}
        {$5$}
        {$2$}
        {$4$}}{\begin{tikzpicture}[scale=0.5, font=\footnotesize, line join=round, line cap=round, >=stealth]
...
\end{ex}
\begin{ex}%[2D1B5-3]
    \immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình vẽ bên.
