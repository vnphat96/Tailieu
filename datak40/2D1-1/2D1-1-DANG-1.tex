...
\end{ex}
\begin{ex}%[2D1B1-2]
 \immini[thm]{Cho hàm số $y=f(x)$ có đồ thị $y=f'(x)$ là một parabol được cho như hình vẽ . Hàm số $y=f(x)$ nghịch biến trên khoảng nào dưới đây?
...
\end{ex}
\begin{ex}%[EX-TF-2024, Lê Quốc Hiệp]%[2D1N1-2]
    \immini[thm]{Cho hàm số $y=f(x)$ có đồ thị như hình vẽ bên. Xét tính đúng, sai của mỗi mệnh đề sau
        \choiceTF
...
\BTTL
\begin{ex}\immini[thm]{Cho hàm số $y=f\left(x\right)$ liên tục trên đoạn $\left[-3;3\right]$ và có đồ thị như hình bên. Tìm các khoảng đơn điệu của hàm số $f\left(x\right)$ trên khoảng $\left(-3;3\right)$.
     \shortans{}
...
\end{ex}
\begin{ex}
    \immini[thm]{Cho hàm số $y=f(x)$ liên tục trên đoạn $[0 ; 3]$ thoả mãn $f'\left(\dfrac{1}{3}\right)=f'(1)=f'\left(\dfrac{5}{2}\right)=0$ và có đồ thị là đường cong như hình bên. Xác định các khoảng đơn điệu và tìm cực trị của hàm số đã cho trên khoảng $(0;3)$.}{\begin{tikzpicture}[line cap=butt,line join=miter,>=stealth,font=\footnotesize,scale=.7]
...
\begin{listEX}[2]
    \item  \begin{tikzpicture}[scale=1,xscale=1.3, font=\footnotesize, line join=round, line cap=round, >=stealth]
...
\end{ex}
\begin{ex}
    \immini{Cho hàm số $y=f(x)$ liên tục trên đoạn $[0 ; 3]$ thoả mãn $f'\left(\dfrac{1}{3}\right)=f'(1)=f'\left(\dfrac{5}{2}\right)=0$ và có đồ thị là đường cong như hình bên. Xác định các khoảng đơn điệu và tìm cực trị của hàm số đã cho trên khoảng $(0;3)$.}{\begin{tikzpicture}[line cap=butt,line join=miter,>=stealth,font=\footnotesize,scale=.7]
            \tikzset{declare function={xmin=-2;xmax=4;ymin=-3;ymax=2;
                    f(\x)=32/45*(\x)^4-32/9*(\x)^3+224/45*(\x)^2-32/15*(\x);
                },
                smooth,samples=450
