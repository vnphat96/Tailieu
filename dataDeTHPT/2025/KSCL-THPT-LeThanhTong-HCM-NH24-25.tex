...
\begin{ex}%[2H2N1-2]%[Dự án C đợt 3 - KSCL LeThanhTong-Võ Hoàng Nghĩa]
	\immini[thm]{Cho hình hộp $ABCD.A'B'C'D'$ có tâm $O$. Khi đó, $\overrightarrow{AB} + \overrightarrow{AD} + \overrightarrow{AA'} + \overrightarrow{AC'}$ bằng
		\choice
		{$\overrightarrow{BD}$}
		{$2\overrightarrow{OC'}$}
...
\begin{ex}%[2D3H1-3]%[Dự án C đợt 3 - KSCL LeThanhTong-Võ Hoàng Nghĩa]
	\immini[thm]{Thời gian (phút) truy cập Internet mỗi buổi tối của một số học sinh được cho trong bảng sau.
...
\begin{ex}%[1D1V1-6]%[Dự án C đợt 3 - KSCL LeThanhTong-Võ Hoàng Nghĩa]
	\immini[thm]{Trên đồng hồ tại thời điểm đang xét kim giờ $OG$ chỉ đúng số $3$, kim phút $OP$ chỉ đúng số $12$. Số đo góc lượng giác mà kim giờ quét được từ lúc xét đến khi kim phút và kim giờ gặp nhau lần đầu tiên bằng
		\choice
		{$\alpha = \dfrac{\pi}{22}$}
		{$\alpha = -\dfrac{2\pi}{45}$}
		{$\alpha=-\dfrac{\pi}{21}$}
...
\begin{ex}%[1H8H6-2]%[Dự án C đợt 3 - KSCL LeThanhTong-Võ Hoàng Nghĩa]
	\immini[thm]{Cho hình chóp $S.ABCD$ có đáy $ABCD$ là hình vuông tâm $O$, đường thẳng $SA$ vuông góc với mặt phẳng đáy và $OC=\sqrt{3}SA$ (tham khảo hình vẽ). Số đo góc phẳng nhị diện $[S,BD,C]$ bằng
		\choice
		{$120^{\circ}$}
		{\True $150^{\circ}$}
		{$30^{\circ}$}
...
\begin{ex}%[2D4V3-1]%[Dự án C đợt 3 - KSCL LeThanhTong-Võ Hoàng Nghĩa]
	\immini[thm]{Cho hàm số $y=f(x)$ có đạo hàm $f'(x)$ liên tục trên đoạn $[0;5]$ và đồ thị hàm số $f'(x)$ trên đoạn $[0;5]$ được cho như hình bên. Mệnh đề nào sau đây đúng?
		\choice
		{$f(0)=f(5)<f(3)$}
		{$f(3)<f(0)=f(5)$}
		{$f(3)<f(0)<f(5)$}
