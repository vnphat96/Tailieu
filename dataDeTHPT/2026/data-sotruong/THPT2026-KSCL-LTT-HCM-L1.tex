...
\begin{ex}%[2H2C2-5]
	\immini[thm]{Trong không gian $Oxyz$, cho bốn điểm $S$, $A$, $B$, $C$ như hình vẽ bên (\textit{mỗi ô lưới là $1$ đơn vị}).
		...
		{\True Xét hình nón $(N)$ có đỉnh $S$, điểm $A$ thuộc đường sinh và hai điểm $B, C$ thuộc đường tròn đáy của $(N)$. Bán kính hình nón bằng $\sqrt{6}$}}{		\begin{tikzpicture}[x={(-0.5cm,-0.4cm)}, y={(0.8cm,0cm)}, z={(0cm,0.8cm)}, scale=0.8, font=\footnotesize, >=stealth]
			% --- Cấu hình ---
