...
\begin{ex}%[Lê Hồng Phi]%[Tex hóa Đề Vted 2023]%[2D4Y1-2]
	\immini[thm]{Trong hình vẽ bên, điểm $M$ biểu diễn số phức $z$. Số phức $\overline{z}$ là 
		\choice
		{$1-2i$}
		{\True $2-i$}
		{$2+i$}
		{$1+2i$} }{\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round,>=stealth,x=0.6cm,y=0.6cm]
			\def \xmin{-1.5};
			\def \xmax{2.7};
			\def \ymin{-1.};
			\def \ymax{1.7};
...
\begin{ex}%[Lê Hồng Phi]%[Tex hóa Đề Vted 2023]%[2H1Y3-2]
	...
	{$6a^3$}
	\loigiai{
		\immini[thm]{Diện tích hình vuông $ABCD$ là $S_{ABCD}=(2a)^2=4a^2$.\\
...
\begin{ex}%[Lê Hồng Phi]%[Tex hóa Đề Vted 2023]%[2D1B5-5]
	\immini[thm]{Cho hàm số $f(x)$ có đồ thị của đạo hàm như hình vẽ bên. Hàm số $f(x)$ đồng biến trên khoảng nào dưới đây?
		\choice
		{$(1;3)$}
		{\True $(-\infty;0)$}
		{$(0;1)$}
...
\begin{ex}%[Lê Hồng Phi]%[Tex hóa Đề Vted 2023]%[2H3Y3-1]
	Trong không gian  $Oxyz$, cho đường thẳng $d\colon \dfrac{x-1}{2}=\dfrac{z+1}{-1}=\dfrac{y-2}{3}$. Một vectơ chỉ phương của $d$ là
	\choice
	...
	\loigiai{
		Một vectơ chỉ phương của $d$ là $\overrightarrow{u}_1=(2;-1;3)$.
	}
\end{ex}
...
\begin{ex}%[Lê Hồng Phi]%[Tex hóa Đề Vted 2023]%[2H3Y1-2]
	Trong không gian  $Oxyz$, cho hai vectơ $\overrightarrow{u}=(1;-2;3)$ và $\overrightarrow{v}=(2;-2;1)$. Khi đó $\overrightarrow{u}\cdot\overrightarrow{v}$ bằng
	\choice
	{\True $9$}
	{$1$}
	{$3$}
...
%%==========Câu 9
\begin{ex}%[Lê Hồng Phi]%[Tex hóa Đề Vted 2023]%[2D1B5-5]
	\immini[thm]{Cho hàm số bậc bốn $f(x)$ có đồ thị của đạo hàm như hình bên. Số điểm cực trị của hàm số $f(x)$ là
		\choice
		{\True $1$}
		{$0$}
		{$2$}
		{$3$}}{\begin{tikzpicture}[scale=1, font=\footnotesize, line join=round, line cap=round,>=stealth,x=0.5cm,y=0.5cm]
			\def \xmin{-2.7};
			\def \xmax{3.1};
			\def \ymin{-4.5};
			\def \ymax{1.5};
...
\begin{ex}%[Lee Van Toan]%[Tex hóa Đề Vted 2023]%[2H3Y3-2]
	Trong không gian $Oxyz$, đường thẳng $d$ qua điểm $M(-1;2;3)$ và có một vectơ chỉ phương $\overrightarrow{u}=(2;-1;3)$ có phương trình là
	\choice
	...
	\loigiai{
		Đường thẳng $d$ qua điểm $M(-1;2;3)$ và có một vectơ chỉ phương $\overrightarrow{u}=(2;-1;3)$ có phương trình là
...
\begin{ex}%[Loc Do]%[Tex hóa Đề Vted 2023]%[2D1Y5-1]
	\immini[thm]{ Hàm số nào dưới đây có đồ thị như hình vẽ bên?
		\motcot
		{$y=x^3-3x+3$}
		{$y=x^4-2x^2+3$}
		{$y=-x^3+3x+3$}
		{\True $y=-x^4+2x^2+3$}}{			\begin{tikzpicture}[thick,>=stealth,scale=0.6] 
...
\begin{ex}%[Loc Do]%[Tex hóa Đề Vted 2023]%[2H3B2-3]
	Trong không gian $Oxyz$, mặt phẳng $(\alpha)$ đi qua $A(2;-2;-1)$ và song song với mặt phẳng $(\beta):x-y+2z+5=0$ có phương trình là
	\choice
	{$x-y+2z+2=0$}
	{$x-y-2z-6=0$}
	{\True $x-y+2z-2=0$}
...
\begin{ex}%[Lương Như Quỳnh]%[Tex hóa Đề Vted 2023]%[2H1B3-4]
	\immini[thm]{Cho hình lăng trụ đứng $ABC.A'B'C'$ có đáy $ ABC $ là tam giác vuông cân tại $ A $ và \break$ AB=AA'=4 $ (tham khảo hình bên). Góc giữa hai đường thẳng $ A'B $ và $ B'C' $ bằng 
		\choice
		{$90^\circ$}
		{$30^\circ$}
		{$45^\circ$}
...
\begin{ex}%[Lương Như Quỳnh]%[Tex hóa Đề Vted 2023]%[1H3B5-3]
	\immini[thm]{Hình chóp $ S. ABC $ có đáy $ABC$ vuông tại $B$, tam giác $ SAB $ vuông tại $B$, tam giác $ SBC $ vuông tại $S$. Biết $AB=a$, $SA=2a$, $BC=4a$ (tham khảo hình vẽ). Khoảng cách từ $C$ đến mặt phẳng $(SAB)$ bằng
		\choice
		{\True $\sqrt{13}a$}
		{$\sqrt{15}a$}
		{$4a$}
...
\begin{ex}%[Lương Như Quỳnh]%[Tex hóa Đề Vted 2023]%[2H3K3-8]
	Trong không gian $Oxyz$, cho đường thẳng $ \Delta \colon \dfrac{x-1}{-1}=\dfrac{y}{2}=\dfrac{z+1}{1} $ và mặt phẳng $ (\alpha) \colon x+2y-2z-1=0 $. Biết mặt phẳng $ (P) $ chứa $ \Delta $ và tạo với $(\alpha)$ một góc nhỏ nhất có phương trình dạng $ 7x+by+cz+d=0 $. Giá trị $ b+c+d $ là 
	\choice
	{$-3$}
	{\True $-23$}
	{$3$}
	{$-5$}
	\loigiai{
		Ta có $A(1;0;-1) \in \Delta$; một vectơ chỉ phương của đường thẳng $ \Delta $ là $\overrightarrow{u}_{\Delta}=(-1;2;1)$; một vectơ pháp tuyến của mặt phẳng $ (P) $ là $ \overrightarrow{n}_P=(7 ; b;c) $.\\
...
\begin{ex}%[Đoàn Minh Tân]%[Tex hóa Đề Vted 2023]%[2H3G2-7]
	Trong không gian $Oxyz$, cho hai điểm $A(1;5;2)$ và $B(5;13;10)$. Gọi $(S)$ là mặt cầu đường kính $AB$. Xét điểm $M$ di động trên $(S)$ sao cho tiếp tuyến của $(S)$ tại $M$ cắt các mặt phẳng tiếp diện của $(S)$ tại $A$ và $B$ lần lượt tại $E$ và $F$. Khi $AE$ vuông góc với $BF$ và $ME=\dfrac{5}{2}MF$ thì độ dài đoạn $OE$ có giá trị nhỏ nhất bằng
	\choice
	{\True $5\sqrt{6}$}
	{$\sqrt{105}$}
	{$6\sqrt{5}$}
	{$3\sqrt{30}$}
	\loigiai{
		\immini[thm]{Mặt cầu $(S)$ có tâm $I(3;9;6)$, bán kính $R=IA=\sqrt{2^2+4^2+4^2}=6$.\\
			Đầu tiên, ta sẽ đi tìm quỹ tích điểm $E$ dựa trên giả thiết đề cho.\\
			Gọi $(P)$ là mặt phẳng tiếp diện của $(S)$ tại $A$.\\
			Khi đó $(P)$ qua $A(1;5;2)$ và có vectơ pháp tuyến $\overrightarrow{AB}=(4;8;8)=4(1;2;2)$ nên có phương trình $x+2y+2z-15=0$.\\
...
\begin{ex}%[Nguyen Huu Chung Kien]%[Tex hóa Đề Vted 2023] %[2D3G2-4] %Câu 46
	...
	\loigiai{
		...
		\allowdisplaybreaks
		\begin{eqnarray*}
			f(x)&=&\displaystyle\int f'(x) \mathrm{~d}x=\ln |x|+C=\heva{&
				\ln x+2,& x>0 \\&
...
\begin{ex}%[Nguyen Huu Chung Kien]%[Tex hóa Đề Vted 2023] %[2D1G2-2] %Câu 47
	\immini[thm]{Xét các số thực âm $a,$ $ b,$ $ c$ sao cho hàm số bậc ba $f(x)$ có đồ thị như hình vẽ bên. Hàm số $g(x)=|f\left(x f(x)\right)-c|$ có bao nhiêu điểm cực trị?
		\choice 
		{$15$}
		{$14$}
		{$11$}
		{\True $13$}}{\begin{tikzpicture}[scale=0.7, line join=round, line cap=round, >=stealth]
	...
	\end{tikzpicture}}
	\loigiai{
		Xét hàm số $u(x)=f(x f(x))-c \Rightarrow g(x)=|u(x)|$, ta đếm số lần đổi dấu của $u(x)$ và $u'(x)$.\\
		Ta có $f(x)=k \cdot(x-a)(x-b)^2,$ $\left(\mathop {\lim \limits_{n \to +\infty}}\limits_{x\to+\infty}  f(x)=-\infty \Rightarrow k<0\right)$ và có hai điểm cực trị $x=m ;$ $x=b$.\\
		...
		Xét $u'(x)=\left(f(x)+x f'(x)\right) \cdot f'(x f(x))=0 \Leftrightarrow\hoac{&f(x)+x f'(x)=0 \\& x f(x)=m \\& x f(x)=b.}$\\
		...
		\begin{eqnarray*}
			f(x)+x f'(x)&=&k(x-a)(x-b)^2+k x \cdot\left[(x-b)^2+2(x-a)(x-b)\right]\\
			&=&k(x-b)[(x-a)(x-b)+x(3 x-2 a-b)]\\
			&=&k(x-b)\left[4 x^2-(3 a+2 b) x+a b\right]
		\end{eqnarray*} có ba nghiệm phân biệt.\\
		Nên $u'(x)$ có $3+2+2=7$ lần đổi dấu, do đó $u(x)$ có $7$ điểm cực trị.\\ Vậy hàm số $g(x)=|u(x)|$ có $6+7=13$ điểm cực trị.\\
		\mathversion{bold}
...
\begin{ex}%[Nguyen Huu Chung Kien]%[Tex hóa Đề Vted 2023] %[2D2G5-5] %Câu 48
	...
	\loigiai{
		\begin{itemize}
			...
			\begin{eqnarray*}
				&&4 a^2-2 a \cdot 3^{b+\frac{1}{2}}+9^b-5^b=0 \Rightarrow \Delta_a'=3\cdot 9^b-4\left(9^b-5^b\right) \geq 0\\
...
\begin{ex}%[Đoàn Minh Tân]%[Tex hóa Đề Vted 2023]%[2D3G3-1]
	...
	{\True $\dfrac{587}{12}-36\ln 2$}
	{$\dfrac{701}{12}-30\ln 2$}
	\loigiai{
		...
		$$S=\displaystyle\int\limits_1^2\left|g(x)-h(x)\right|\mathrm{\,d}x=\displaystyle\int\limits_1^2\left|\dfrac{(x+3)(x-1)(3x^2-8x-12)}{x}\right|\mathrm{\,d}x=\dfrac{587}{12}-36\ln 2.$$
	}
\end{ex}
